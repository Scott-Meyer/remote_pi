/* ===========================================================
   Remote Pi FlightDeck — release manifest
   ...
   configurable (NEXT_PUBLIC_FLIGHTDECK_MANIFEST_URL) and the loader falls
   back to MOCK_MANIFEST — same shape — whenever the fetch fails. The page
   still renders, just flagged "not yet published".
   =========================================================== */

export type FlightDeckPlatform = "macos" | "windows" | "linux";
export type FlightDeckArch = "universal" | "x64" | "arm64";
export type FlightDeckFormat = "dmg" | "exe" | "deb" | "rpm";

/** One downloadable build. Every field is produced by flightdeck-release.yml. */
export type FlightDeckArtifact = {
  platform: FlightDeckPlatform;
  arch: FlightDeckArch;
  format: FlightDeckFormat;
  url: string;
  sha256: string;
  size: number;
};

export type FlightDeckManifest = {
  version: string;
  date: string;
  notes: string;
  artifacts: FlightDeckArtifact[];
};

/**
 * Where the live manifest lives. Set NEXT_PUBLIC_FLIGHTDECK_MANIFEST_URL once
 * the release pipeline publishes to the VPS; until then this default 404s
 * and the page falls back to MOCK_MANIFEST.
 */
export const MANIFEST_URL =
  process.env.NEXT_PUBLIC_FLIGHTDECK_MANIFEST_URL ??
  "https://rp-s3.jacobmoura.work/downloads/flightdeck/latest.json";
 ...
 */
export const MOCK_MANIFEST: FlightDeckManifest = {
  version: "1.0.0",
  date: "2026-06-12",
  notes:
    "First public FlightDeck build. Pair from the desktop, watch live agent sessions, and manage 24/7 daemons and schedules.",
  artifacts: [
    {
      platform: "macos",
      arch: "universal",
      format: "dmg",
      url: "https://remote-pi.jacobmoura.work/downloads/flightdeck/1.0.0/RemotePiFlightDeck-1.0.0-macos-universal.dmg",
    ...
    },
    {
      ...
      arch: "x64",
      format: "exe",
      url: "https://remote-pi.jacobmoura.work/downloads/flightdeck/1.0.0/RemotePiFlightDeck-Setup-1.0.0-windows-x64.exe",
    ...
    },
    {
      ...
      arch: "x64",
      format: "deb",
      url: "https://remote-pi.jacobmoura.work/downloads/flightdeck/1.0.0/remote-pi-flightdeck_1.0.0_amd64.deb",
    ...
    },
    {
      ...
      arch: "arm64",
      format: "deb",
      url: "https://remote-pi.jacobmoura.work/downloads/flightdeck/1.0.0/remote-pi-flightdeck_1.0.0_arm64.deb",
    ...
    },
    {
      ...
      arch: "x64",
      format: "rpm",
      url: "https://remote-pi.jacobmoura.work/downloads/flightdeck/1.0.0/remote-pi-flightdeck-1.0.0.x86_64.rpm",
    ...
    },
    {
      ...
      arch: "arm64",
      format: "rpm",
      url: "https://remote-pi.jacobmoura.work/downloads/flightdeck/1.0.0/remote-pi-flightdeck-1.0.0.aarch64.rpm",
...
function isManifest(d: unknown): d is FlightDeckManifest {
 ...
 */
export async function loadFlightDeckManifest(): Promise<ManifestLoad> {
  try {
...
export const ARCH_LABEL: Record<FlightDeckArch, string> = {
