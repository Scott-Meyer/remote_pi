...
export const metadata: Metadata = {
  title: "FlightDeck reference",
  description:
    "Reference for Remote Pi FlightDeck: the internal CLI, .ckp pane layouts, .flightdeck/tasks.json Task Run, the theme format, agent turn status hooks, and databases.",
};

const GITHUB_URL = "https://github.com/jacobaraujo7/remote_pi";
const FLIGHTDECK_DOCS =
  "https://github.com/jacobaraujo7/remote_pi/tree/main/flightdeck/docs";
const THEME_SCHEMA =
  "https://raw.githubusercontent.com/jacobaraujo7/remote_pi/main/flightdeck/docs/theme.schema.json";
const TASKS_SCHEMA =
  "https://github.com/jacobaraujo7/remote_pi/blob/main/flightdeck/docs/tasks.schema.json";
const THEME_EXAMPLE =
  "https://github.com/jacobaraujo7/remote_pi/blob/main/flightdeck/docs/theme.example.json";

const DOCS_TOC: TocItem[] = [
  { id: "install", label: "Install" },
  {
    id: "cli",
    label: <>The <InlineCode>flightdeck</InlineCode> CLI</>,
    sub: [
...
export default function FlightDeckDocsPage() {
  return (
    <div className="page">
      <div className="page-body">
        <div className="wrap">
          <header className="page-head reveal">
            <span className="eyebrow">Documentation · FlightDeck</span>
            <h1>FlightDeck reference</h1>
            ...
            <p className="lede">
              ...
              your machine: the internal <InlineCode>flightdeck</InlineCode> CLI
              that agents use to drive tabs, the{" "}
              <InlineCode>.ckp</InlineCode> pane layouts, the{" "}
              <InlineCode>.flightdeck/tasks.json</InlineCode> Task Run file, the
              theme format, and the harness hooks behind the turn status. For
              the product tour, see the{" "}
              <Link href="/flightdeck" className="text-accent underline">
                FlightDeck page
              </Link>
          ...
          <div className="docs-layout">
            ...
            <article className="prose docs-article">
              ...
              <DocsSection id="install" title="Install">
                <p>
                  FlightDeck ships for macOS, Windows, and Linux. macOS builds are
                ...
                <p>
                  FlightDeck is a terminal first: it needs no account, no cloud,
              ...
              <DocsSection id="cli" title="The flightdeck CLI">
                <p>
                  FlightDeck materializes a small binary at{" "}
                  <InlineCode>~/.flightdeck/bin/flightdeck</InlineCode> and puts that
                  folder on the <InlineCode>PATH</InlineCode> of the terminals
                  it spawns — and only those. So the CLI exists for anything
                  running inside FlightDeck (you, a script, an agent) and does not
                ...
                <p>
                  This is what makes FlightDeck an <em>agentic</em> multiplexer:
                ...
                <DocsSubsection id="cli-targets" title="Targets & ids">
                    ...
                    rows={[
                      [
                        ...
                        <>
                          ...
                          <InlineCode>$FLIGHTDECK_TAB_ID</InlineCode> (the current
                          tab; legacy fallback{" "}
                          <InlineCode>$FLIGHTDECK_PANE_ID</InlineCode>).
                        </>,
                      ],
                      [
                        ...
                        <>
                          ...
                          the app. Works from outside a FlightDeck terminal too
                          (dictation tools, scripts): with no env inherited the
                          CLI finds the app through{" "}
                          <InlineCode>~/.flightdeck/status.sock</InlineCode>. Wins
                  ...
                  <Callout variant="warning" title="Tab ids reset on boot">
                    <p>
                      ...
                      <InlineCode>flightdeck list-tabs</InlineCode>, or give a tab
                    ...
                    rows={[
                      ...
                      [
                        ...
                        <>
                          ...
                          <InlineCode>flightdeck &lt;file&gt;</InlineCode> is a
...
flightdeck read-tab Worker --lines 50

# run and follow a project task
flightdeck list-tasks
flightdeck read-task npm:dev --lines 80
...
flightdeck db query --db dev-local --sql "SELECT * FROM orders LIMIT 5"`}
                ...
                <DocsSubsection id="cli-read" title="Reading output">
                  ...
                  <p>
                    ...
                    <InlineCode>.flightdeck/tasks.json</InlineCode>.
                  </p>
                </DocsSubsection>
              </DocsSection>
              ...
              <DocsSection id="layouts" title=".ckp pane layouts">
                ...
                <ul>
                  ...
                  <li>
                    <strong>CLI</strong> —{" "}
                    <InlineCode>flightdeck orchestrate dev.ckp</InlineCode> from
                    inside a tab.
                  </li>
                  <li>
                ...
                <DocsSubsection id="layouts-merge" title="Merge semantics">
                  ...
                  <p>
                    FlightDeck treats <InlineCode>.ckp</InlineCode> as YAML for
                    highlighting and shows the FlightDeck logo as the file icon in
                    the tree.
                  </p>
                </DocsSubsection>
              </DocsSection>
              ...
              <DocsSection id="tasks" title="Task Run">
                ...
                <ul>
                  <li>
                    ...
                    FlightDeck reads the manifests (
                  ...
                  <li>
                    ...
                    <InlineCode>.flightdeck/tasks.json</InlineCode>, for
                ...
                <DocsSubsection id="tasks-file" title="Where the file lives">
                  <p>
                    ...
                    Discovery is literal — FlightDeck does not walk up the tree.
                    For a single package, open the package folder. For a{" "}
                    <strong>monorepo</strong>, open the root and let one{" "}
                    <InlineCode>.flightdeck/tasks.json</InlineCode> drive the
                  ...
                  <p>
                    ...
                    for editor autocomplete; FlightDeck ignores the field when
                    running.
                  </p>
                  <CodeBlock
              ...
              <DocsSection id="databases" title="Databases">
                <p>
                  ...
                  <InlineCode>.flightdeck/databases.json</InlineCode>, plus any
                  SQLite files FlightDeck auto-detects in the project. SQLite,
                ...
                <CodeBlock
                  label="FlightDeck terminal"
                  prompt
                  code={`flightdeck db list
flightdeck db schema --db dev-local orders
flightdeck db query --db dev-local --sql "SELECT * FROM orders LIMIT 5"
flightdeck db run reports/daily.dbq

# non-SQL engines have their own verbs
flightdeck redis --db cache --command "SCAN 0 COUNT 20"
flightdeck mongo --db atlas --database shop --command '{"find":"orders","limit":5}'

# open the same thing visually for a human
flightdeck redis browse --db cache
flightdeck mongo browse --db atlas --database shop`}
                />
                <Callout variant="note" title="Per-connection guardrails">
                  <p>
                    ...
                    <InlineCode>flightdeck db --help</InlineCode> for the full
                    surface.
                  </p>
                </Callout>
              </DocsSection>
              ...
              <DocsSection id="themes" title="Themes">
                <p>
                  FlightDeck ships nine built-in themes —{" "}
                  <InlineCode>flightdeck</InlineCode>,{" "}
                  <InlineCode>flightdeck.2</InlineCode>,{" "}
                    ...
                    rows={[
                      ...
                      [
                        ...
                        <>
                          ...
                          inherits from <InlineCode>flightdeck</InlineCode>.
                        </>,
                      ],
                      [
                ...
                <DocsSubsection id="themes-tokens" title="Tokens">
                  ...
                  <p>
                    ...
                    so the app and the schema can never drift apart. FlightDeck
              ...
              <DocsSection id="turn-status" title="Agent turn status">
                <p>
                  When an agent runs in a tab, FlightDeck shows whether it is{" "}
                ...
                <ol>
                  <li>
                    At boot, FlightDeck materializes the internal CLI at{" "}
                    <InlineCode>~/.flightdeck/bin/flightdeck</InlineCode> and
                    registers <InlineCode>flightdeck hook</InlineCode> on the
                  ...
                  <li>
                    Routing is by the <InlineCode>FLIGHTDECK_PANE_ID</InlineCode>{" "}
                    env var, which the app injects into the tab&apos;s PTY. An
                    agent session started <em>outside</em> FlightDeck does not have
                  ...
                  rows={[
                    ...
                    [
                      ...
                      <>
                        ...
                        silently ignores an untrusted hook, so FlightDeck computes
                        and writes the trust hash for you, between{" "}
                        <InlineCode>{"# >>> flightdeck hooks"}</InlineCode>{" "}
                        delimiters).
                      </>,
                    ],
                  ]}
                ...
                <p>
                  ...
                  marked entry (<InlineCode>_flightdeck: v1</InlineCode>):
                ...
                >
                  <p>
                    ...
                    <InlineCode>flightdeck hook --harness &lt;name&gt;</InlineCode>{" "}
              ...
              <DocsSection id="sounds" title="Sounds & notifications">
                <p>
                  ...
                  notification. Since an agent started outside FlightDeck never
              ...
              <DocsSection id="language" title="Language">
                <p>
                  FlightDeck&apos;s interface is fully localized in{" "}
                ...
                <ul>
                  ...
                  <li>
                    <Link
                      ...
                      className="text-accent underline"
                    >
                      Tutorial: an agent team in FlightDeck
                    </Link>
                    .
                  </li>
                  <li>
