// flightdeck-server (plano 58, Wave 0) — binário headless, sem Flutter.
//
...
Future<void> main(List<String> args) async {
  ...
  await runZonedGuarded(
    ...
    (error, stack) {
      stderr.writeln('flightdeck-server: erro não tratado: $error');
...
Future<void> _run(List<String> args) async {
  final socketPath =
      ...
      '${Directory.systemTemp.path}/flightdeck-server-$pid.sock';
  ...
  if (idleSeconds != null && idleSeconds > 0) {
    ...
    server.onIdleExit = () async {
      stdout.writeln('flightdeck-server idle, exiting');
      await server.close();
      exit(0);
    };
  }
  ...
  // HOST, apontando pra CLI `flightdeck hook` (resolvida por --cli, ao lado do
  ...
  stdout.writeln('flightdeck-server listening on $socketPath');
  ...
  Future<void> shutdown() async {
    try {
      ...
      stderr.writeln('flightdeck-server: desligamento forçado ($e)');
    }
    exit(0);
  }
