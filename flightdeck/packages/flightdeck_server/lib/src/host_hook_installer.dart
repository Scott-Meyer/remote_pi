...
/// headless (sem Flutter) — roda dentro do `flightdeck-server`. Assim, uma sessão
/// de agente num terminal REMOTO reporta o turno via `flightdeck hook` → socket de
/// status do host → protocolo → cliente (spinner/chime).
///
/// Idempotente: remove/reinsere só o entry marcado (`_flightdeck`), preservando
...
class HostHookInstaller {
  ...
  /// Caminho explícito da CLI `flightdeck` (arg `--cli` do server). Se nulo,
  /// resolve ao lado do executável do server ou no PATH.
  final String? cliPathOverride;

  static const String _marker = '_flightdeck';
  ...
  /// Resolve a CLI `flightdeck` que sabe o subcomando `hook`. Público porque o
  /// servidor também precisa do caminho: é a pasta dela que entra no PATH das
  /// PTYs, para `flightdeck …` resolver num terminal remoto. Ordem: override →
  ...
  Future<String?> resolveCli() async {
    final candidates = <String?>[
      if (cliPathOverride != null && cliPathOverride!.isNotEmpty)
        cliPathOverride!,
      _besideServer('flightdeck'),
      await _which('flightdeck'),
    ].whereType<String>();
