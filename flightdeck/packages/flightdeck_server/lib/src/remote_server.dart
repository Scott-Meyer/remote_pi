...
import 'package:flightdeck_protocol/flightdeck_protocol.dart';
...
/// Servidor do protocolo FlightDeck Remote sobre socket local (UDS).
///
...
class RemoteServer {
  /// Caminho da CLI `flightdeck` **neste host**, resolvido no bootstrap. A pasta
  /// dela entra no PATH de cada PTY, para que `flightdeck …` funcione num
  ...
  /// Vigia a posse do caminho anunciado. Um FlightDeck novo bindando por cima
  ...
  /// o FlightDeck do outro lado. Vai pelo RPC do protocolo, na direção inversa da
  ...
  Future<Map<String, Object?>> _forwardCommandToClient(
    ...
    if (_connections.isEmpty) {
      return <String, Object?>{
        'ok': false,
        'error': 'no FlightDeck client attached to this host',
      };
    }
    ...
    if (connection == null) {
      if (_connections.length > 1) {
        return <String, Object?>{
          'ok': false,
          'error': tabId.isEmpty
              ? 'more than one FlightDeck is attached to this host: run the '
                    'command from a terminal tab, so it knows which one to ask'
              : 'no attached FlightDeck owns tab "$tabId" (it may predate the '
    ...
    try {
      ...
      return <String, Object?>{
        'ok': false,
        'error':
            'the FlightDeck client did not answer in '
...
const _statusEnvKeys = <String>{
  'FLIGHTDECK_STATUS_SOCK',
  'FLIGHTDECK_STATUS_PORT',
  'FLIGHTDECK_STATUS_TOKEN',
};
...
class _Connection {
  ...
  /// `FLIGHTDECK_TAB_ID` de cada PTY que ESTE cliente abriu. É o que diz de quem
  ...
  /// (`FLIGHTDECK_STATUS_SOCK` no POSIX; `FLIGHTDECK_STATUS_PORT` + `_TOKEN` no
    ...
    try {
      ...
      switch (message) {
        case PtyOpen():
          ...
          // cliente já manda FLIGHTDECK_PANE_ID); assim o hook do agente no host
          ...
          final env = _statusEnv.isEmpty || _localClient
              ...
              : {
                  ...
                  // FLIGHTDECK_STATUS_SOCK ANTES de porta/token (cli/src/
                  ...
                  // saber que os comandos são atendidos pelo FlightDeck do OUTRO
                  // lado do SSH — e que nem todos atravessam.
                  'FLIGHTDECK_REMOTE': '1',
                  // Deixa o binário `flightdeck` do host alcançável no shell. O
          ...
          final tabId = env['FLIGHTDECK_TAB_ID'];
  ...
  /// Prefixa a pasta da CLI `flightdeck` deste host no PATH da PTY, para que
  /// `flightdeck …` resolva num terminal remoto como resolve num local. Sem a CLI
...
/// UNIX local onde o `flightdeck hook` (rodando junto do agente na PTY) escreve
...
class TurnStatusReceiver {
  ...
  /// Comando da CLI `flightdeck` rodando NO HOST (`{"type":"cmd",…}`). O host não
  ...
  /// porta + token — as duas formas que o `flightdeck hook` já sabe ler.
  Map<String, String> get hookEnv {
    final endpoint = _endpoint;
    if (endpoint == null) return const <String, String>{};
    final token = endpoint.token;
    if (token == null) {
      return <String, String>{'FLIGHTDECK_STATUS_SOCK': endpoint.path};
    }
    return <String, String>{
      'FLIGHTDECK_STATUS_PORT': '${endpoint.listener.port}',
      'FLIGHTDECK_STATUS_TOKEN': token,
    };
  }
