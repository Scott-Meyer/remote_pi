...
import 'package:flightdeck_server/flightdeck_server.dart';
...
void main() {
  ...
  setUp(() {
    dir = Directory.systemTemp.createTempSync('flightdeck-transport');
    path = '${dir.path}/flightdeck-server.sock';
  ...
  test('cliente local mantém o env de status que mandou', () async {
    ...
    // O socket do app é a via da CLI interna (`flightdeck send`) além do status;
    // sobrescrevê-lo num PTY local deixaria a CLI muda em toda aba.
    expect(terminals.lastEnv['FLIGHTDECK_STATUS_SOCK'], '/from/client.sock');
  });
  ...
  test('cliente remoto tem o env de status trocado pelo do servidor', () async {
    ...
    // FLIGHTDECK_STATUS_SOCK herdado à porta+token, e o status se perderia.
    expect(terminals.lastEnv['FLIGHTDECK_STATUS_SOCK'], isNull);
    expect(terminals.lastEnv['FLIGHTDECK_STATUS_PORT'], isNotNull);
    expect(terminals.lastEnv['FLIGHTDECK_STATUS_TOKEN'], isNotNull);
  });
}
  ...
  socket.add(
    utf8.encode(
      codec.encode(
        const PtyOpen(
          executable: '/bin/zsh',
          environment: {'FLIGHTDECK_STATUS_SOCK': '/from/client.sock'},
        ),
      ),
    ),
  );
