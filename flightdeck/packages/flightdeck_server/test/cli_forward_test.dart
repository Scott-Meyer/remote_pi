// A CLI `flightdeck` rodando NO HOST não executa nada: o servidor encaminha o
...
void main() {
  ...
  setUp(() {
    dir = Directory.systemTemp.createTempSync('flightdeck-cli-forward');
  ...
  test(
    ...
    () async {
      ...
      // Cliente que se comporta como o FlightDeck: responde ao `cli.cmd`.
  ...
  test('sem cliente conectado, a CLI recebe erro em vez de pendurar', () async {
    ...
    expect(reply['ok'], isFalse);
    expect(reply['error'], contains('no FlightDeck client'));
  });
  ...
  test('aba órfã com vários clientes explica em vez de chutar', () async {
    ...
    expect(reply['ok'], isFalse);
    expect(reply['error'], contains('no attached FlightDeck owns tab'));
  });
...
/// Abre uma PTY por esta conexão declarando `FLIGHTDECK_TAB_ID` — é assim que o
...
Future<void> _claimTab(_TestClient client, String tabId) async {
  ...
  client.send(
    PtyOpen(executable: '/bin/zsh', environment: {'FLIGHTDECK_TAB_ID': tabId}),
  );
...
/// Conecta como cliente FlightDeck (handshake completo) e devolve a conexão.
