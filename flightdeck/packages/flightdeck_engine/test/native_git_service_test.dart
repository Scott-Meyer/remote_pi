...
void main() {
  ...
  test('pasta nova é enumerada arquivo a arquivo (-uall)', () async {
    // Sem `-uall` o git colapsa numa entrada só (`?? .flightdeck/`) e o cliente
    // desenhava a pasta como se fosse um arquivo no Source Control.
    Directory('${repo.path}/.flightdeck').createSync();
    File('${repo.path}/.flightdeck/tasks.json').writeAsStringSync('{}\n');
    File('${repo.path}/.flightdeck/databases.json').writeAsStringSync('{}\n');

    final status = await git.status(repo.path);
    final paths = status.files.map((f) => f.path).toSet();
    expect(paths, contains('.flightdeck/tasks.json'));
    expect(paths, contains('.flightdeck/databases.json'));
    expect(
