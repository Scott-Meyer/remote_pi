...
void main() {
  ...
  setUp(() {
    dir = Directory.systemTemp.createTempSync('flightdeck-endpoint');
  ...
  test('HelloAck leva a identidade do servidor (adoção do sidecar)', () {
    const ack = HelloAck(
      ...
      server: '0.1.0',
      executable: r'C:\Program Files\FlightDeck\flightdeck-server.exe',
      pid: 4242,
    );
    final json = ack.toJson();
    expect(json['exe'], r'C:\Program Files\FlightDeck\flightdeck-server.exe');
...
/// Posse do caminho anunciado: um FlightDeck novo bindando por cima deixa o
...
void ownershipTests() {
  ...
  setUp(() {
    dir = Directory.systemTemp.createTempSync('flightdeck-owner');
    path = '${dir.path}/flightdeck-server.sock';
  });
  tearDown(() => dir.deleteSync(recursive: true));
