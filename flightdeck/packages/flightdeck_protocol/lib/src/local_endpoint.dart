...
/// Ponto de encontro local entre o `flightdeck-server` e quem fala com ele na
...
/// permissões do `~/.flightdeck`. Ele viaja no `Hello` (campo `tok`) e o servidor
...
class LocalEndpoint {
  ...
  static Future<LocalSocket> connect(String path) async {
    ...
    if (decoded is! Map) {
      throw const FormatException('invalid flightdeck endpoint file');
    }
    final port = decoded['port'];
    if (port is! int) {
      throw const FormatException('flightdeck endpoint file without port');
    }
...
class LocalListener {
  ...
  ///
  /// Um segundo FlightDeck subindo binda o mesmo caminho por cima e o servidor
