...
// 1. instala o flightdeck-server no host via ssh (binário local → stdin → arquivo)
...
Future<void> main(List<String> args) async {
  ...
  final binary = '$root/build/wave0/flightdeck-server';
  final dylib = '$root/build/wave0/libflightdeck_pty.dylib';
  ...
  try {
    ...
    await _push(target, binary, '~/.flightdeck/bin/flightdeck-server');
    await _push(target, dylib, '~/.flightdeck/bin/libflightdeck_pty.dylib');
    ...
    await _ssh(
      target,
      'FLIGHTDECK_PTY_DYLIB=\$HOME/.flightdeck/bin/libflightdeck_pty.dylib '
      'nohup \$HOME/.flightdeck/bin/flightdeck-server '
      '--socket \$HOME/.flightdeck/flightdeck-server.sock --exit-on-idle 0 '
    ...
    final remoteSock = '$remoteHome/.flightdeck/flightdeck-server.sock';
    final localSock =
...
Future<void> _push(String target, String local, String remote) async {
  final process = await Process.start('ssh', [
    ...
    'BatchMode=yes',
    target,
    'mkdir -p ~/.flightdeck/bin && cat > $remote && chmod +x $remote',
  ]);
