...
// abre shell via flightdeck-server, digita, vê saída, DESCONECTA, reata a partir
...
Future<void> main() async {
  ...
  final server = await Process.start(
    ...
    environment: {
      ...
      'FLIGHTDECK_PTY_DYLIB': '$repoRoot/build/wave0/libflightdeck_pty.dylib',
    },
  );
...
String _flightdeckRoot() {
  // O script roda de flightdeck/ (dart run tool/wave0/...) ou de tool/wave0.
  var dir = Directory.current;
  while (!File('${dir.path}/pubspec.yaml').existsSync() ||
      !Directory('${dir.path}/packages/flightdeck_server').existsSync()) {
    final parent = dir.parent;
    if (parent.path == dir.path) {
      throw StateError('run from the flightdeck/ folder');
    }
    dir = parent;
  }
  return dir.path;
