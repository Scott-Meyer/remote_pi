...
// in-process) vs caminho loopback (flightdeck-server + UDS + JSONL/base64).
...
Future<void> main() async {
  final repoRoot = _flightdeckRoot();
  final dylib = '$repoRoot/build/wave0/libflightdeck_pty.dylib';
  ...
  stdout.writeln('== loopback (flightdeck-server via UDS) ==');
  final socketPath =
  ...
  final server = await Process.start(
    ...
    [
      'run',
      'packages/flightdeck_server/bin/flightdeck_server.dart',
      '--socket',
      socketPath,
    ],
    workingDirectory: repoRoot,
    environment: {...Platform.environment, 'FLIGHTDECK_PTY_DYLIB': dylib},
  );
  await server.stdout
...
String _flightdeckRoot() {
  var dir = Directory.current;
  while (!Directory('${dir.path}/packages/flightdeck_server').existsSync()) {
    final parent = dir.parent;
    if (parent.path == dir.path) throw StateError('run from flightdeck/');
    dir = parent;
  }
  return dir.path;
}
