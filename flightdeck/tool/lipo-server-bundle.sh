#!/usr/bin/env bash
# Junta a fatia x86_64 do flightdeck-server (plano 58) ao bundle arm64 do host,
...
#                    Resources/flightdeck-server-bundle)
...
#
# - dylibs e binários nativos (bin/flightdeck, a CLI em Rust) viram Mach-O
#   universais via `lipo -create`. É o caminho normal do macOS.
#
# - o executável do servidor (`bin/flightdeck-server`) é um AOT do Dart, e AOT do
...
#   Então ele sai do bundle como DOIS arquivos finos, `flightdeck-server-arm64` e
#   `flightdeck-server-x64`, e quem escolhe é o runtime (ver
...
# O resto de bin/ e todas as dylibs. libflightdeck_pty já sai universal do
