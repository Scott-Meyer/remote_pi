#!/usr/bin/env bash
# Cross-compiles the flightdeck-server bundle deployed by a macOS client to Linux
...
(
  cd "$ROOT/packages/flightdeck_server"
...
# flightdeck_pty is intentionally not a Dart native asset yet. Build the same C
...
"$ZIG" cc -target aarch64-linux-gnu -shared -O2 -DDART_SHARED_LIB \
  -o "$DEST/lib/libflightdeck_pty.so" \
  "$ROOT/plugins/flightdeck_pty/src/flightdeck_pty.c" \
  -I"$ROOT/plugins/flightdeck_pty/src" -lpthread
...
cp "$ROOT/cli/target/$TARGET/release/flightdeck" "$DEST/bin/flightdeck"
chmod +x "$DEST/bin/flightdeck-server" "$DEST/bin/flightdeck"
