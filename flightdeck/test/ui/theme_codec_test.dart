...
FlightDeckThemeSpec parse(String source) => FlightDeckThemeSpec.fromJson(
  jsonDecode(source) as Map<String, Object?>,
  resolveBase: builtInThemeById,
  fallbackBase: flightdeckDefaultTheme,
);
...
void main() {
  ...
  group('FlightDeckThemeSpec.fromJson', () {
    test('tema mínimo herda todo o resto do tema nativo', () {
      ...
      expect(dark.terminal.background, flightdeckTerminalThemeDark.background);
    });

    test('extends usa o built-in nomeado como base', () {
      final theme = parse('''
        {
          "id": "acme.on-flightdeck",
          "name": "On FlightDeck",
          "extends": "flightdeck",
          "variants": { "dark": { "ui": { "accent": "#FF0088" } } }
        }
    ...
    test('export é autônomo: reimportar dá o mesmo tema', () {
      final original = parse('''
        {
          "id": "acme.on-flightdeck", "name": "On FlightDeck",
          "extends": "flightdeck",
          "variants": { "dark": { "ui": { "accent": "#FF0088" } } }
        }
      ''');
  ...
  group('temas built-in', () {
    ...
    test('FlightDeck: o chrome é mais fundo que o corpo da aba', () {
      for (final b in Brightness.values) {
        final ui = flightdeckDefaultTheme.variantFor(b).ui;
        expect(
          lum(ui.bg),
    ...
    test('FlightDeck 2: inverte — o corpo da aba é mais fundo que o chrome', () {
      for (final b in Brightness.values) {
        final ui = flightdeck2Theme.variantFor(b).ui;
        expect(
          lum(ui.panel),
          lessThan(lum(ui.bg)),
          reason: 'a inversão é a razão de existir do FlightDeck 2 ($b)',
        );
      }
    });

    test(
      'FlightDeck 2: as camadas elevadas ficam acima do chrome E do conteúdo',
      () {
        for (final b in Brightness.values) {
          final ui = flightdeck2Theme.variantFor(b).ui;
    ...
    test('FlightDeck 2: o terminal acompanha o fundo da aba', () {
      for (final b in Brightness.values) {
        final variant = flightdeck2Theme.variantFor(b);
    ...
    test('tema importado sem `syntax`/`terminal` segue o próprio campo', () {
      ...
      // ficaria com o campo do FlightDeck dentro das próprias superfícies.
      final theme = parse('''
        {
    ...
    test(
      'Violet herda a estrutura de superfícies do FlightDeck que o originou',
      () {
        // A razão de o Violet ser derivado (e não escrito à mão) é essa: ele
        // acompanha a calibragem do FlightDeck correspondente de graça. Se alguém
        // reescrever o Violet com hexes soltos, esta comparação denuncia.
        for (final (base, violet) in [
          (flightdeckDefaultTheme, violet1Theme),
          (flightdeck2Theme, violet2Theme),
        ]) {
          for (final b in Brightness.values) {
    ...
    test('Midnight muda de arranjo conforme o brilho — de propósito', () {
      // Escuro: inverte (conteúdo afunda), como o FlightDeck 2.
      final dark = midnightTheme.variantFor(Brightness.dark).ui;
      expect(
        lum(dark.panel),
        lessThan(lum(dark.bg)),
        reason: 'no escuro o Midnight herda a inversão do FlightDeck 2',
      );
      ...
      expect(
        ...
        reason: 'no claro o Midnight volta ao arranjo do FlightDeck 1',
      );

      // E os painéis do claro são mais fundos que os do FlightDeck 1: é o que
      // devolve a hierarquia que o modo claro perde.
      expect(lum(light.bg), lessThan(lum(flightdeckDefaultTheme.light!.ui.bg)));
    });
    ...
    test('painel com tom não deixa o campo claro sem tom', () {
      ...
      // neutro (o FlightDeck oficial, o Pantera) não tem tom a levar para o campo,
  ...
  group('docs/theme.schema.json', () {
    ...
    test('os tokens do schema são exatamente os que o codec serializa', () {
      ...
      expect(
        ...
        terminalThemeToJson(flightdeckTerminalThemeDark).keys.toSet(),
      );
    });
