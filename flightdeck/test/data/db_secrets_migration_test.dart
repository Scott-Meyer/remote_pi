...
/// remoto no arquivo. Como o `flightdeck-server` é headless e nunca teve acesso ao
...
void main() {
  ...
  setUp(() {
    dir = Directory.systemTemp.createTempSync('flightdeck-secrets-migration');
  ...
  test('a chave local é a MESMA que o host usa', () {
    ...
    // o flightdeck-server procura ao atender um cliente remoto.
    expect(
  ...
  test('a chave não depende mais do workspaceId', () {
    ...
    expect(deUmCliente, isNot(contains('flightdeck.db.')));
  });
  ...
  test('migra do cofre do SO na primeira leitura e apaga de lá', () async {
    final legacy = _SpyLegacy({'flightdeck.db.w1.dev': 'senha-do-keychain'});
    final secrets = DbSecretsImpl(store: store, legacy: legacy);
    const key = 'chave-nova';

    expect(
      await secrets.read(key, legacyKey: 'flightdeck.db.w1.dev'),
      'senha-do-keychain',
    );

    // Ficou no cofre único — é o que o flightdeck-server lê.
    ...
    legacy.reads.clear();
    expect(await secrets.read(key, legacyKey: 'flightdeck.db.w1.dev'), isNotNull);
