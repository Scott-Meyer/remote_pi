...
/// O alias curto `deck` é o MESMO binário da CLI, criado ao lado dela: symlink
...
void main() {
  ...
  setUp(() async {
    ...
    await File('${dir.path}/flightdeck').writeAsString('#!/bin/sh\necho oi\n');
  });
  tearDown(() async {
    if (await dir.exists()) await dir.delete(recursive: true);
  });

  String aliasPath() => '${dir.path}/${Platform.isWindows ? 'deck.exe' : 'deck'}';

  test('cria o alias apontando para a CLI', () async {
    await installer.ensureShortAlias(dir.path, 'flightdeck');
    ...
    if (!Platform.isWindows) {
      ...
      expect(Link(aliasPath()).targetSync(), 'flightdeck');
    }
  ...
  test('sem CLI materializada, não inventa alias', () async {
    ...
    await installer.ensureShortAlias(vazio.path, 'flightdeck');
    expect(
      File(
        '${vazio.path}/${Platform.isWindows ? 'deck.exe' : 'deck'}',
      ).existsSync(),
      isFalse,
    );
  });
