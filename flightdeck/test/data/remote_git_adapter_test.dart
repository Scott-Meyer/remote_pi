...
import 'package:flightdeck_core/flightdeck_core.dart' as core;
...
void main() {
  ...
  test('pasta untracked colapsada não vira "arquivo" com barra na chave', () {
    ...
    // guardada com a barra fazia a árvore desenhar `.flightdeck` como arquivo.
    final info = remoteGitInfo(status('main', [('?', '?', '.flightdeck/')]));

    expect(info.files.keys, ['.flightdeck']);
    expect(info.files['.flightdeck'], GitFileStatus.untracked);
    expect(info.untrackedDirs, {'.flightdeck'});
    // E os descendentes herdam o untracked, mesmo sem terem sido enumerados.
    expect(info.isUntracked('.flightdeck/tasks.json'), isTrue);
  });
