...
void main() {
  ...
  setUp(() async {
    repo = await Directory.systemTemp.createTemp('flightdeck_git_test_');
    await git(['init']);
  ...
  test('pasta não-git → null', () async {
    ...
    final plain = await Directory.systemTemp.createTemp('flightdeck_plain_');
  ...
  test('ahead/behind vs upstream local', () async {
    ...
    final bare = await Directory.systemTemp.createTemp('flightdeck_bare_');
