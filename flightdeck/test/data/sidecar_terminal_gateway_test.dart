...
void main() {
  ...
  final binary =
      [
        File('build/server-bundle/bin/flightdeck-server'),
        File('build/wave0/flightdeck-server'),
      ].firstWhere(
        (f) => f.existsSync(),
        orElse: () => File('build/wave0/flightdeck-server'),
      );

  test('terminal via sidecar: eco, ack e kill', () async {
    if (!binary.existsSync()) {
      markTestSkipped(
        'binário do flightdeck-server ausente; rode tool/build-sidecar.sh',
      );
      return;
    }
