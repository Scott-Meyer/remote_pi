...
import 'package:flightdeck/app/flightdeck/data/remote/host_shell/posix_host_shell.dart';
import 'package:flightdeck/app/flightdeck/data/remote/host_shell/windows_host_shell.dart';
...
ClientBundle _posixTestBundle(Directory root) {
  ...
  final server = File('${bin.path}/flightdeck-server')
    ..writeAsStringSync('new-server');
  File('${bin.path}/flightdeck').writeAsStringSync('new-cli');
  File('${lib.path}/libflightdeck_pty.dylib').writeAsStringSync('new-pty');
...
void main() {
  ...
  group('windowsPowerShellCommand (D1)', () {
    ...
    test('não deixa aspas nem espaço vazarem para o argv', () {
      ...
      final command = windowsPowerShellCommand(
        r'Test-Path "C:\Users\John Smith\.flightdeck"',
      );
  ...
  group('WindowsHostShell', () {
    ...
    test('killServer filtra pelo caminho, não pelo nome', () async {
      ...
      expect(script, contains(r'.flightdeck\server\bin\flightdeck-server.exe'));
    });

    test('installFromHost devolve false sem FlightDeck no host (D2)', () async {
    ...
    test('installFromHost copia LOCALMENTE, sem tráfego pelo SSH', () async {
      ...
      expect(script, contains('flightdeck-server-bundle'));
    });
  ...
  group('PosixHostShell (comportamento preservado)', () {
    ...
    test('readEndpoint é determinístico e não gasta SSH', () async {
      ...
      expect(
        ...
        '/Users/jacob/.flightdeck/flightdeck-server.sock',
      );
    ...
    test('startServer segue no nohup, com log de arranque', () async {
      ...
      expect(command, contains('libflightdeck_pty.dylib'));
    ...
    test('startServer no Linux pede a .so', () async {
      ...
      expect(exec.commands.single, contains('libflightdeck_pty.so'));
    });
    ...
    test(
      ...
      () async {
        ...
        File('${bin.path}/flightdeck').writeAsStringSync('cli');
        File('${lib.path}/libflightdeck_pty.dylib').writeAsStringSync('pty');
        final exec = _FakeExec((_) => [(0, '', '')]);
        ...
        expect(
          ...
          contains('no_nest_rename "\$HOME/.flightdeck/server.staging-'),
        );
        ...
        expect(exec.commands[swapIndex], contains('"\$HOME/.flightdeck/server"'));
        expect(
          exec.commands[swapIndex],
          isNot(contains('mv "\$HOME/.flightdeck/server.staging-')),
        );
        expect(
          exec.commands[swapIndex],
        ...
        expect(
          exec.commands.last,
          contains('rm -rf "\$HOME/.flightdeck/server.install.lock"'),
        );
        final manifestText = utf8.decode(exec.stdinPayloads[manifestIndex]!);
        expect(manifestText, contains('  bin/flightdeck-server\n'));
        expect(manifestText, contains('  bin/flightdeck\n'));
        expect(manifestText, contains('  lib/libflightdeck_pty.dylib\n'));
      },
    );
    ...
    test(
      ...
      () async {
        ...
        expect(
          File(
            '${home.path}/.flightdeck/server/bin/flightdeck-server',
          ).readAsStringSync(),
          'new-server',
        );
      },
    ...
    test('upload interrompido não promove staging parcial', () async {
      final root = Directory.systemTemp.createTempSync('flightdeck-posix-fail');
      addTearDown(() => root.deleteSync(recursive: true));
      final bin = Directory('${root.path}/bin')..createSync();
      final lib = Directory('${root.path}/lib')..createSync();
      final server = File('${bin.path}/flightdeck-server')
      ...
      File('${lib.path}/libflightdeck_pty.dylib').writeAsStringSync('pty');
      final exec = _FakeExec(
        (command) => command.contains('libflightdeck_pty.dylib')
            ? [(1, '', 'network cut')]
            : [(0, '', '')],
      );
      ...
      final cleanupIndex = exec.commands.indexWhere(
        (command) =>
            command.contains('rm -rf "\$HOME/.flightdeck/server.staging-'),
      );
    ...
    test('lock ocupado/timeout impede qualquer upload ou promoção', () async {
      final root = Directory.systemTemp.createTempSync('flightdeck-posix-locked');
      addTearDown(() => root.deleteSync(recursive: true));
      final bin = Directory('${root.path}/bin')..createSync();
      final server = File('${bin.path}/flightdeck-server')
        ..writeAsStringSync('server');
    ...
    test('falha na promoção repara backup antes de soltar o lock', () async {
      ...
      final recoveryIndex = exec.commands.lastIndexWhere(
        (command) =>
            ...
            command.contains(
              'no_nest_rename "\$HOME/.flightdeck/server.previous-',
            ) &&
            command.contains('"\$HOME/.flightdeck/server"'),
      );
      final releaseIndex = exec.commands.lastIndexWhere(
        (command) =>
            command.contains('rm -rf "\$HOME/.flightdeck/server.install.lock"'),
      );
      expect(recoveryIndex, greaterThan(promotionIndex));
      expect(releaseIndex, greaterThan(recoveryIndex));
      expect(
        exec.commands[recoveryIndex],
        isNot(contains('rm -rf "\$HOME/.flightdeck/server.previous-')),
      );
    });
    ...
    test(
      ...
      () async {
        final root = Directory.systemTemp.createTempSync(
          'flightdeck-posix-guard-fs',
        );
        addTearDown(() => root.deleteSync(recursive: true));
        final home = Directory('${root.path}/home')..createSync();
        final flightdeck = Directory('${home.path}/.flightdeck')..createSync();
        final live = Directory('${flightdeck.path}/server')..createSync();
...
if [ "$1" = "-T" ] && [ "$2" = "--" ]; then
  ...
  destination=$4
  case "$source" in
    "$HOME/.flightdeck/server.staging-"*)
      if [ "$destination" = "$HOME/.flightdeck/server" ]; then
...
''');
        ...
        expect(
          File('${stages.single.path}/bin/flightdeck-server').readAsStringSync(),
          'new-server',
        );
      },
    ...
    test(
      ...
      () async {
        final root = Directory.systemTemp.createTempSync(
          'flightdeck-posix-prior-recovery-race',
        );
        addTearDown(() => root.deleteSync(recursive: true));
        final home = Directory('${root.path}/home')..createSync();
        final flightdeck = Directory('${home.path}/.flightdeck')..createSync();
        final prior = Directory('${flightdeck.path}/server.previous-interrupted')
          ..createSync();
...
if [ "$1" = "-T" ] && [ "$2" = "--" ]; then
  ...
  destination=$4
  case "$source" in
    "$HOME/.flightdeck/server.previous-"*)
      if [ "$destination" = "$HOME/.flightdeck/server" ]; then
        mkdir -p "$destination"
...
''');
        ...
        expect(
          File('${flightdeck.path}/server/unexpected').readAsStringSync(),
          'raced',
        );
        expect(
          Directory(
            '${flightdeck.path}/server/server.previous-interrupted',
          ).existsSync(),
          isFalse,
        );
        expect(
      ...
      () async {
        ...
        Future<(int, String, String)> interruptedExec(
          ...
          if (command.contains('sha256sum -c bundle.manifest')) {
            final stage = flightdeck.listSync().whereType<Directory>().singleWhere(
              (directory) => directory.path
                  .split(Platform.pathSeparator)
                  .last
                  .startsWith('server.staging-'),
            ...
            live.renameSync('${flightdeck.path}/server.previous-$token');
            live.createSync();
