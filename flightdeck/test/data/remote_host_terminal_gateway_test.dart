...
/// - `ssh <FLIGHTDECK_TEST_SSH_HOST>` sem senha (env; ex.: `jacob@localhost`
///   com a própria chave em authorized_keys).
/// Sem essas condições o teste é pulado (o backend já tem e2e dedicado).
void main() {
  final target = Platform.environment['FLIGHTDECK_TEST_SSH_HOST'];
  final binary = File('build/wave0/flightdeck-server');

  test('terminal remoto via SSH: eco, ack, kill', () async {
    if (target == null || !binary.existsSync()) {
      markTestSkipped(
        'defina FLIGHTDECK_TEST_SSH_HOST e rode tool/build-sidecar.sh',
      );
      return;
    }
