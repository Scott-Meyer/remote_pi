...
class _FakeInstaller extends CodexHookInstallerImpl {
  ...
  @override
  Future<String?> ensureCli() async => '/fake/flightdeck';
...
void main() {
  ...
  group('trustHash', () {
    ...
    const command =
        '/private/tmp/claude-501/-Users-jacob-Projects-remote-pi-flightdeck'
  ...
  group('writeConfig', () {
    test('instala os sete eventos com trust correspondente', () async {
      const installer = _FakeInstaller('/bin/flightdeck hook');
      await installer.writeConfig(
        home: home.path,
        command: '/bin/flightdeck hook',
      );
    ...
    test('marca o comando com --harness codex', () async {
      ...
      expect(handler['command'], '/bin/flightdeck hook --harness codex');
    });
    ...
    test('re-rodar não duplica entries nem blocos de trust', () async {
      ...
      final config = readConfig();
      expect('# >>> flightdeck hooks'.allMatches(config).length, 1);
      expect('trusted_hash'.allMatches(config).length, 7);
    });
    ...
    test(
      ...
      () async {
        ...
        expect('# >>> flightdeck hooks'.allMatches(content).length, 1);
        // O que é nosso vem no fim, depois do conteúdo do usuário.
        expect(
          content.indexOf('# >>> flightdeck hooks'),
