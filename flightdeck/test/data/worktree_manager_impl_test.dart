...
void main() {
  ...
  setUp(() async {
    repo = await Directory.systemTemp.createTemp('flightdeck_wt_test_');
    await git(['init']);
  ...
  test('add → list → namespace → remove (ciclo completo)', () async {
    ...
    expect(wt.path, endsWith('/.flightdeck/worktrees/feat/sso'));
    ...
    expect(
      ...
      contains('.flightdeck/worktrees/'),
    );
    final status = await git(['status', '--porcelain']);
    expect(status.stdout.toString(), isNot(contains('.flightdeck')));
  ...
  test('list/namespace de pasta não-git devolvem vazio', () async {
    final tmp = await Directory.systemTemp.createTemp('flightdeck_nogit_');
    addTearDown(() async {
