...
Como o FlightDeck sabe que um agente numa aba está trabalhando, terminou o turno ou
...
1. No boot, `bootstrapper.dart` roda os `HookInstaller` de cada harness. Eles
   materializam a CLI interna em `~/.flightdeck/bin/flightdeck` e registram
   `<cli> hook` nos eventos de ciclo de vida da config do harness.
2. A cada evento, o harness executa `flightdeck hook` passando um JSON pelo stdin.
3. `cli/src/hook.rs` traduz o evento num status (`working` / `waiting` / `idle`)
   e manda pro app por socket Unix (TCP + token no Windows).
4. O roteamento é pela env `FLIGHTDECK_PANE_ID`, que o app injeta na PTY da aba.
   Sessão de agente aberta **fora** do FlightDeck não tem essa env, então o hook é
...
(`_flightdeck: v1`): re-rodar remove o entry antigo nosso e re-adiciona, sem nunca
...
(`flightdeck hook --harness codex`), o helper carimba o payload com `hn`, e o
...
- **Hash**: `sha256` (hex, prefixado `sha256:`) do **JSON canônico** — chaves
  ...
  Campos extras no arquivo — como o nosso marcador `_flightdeck` — são ignorados
  na desserialização e **não** afetam o hash.

Como o hash deriva da config e não do binário, o FlightDeck computa e grava o trust
...
(`# >>> flightdeck hooks` … `# <<< flightdeck hooks`) e é regerado a cada boot;
...
lib/app/flightdeck/domain/contracts/hook_installer.dart
lib/app/flightdeck/data/hooks/hook_installer_base.dart      # CLI + comando (comum)
lib/app/flightdeck/data/hooks/claude_hook_installer_impl.dart
lib/app/flightdeck/data/hooks/codex_hook_installer_impl.dart
