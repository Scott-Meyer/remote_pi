# `.flightdeck/tasks.json` — Task Run

O **Task Run** do flightdeck roda os comandos de build/dev do seu projeto
...
1. **Detecção automática** — ao abrir um projeto, o flightdeck lê os manifestos
   (`package.json scripts`, `pubspec.yaml`) e já mostra tasks **sem config**.
2. **Manual** — este arquivo, `.flightdeck/tasks.json`, na pasta que você abre como
...
Na **raiz do workspace** que você abre no flightdeck. A descoberta é literal (sem
subir na árvore), então:

- Projeto de pacote único → abra a pasta do pacote; `.flightdeck/tasks.json` ali.
- **Monorepo** → abra a raiz; um único `.flightdeck/tasks.json` na raiz dirige os
...
> O flightdeck gera o `id` da task automaticamente (`json:<label>`); ele sobrescreve
...
flightdeck reimplementa via observação de arquivos. Fica **sempre ligado** enquanto
...
```jsonc
{ "$schema": "../flightdeck/docs/tasks.schema.json", "tasks": [ ... ] }
```

(O flightdeck ignora `$schema` ao executar — é só pro editor.)
