...
O FlightDeck já sabe quando um agente está trabalhando, terminou o turno ou precisa
de ação do usuário. O mecanismo (ver `project_flightdeck_claude_turn_status`):

1. `ClaudeHookInstallerImpl` materializa a CLI interna em `~/.flightdeck/bin/` e faz
   append idempotente de um entry marcado (`_flightdeck: v1`) em cada evento de
...
3. `cli/src/hook.rs` traduz o evento num status (`working` / `waiting` / `idle`)
   ...
   `FLIGHTDECK_PANE_ID` que o app injeta na PTY da aba.
4. Sessão de agente fora do FlightDeck não tem essas envs → o hook é no-op. Gate
...
- Campos extras no handler (nosso marcador `_flightdeck`) são ignorados pelo
...
2. E2E manual, com build `.debug` (bundle id isolado, memória
   `project_flightdeck_debug_flavor`): abrir aba, rodar `codex`, mandar um prompt
...
3. Conferir que uma sessão `codex` **fora** do FlightDeck não emite nada (gate por
   env).
...
1. Seção no `CLAUDE.md` do flightdeck (ou em `docs/`) descrevendo que o status de
...
- [ ] Wave 3: sessão de Codex fora do FlightDeck não emite status **(a testar)**
