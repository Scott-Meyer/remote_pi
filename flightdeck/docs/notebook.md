...
cada um com um frontmatter YAML raso. O FlightDeck mostra a pasta como **item
único** na árvore (logo do FlightDeck, sem expandir) e abre como uma tab de notas.
Fora do app é uma pasta comum: git, Obsidian e o agente no terminal veem os
`.md` direto. Plano de referência: `plan/62-flightdeck-gallery.md`, passo 4.
...
```sh
flightdeck note add <dir.notebook> --title <t> [--tag <a>]... [--body <texto> | --body -]
flightdeck note list <dir.notebook> [--json]
```
...
(ver `flightdeck_cli_handler.dart`).
