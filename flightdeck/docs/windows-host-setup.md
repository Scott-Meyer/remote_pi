...
Como deixar uma máquina Windows pronta para ser **host** do FlightDeck (plano 61).
Do outro lado, o cliente pode ser macOS, Linux, Windows ou iPad.

Pré-requisito de produto: **o FlightDeck desktop precisa estar instalado no host**.
O servidor remoto é instalado copiando o `flightdeck-server-bundle` que o app já
deixa na máquina (decisão D2) — nenhum binário viaja pelo SSH. Sem o FlightDeck
instalado lá, o cliente falha com "Windows mas não tem o FlightDeck instalado".
...
> Não é preciso trocar o `DefaultShell` para PowerShell. O FlightDeck invoca
...
`BatchMode=yes` é o mesmo modo que o FlightDeck usa: ele proíbe qualquer pergunta
...
| Conecta, mas o FlightDeck diz que não achou o FlightDeck no host | O app não está instalado no host (ver pré-requisito no topo) |
...
- **O servidor escuta em loopback, nunca na rede.** No Windows o `dart:io` não
  tem socket UNIX, então o `flightdeck-server` escuta numa porta TCP de loopback
  efêmera e anuncia porta + token num arquivo de rendezvous
  (`~/.flightdeck/flightdeck-server.sock`, que ali é um JSON, não um socket). O acesso
...
- **O token existe porque loopback não protege.** Uma porta de loopback aceita
  ...
  protegido pelas permissões do `~/.flightdeck`. O token viaja no handshake e o
