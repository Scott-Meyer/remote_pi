# Protocolo `pi --mode rpc` — schema usado pelo FlightDeck

Documento do **Passo 0 (spike)** do [plano 37](../../plan/37-desktop-flightdeck.md).
...
> O FlightDeck spawna **pi puro** (decisão B do plano 37): `pi --mode rpc
...
## Comandos que o FlightDeck envia (stdin)

No MVP o FlightDeck só precisa de **um** comando. Os demais existem no protocolo e
...
## Eventos que o FlightDeck recebe (stdout)
...
6. **App macOS não herda o PATH do shell.** O binário `pi` é resolvido por
   ...
   `--dart-define=FLIGHTDECK_PI_PATH=…`. Ver `lib/config/env.dart`.
