# Formato de tema do FlightDeck (`flightdeck-theme-1`)
...
```json
{
  "$schema": "https://raw.githubusercontent.com/jacobaraujo7/remote_pi/main/flightdeck/docs/theme.schema.json",
  "id": "acme.aurora",
  "name": "Aurora",
  "author": "Acme",
  "version": "1.0.0",
...
|---|---|---|
| `id` | sim | Identidade estável, namespaced (`publisher.nome`). É o que fica salvo nas preferências — renomear o `name` não perde a escolha do usuário. Não pode colidir com um built-in (`flightdeck`, `flightdeck.2`, `violet`, `violet.2`, `midnight`, `rose`, `sun`, `flexoki`, `pantera`). |
| `name` | sim | O que aparece no seletor. |
| `author`, `version` | não | Metadados. |
| `extends` | não | Id de um tema built-in do qual herdar. Hoje existem `flightdeck`, `flightdeck.2`, `violet`, `violet.2`, `midnight`, `rose`, `sun`, `flexoki` e `pantera`. Ausente = herda de `flightdeck`. |
