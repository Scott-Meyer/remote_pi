# Protocolo FlightDeck Remote (v1, rascunho da Wave 0)

Protocolo cliente ↔ `flightdeck-server` do plano 58. Nesta wave cobre o
**handshake** e o **domínio Terminais**; arquivos, git e databases entram nas
waves seguintes com o mesmo envelope.

As mensagens são classes Dart em `packages/flightdeck_protocol` — a mesma classe
...
`FLIGHTDECK_STATUS_SOCK` que o cliente injetou, porque aquele socket é a via da
CLI interna (`flightdeck send`, `list-tabs`, `db`) além do status de turno. Para
...
outro transporte — o hook lê `FLIGHTDECK_STATUS_SOCK` antes de porta+token, e uma
...
`~/.flightdeck/db-secrets.json`, chaveado por `(raiz do workspace, nome da
...
digitada no host não valia para cliente nenhum — o `flightdeck-server` é headless
