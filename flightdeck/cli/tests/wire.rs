...
/// Sobe um listener num caminho temporário, roda `flightdeck <args>` apontado pra
...
fn run_against_fake_app(args: &[&str], response: &str) -> (Value, String, String, i32) {
    let dir = unique_dir("flightdeck-wire");
    ...
    let out = Command::new(env!("CARGO_BIN_EXE_flightdeck"))
        .args(args)
        .env("FLIGHTDECK_STATUS_SOCK", &sock_path)
        .env("FLIGHTDECK_TAB_ID", "t7")
        .env_remove("FLIGHTDECK_STATUS_PORT")
        .env_remove("FLIGHTDECK_STATUS_TOKEN")
        .output()
        .unwrap();
...
fn send_manda_texto_em_base64_com_type_cmd() {
    ...
    assert_eq!(
        req["tabId"], "t7",
        "cai no FLIGHTDECK_TAB_ID quando não passam --tab-id"
    );
    // "olá mundo" em base64
...
fn erro_do_app_vira_exit_1_no_stderr() {
    ...
    assert_eq!(stdout, "");
    assert_eq!(stderr, "flightdeck: boom\n");
    assert_eq!(code, 1);
}
...
fn close_tab_sem_alvo_nao_manda_target() {
    // Sem posicional o app cai na própria tab ($FLIGHTDECK_TAB_ID) — mandar
...
fn hook_reporta_status_pelo_mesmo_socket() {
    ...
    let dir = unique_dir("flightdeck-hook");
    std::fs::create_dir_all(&dir).unwrap();
    let sock_path = dir.join("status.sock");
    let listener = UnixListener::bind(&sock_path).unwrap();
    let server = std::thread::spawn(move || {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_flightdeck"))
        .arg("hook")
        .env("FLIGHTDECK_STATUS_SOCK", &sock_path)
        .env("FLIGHTDECK_PANE_ID", "t3")
...
#[test]
fn hook_fora_do_flightdeck_e_noop_silencioso() {
    let out = Command::new(env!("CARGO_BIN_EXE_flightdeck"))
        .arg("hook")
        .env_remove("FLIGHTDECK_PANE_ID")
        .env_remove("FLIGHTDECK_STATUS_SOCK")
        .env_remove("FLIGHTDECK_STATUS_PORT")
...
fn send_com_enter_manda_o_retorno_numa_segunda_escrita() {
    ...
    let dir = unique_dir("flightdeck-enter");
    std::fs::create_dir_all(&dir).unwrap();
    let sock_path = dir.join("status.sock");
    let listener = UnixListener::bind(&sock_path).unwrap();
...
fn focused_ignora_o_tab_id_do_ambiente() {
    // FLIGHTDECK_TAB_ID está setado no helper; --focused tem que vencer, senão o
...
fn acha_o_socket_bem_conhecido_sem_env_nenhuma() {
    ...
    // ~/.flightdeck/status.sock. Aqui apontamos HOME pro temp pra não depender do
    // app real estar rodando.
    let dir = unique_dir("flightdeck-discovery");
    std::fs::create_dir_all(dir.join(".flightdeck")).unwrap();
    let sock_path = dir.join(".flightdeck/status.sock");
    let listener = UnixListener::bind(&sock_path).unwrap();
    let server = std::thread::spawn(move || {
        let (stream, _) = listener.accept().unwrap();
        let mut reader = BufReader::new(stream);
...
fn sem_app_nenhum_falha_com_exit_3() {
    let dir = unique_dir("flightdeck-noapp");
    std::fs::create_dir_all(&dir).unwrap();
    let out = Command::new(env!("CARGO_BIN_EXE_flightdeck"))
        .args(["list-tabs"])
        .env("HOME", &dir)
