...
//!
//! Porte fiel dos helpers do `tool/flightdeck_cli.dart` — o formato de saída é
//! consumido por humano E por agente, então largura de coluna e espaçamento
//! importam.

use std::path::Path;

/// Id da própria tab: `FLIGHTDECK_TAB_ID` (novo) com fallback pro legado
/// `FLIGHTDECK_PANE_ID`. O app injeta os dois; o fallback cobre binário novo com
/// app antigo (só PANE_ID) e vice-versa.
pub fn self_tab_id() -> Option<String> {
    env_non_empty("FLIGHTDECK_TAB_ID").or_else(|| env_non_empty("FLIGHTDECK_PANE_ID"))
}
