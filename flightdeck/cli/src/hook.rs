//! `flightdeck hook` — helper que os agentes invocam nos hooks de ciclo de vida.
...
//! (working / waiting / idle) e mandamos pro FlightDeck pelo mesmo socket da CLI,
...
//! PTY o `FLIGHTDECK_PANE_ID` (roteamento) e o `FLIGHTDECK_STATUS_SOCK` (caminho do
//! socket); o hook herda os dois. Sessões de agente fora do FlightDeck não têm
...
fn try_run(harness: String) -> Option<()> {
    let pane_id = env_non_empty("FLIGHTDECK_PANE_ID")?; // não é sessão do FlightDeck
    let sock = env_non_empty("FLIGHTDECK_STATUS_SOCK");
...
/// interactive session that owns the FlightDeck tab.
///
