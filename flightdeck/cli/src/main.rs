//! `flightdeck` — CLI **interna** do FlightDeck, e o helper de hook do Claude Code.
//!
//! Fica visível apenas dentro dos terminais que o app spawna (o app prependa
//! `~/.flightdeck/bin[-debug]` no PATH só dessas abas) e fala com o app por um
//! socket (`FLIGHTDECK_STATUS_SOCK` no POSIX; `FLIGHTDECK_STATUS_PORT` +
//! `FLIGHTDECK_STATUS_TOKEN` no Windows), discriminando `type:"cmd"` no wire.
//!
...
//! e `FLIGHTDECK_PANE_ID` ficam como **aliases legados**.
//!
//! O subcomando `hook` absorve o antigo binário `flightdeck-hook`: um binário só,
...
/// olhando `flightdeck --version`, se o binário instalado ainda é o Dart antigo
/// (sem sufixo) ou este.
...
fn main() {
    ...
    if first == "--version" || first == "-v" {
        println!("flightdeck {VERSION}");
        std::process::exit(0);
    }

    let args = &argv[1..];
    match first {
        // Hook de ciclo de vida dos harnesses (antigo binário `flightdeck-hook`).
        ...
        "rename-tab" => commands::rename_tab(args),
