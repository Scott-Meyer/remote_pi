...
//! status no hook). POSIX usa socket Unix (`FLIGHTDECK_STATUS_SOCK`); Windows usa
//! TCP no loopback (`FLIGHTDECK_STATUS_PORT` + `FLIGHTDECK_STATUS_TOKEN`), porque lá
...
/// não estão setadas (não estamos dentro de um terminal do FlightDeck).
/// Flavor assado no binário pelo build (ver `build.rs`). `Some("debug")` no
/// build de desenvolvimento.
const FLAVOR: Option<&str> = option_env!("FLIGHTDECK_FLAVOR");

/// Sockets a tentar, em ordem: o do ambiente (injetado pelo app na PTY da aba)
/// e, como fallback, os caminhos bem conhecidos.
///
/// O fallback é o que permite usar a CLI **de fora** de um terminal do FlightDeck
/// (ex.: uma ferramenta de ditado chamando `flightdeck send --focused`), onde não
/// existe env nenhuma herdada.
///
/// Release e debug têm sockets separados de propósito (um FlightDeck de dev e um
...
fn candidate_sockets() -> Vec<String> {
    let mut out = Vec::new();
    if let Some(path) = env_non_empty("FLIGHTDECK_STATUS_SOCK") {
        out.push(path);
    }
    if let Some(home) = crate::util::home_dir() {
        let release = format!("{home}/.flightdeck/status.sock");
        let debug = format!("{home}/.flightdeck/status-debug.sock");
...
fn connect() -> Result<Option<Conn>, String> {
    let port = env_non_empty("FLIGHTDECK_STATUS_PORT").and_then(|p| p.parse::<u16>().ok());
...
pub fn transport_configured() -> bool {
    if env_non_empty("FLIGHTDECK_STATUS_SOCK").is_some()
        || env_non_empty("FLIGHTDECK_STATUS_PORT")
...
///
/// Fora de um terminal do FlightDeck, encerra com exit 3 (mesma mensagem da CLI
...
pub fn request(mut req: Value, timeout: Duration) -> Value {
    if !transport_configured() {
        die(
            "flightdeck: no FlightDeck app to talk to (FLIGHTDECK_STATUS_SOCK is unset and \
no socket found in ~/.flightdeck). Is the app running?",
            3,
        );
    }
    req["type"] = json!("cmd");
    if let Ok(tok) = std::env::var("FLIGHTDECK_STATUS_TOKEN") {
        req["tok"] = json!(tok);
    }
    ...
    let mut conn = match connect() {
        Ok(Some(c)) => c,
        Ok(None) => die(
            "flightdeck: not inside a FlightDeck terminal (FLIGHTDECK_STATUS_SOCK is unset)",
            3,
        ),
        Err(e) => die(&format!("flightdeck: could not connect to app: {e}"), 3),
    };
    ...
    {
        die(&format!("flightdeck: could not connect to app: {e}"), 3);
    }
...
pub fn fail_with(resp: &Value) -> ! {
    die(&format!("flightdeck: {}", error_text(resp)), 1)
}
