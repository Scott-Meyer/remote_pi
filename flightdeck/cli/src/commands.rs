//! Os verbos da CLI. Porte fiel do `tool/flightdeck_cli.dart`: mesmos comandos de
...
pub fn send(args: &[String]) -> ! {
    ...
    if text.is_empty() {
        die("flightdeck send: missing text to send", 2);
    }
...
pub fn send_key(args: &[String]) -> ! {
    ...
    if parsed.positionals.is_empty() {
        die("flightdeck send-key: missing key (e.g. Enter, C-c, Escape)", 2);
    }
    let mut buf = String::new();
    for name in &parsed.positionals {
        match keys::resolve(name) {
            Some(seq) => buf.push_str(&seq),
            None => die(&format!("flightdeck send-key: unknown key \"{name}\""), 2),
        }
    }
...
/// fora de um terminal do FlightDeck (onde o app injeta `FLIGHTDECK_TAB_ID`), digitar
/// num pane arbitrário seria pior que falhar.
fn resolve_target(tab_id: Option<String>) -> String {
    match tab_id {
        Some(id) if !id.is_empty() => id,
        _ => die(
            "flightdeck: no target — pass --tab-id <id> or run inside a FlightDeck \
terminal (FLIGHTDECK_TAB_ID is unset). Use `flightdeck list-tabs`.",
            2,
        ),
    }
}
...
pub fn open(args: &[String]) -> ! {
    let parsed = Flags::parse(args);
    if parsed.positionals.is_empty() {
        die("flightdeck open: missing file path", 2);
    }
...
const NEW_TAB_HELP: &str = "flightdeck new-tab [--cwd <dir>] [--title <name>] [--split h|v]
...
pub fn new_tab(args: &[String]) -> ! {
    ...
    let wire_split = match split.as_deref().map(|s| s.to_lowercase()) {
        ...
        Some(_) => die(
            &format!(
                "flightdeck new-tab: invalid --split \"{}\" (use h|right or v|down)",
...
const CLOSE_TAB_HELP: &str = "flightdeck close-tab [<label|tab-id>] [--json]
  Closes a tab (the counterpart of `new-tab`). Without a target, closes the
  CURRENT tab — which ends the shell you are typing in.
  A target may be a stable tab label or a tab-id (see `flightdeck list-tabs`).
...
pub fn close_tab(args: &[String]) -> ! {
    ...
    // Sem alvo posicional, o server cai na própria tab ($FLIGHTDECK_TAB_ID).
    with_tab_id(&mut req, parsed.effective_tab_id());
...
const RENAME_TAB_HELP: &str = "flightdeck rename-tab [<label|tab-id>] <new-name> [--tab-id <id>] [--json]
  Renames a tab in FlightDeck and locks its manual label.
  Without target, renames the current tab ($FLIGHTDECK_TAB_ID).
  Prints the tab id.";
...
pub fn rename_tab(args: &[String]) -> ! {
    ...
    while i < args.len() {
        ...
        if a == "--tab-id" || a == "-t" {
            if i + 1 >= args.len() {
                die("flightdeck: --tab-id requires a value", 2);
            }
            i += 1;
        ...
        if a.starts_with('-') {
            die(&format!("flightdeck: unknown flag \"{a}\""), 2);
        }
        if target.is_none() {
    ...
    let (target_val, raw_name) = match (target, new_name) {
        ...
        (Some(n), None) => (None, n),
        _ => die("flightdeck rename-tab: missing new name", 2),
    };
...
const NEW_WORKSPACE_HELP: &str = "flightdeck new-workspace <path> [--host <ssh-target>] [--name <title>] [--json]
  Adds a top-level workspace in FlightDeck's sidebar (local or remote), selects it,
  and ensures an initial terminal tab is opened.
  Aliases: `flightdeck open-workspace`, `flightdeck new-remote-workspace`
  ...
  --tab-id <id>   caller tab (default: this tab / $FLIGHTDECK_TAB_ID)
...
pub fn new_workspace(args: &[String]) -> ! {
    ...
    while i < args.len() {
        ...
        if a == "--host" || a == "--remote" {
            if i + 1 >= args.len() {
                die("flightdeck new-workspace: --host requires a value", 2);
            }
            i += 1;
        ...
        if a == "--path" {
            if i + 1 >= args.len() {
                die("flightdeck new-workspace: --path requires a value", 2);
            }
            i += 1;
        ...
        if a == "--name" {
            if i + 1 >= args.len() {
                die("flightdeck new-workspace: --name requires a value", 2);
            }
            i += 1;
    ...
    let raw_path = match path {
        ...
        _ => die("flightdeck new-workspace: missing <path> (or --path <path>)", 2),
    };
...
const CLOSE_WORKSPACE_HELP: &str = "flightdeck close-workspace [<id|path>] [--tab-id <id>] [--json]
  Closes a top-level project from FlightDeck (ends its agents/tabs; folder on
  disk is kept).
...
const RENAME_WORKSPACE_HELP: &str = "flightdeck rename-workspace [<id|path>] <new-name> [--tab-id <id>] [--json]
  Renames the display title of a workspace in FlightDeck's rail.
  Target may be a workspace UUID, directory path, or unique name.
...
pub fn rename_workspace(args: &[String]) -> ! {
    ...
    let (target_val, raw_name) = match (target, new_name) {
        (Some(t), Some(n)) => (Some(t), n),
        (Some(n), None) => (None, n),
        _ => die("flightdeck rename-workspace: missing workspace name", 2),
    };
...
pub fn browse_url(args: &[String]) -> ! {
    ...
    let Some(url) = url.filter(|u| !u.is_empty()) else {
        die(
            &format!("flightdeck browse: missing <url>\n\n{BROWSE_HELP}"),
            2,
        )
    };
...
const ORCHESTRATE_HELP: &str = "flightdeck orchestrate <file.ckp> [--json]
...
pub fn orchestrate(args: &[String]) -> ! {
    ...
    let file = match file {
        ...
        _ => die("flightdeck orchestrate: missing <file.ckp>", 2),
    };
...
pub fn read(cmd: &str, args: &[String]) -> ! {
    ...
    if cmd == "read-task" && target.is_empty() {
        die("flightdeck read-task: missing task id", 2);
    }
    let mut cmd_args = Map::new();
    if !target.is_empty() {
        cmd_args.insert("target".into(), json!(target));
    ...
    let text = match b64().decode(encoded.as_bytes()) {
        Ok(bytes) => match String::from_utf8(bytes) {
            Ok(s) => s,
            Err(_) => die("flightdeck: malformed payload", 1),
        },
        Err(_) => die("flightdeck: malformed payload", 1),
    };
    if !text.is_empty() {
    ...
    if data.get("truncated") == Some(&Value::Bool(true)) {
        eprintln!("flightdeck: output truncated (server cap 2000 lines/read — page with --offset)");
    }
    std::process::exit(0)
}
...
pub fn db(args: &[String]) -> ! {
        ...
        if let Some(eq) = a.find('=') {
            if a.starts_with("--") && eq > 0 {
                ...
                match key {
                    ...
                    _ => db_fail(
                        "error",
                        &format!("unknown flag \"{key}\" (see `flightdeck db --help`)"),
                    ),
                }
                continue;
            }
    ...
    let wire = match sub.as_str() {
        ...
        other => db_fail(
            "error",
            &format!("unknown subcommand \"{other}\" (see `flightdeck db --help`)"),
        ),
    };
...
pub fn http(args: &[String]) -> ! {
        ...
        if let Some(eq) = a.find('=') {
            if a.starts_with("--") && eq > 0 {
                ...
                match key {
                    ...
                    _ => db_fail(
                        "error",
                        &format!("unknown flag \"{key}\" (see `flightdeck http --help`)"),
                    ),
                }
                continue;
            }
    ...
    let wire = match sub.as_str() {
        ...
        other => db_fail(
            "error",
            &format!("unknown subcommand \"{other}\" (see `flightdeck http --help`)"),
        ),
    };
...
const REDIS_HELP: &str = "flightdeck redis --db <conn> <COMMAND> [args...]
  e.g. flightdeck redis --db cache GET session:42
  Output: one JSON line. Connection registered in the Database panel.
flightdeck redis browse --db <conn> [--pattern 'user:*']
...
const MONGO_HELP: &str = concat!(
    "flightdeck mongo --db <conn> [--database <name>] --command '<json>'\n",
    "  e.g. flightdeck mongo --db app --command '{\"find\":\"users\",\"filter\":{}}'\n",
    ...
    "flightdeck mongo browse --db <conn> [--database <name>] <collection> [--filter '<json>']\n",
...
pub fn install_skill(args: &[String]) -> ! {
    ...
    let home = match crate::util::home_dir() {
        Some(h) => h,
        None => die("flightdeck: HOME not resolved", 1),
    };
    let dir = format!("{home}/.claude/skills/flightdeck-cli");
    let path = format!("{dir}/SKILL.md");
    if !parsed.force {
        if let Ok(current) = std::fs::read_to_string(&path) {
            if current == SKILL_MARKDOWN {
                println!("flightdeck: skill already installed ({path})");
                std::process::exit(0);
            }
        }
    }
    if let Err(e) = std::fs::create_dir_all(&dir) {
        die(&format!("flightdeck: {e}"), 1);
    }
    if let Err(e) = std::fs::write(&path, SKILL_MARKDOWN) {
        die(&format!("flightdeck: {e}"), 1);
    }
    println!("flightdeck: skill installed at {path}");
    std::process::exit(0)
}
...
mod tests {
    ...
    fn label_manual_vence_titulo() {
        let data = vec![json!({"id": "t2", "label": "FlightDeck", "title": "zsh"})];
        let lines = format_list("list-panes", &data);
        assert!(lines[0].ends_with("⚲ FlightDeck"), "{}", lines[0]);
    }
...
const NOTE_HELP: &str = "flightdeck note <add|list> <folder.notebook> [flags]
...
pub fn note(args: &[String]) -> ! {
    ...
    while i < rest.len() {
        ...
        match a {
            ...
            "--title" | "--tag" | "--body" | "--tab-id" => {
                if i + 1 >= rest.len() {
                    die(&format!("flightdeck note: {a} requires a value"), 2);
                }
                i += 1;
                let v = rest[i].clone();
                match a {
            ...
            _ => {
                if let Some(v) = a.strip_prefix("--title=") {
                    ...
                    die(&format!("flightdeck note: unknown flag {a}"), 2);
                } else {
    ...
    if positionals.is_empty() {
        die("flightdeck note: missing <folder.notebook>", 2);
    }
    let notebook = resolve_path(&positionals[0]);
    if !notebook.to_lowercase().ends_with(".notebook") {
        die("flightdeck note: the folder name must end in .notebook", 2);
    }
    let tab_id = tab_id.or_else(|| std::env::var("FLIGHTDECK_TAB_ID").ok());

    match sub.as_str() {
        "add" => {
            let title = match title {
                Some(t) if !t.trim().is_empty() => t,
                _ => die("flightdeck note add: --title is required", 2),
            };
            let body = match body {
                Some(b) if b == "-" => {
                    let mut buf = String::new();
                    if std::io::Read::read_to_string(&mut std::io::stdin(), &mut buf).is_err() {
                        die("flightdeck note add: could not read stdin", 2);
                    }
                    buf
                }
                Some(b) => b,
        ...
        _ => die(&format!("flightdeck note: unknown subcommand \"{sub}\""), 2),
    }
}
