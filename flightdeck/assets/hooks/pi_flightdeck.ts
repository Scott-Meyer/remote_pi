// Managed by FlightDeck. Reports turn status and synchronizes reactive session naming.
import type { ExtensionAPI } from "@earendil-works/pi-coding-agent";
import * as net from "net";
import * as path from "path";

export default function flightdeckTurnStatus(pi: ExtensionAPI) {
  // pi-subagents children inherit the tab's FLIGHTDECK_* variables. Without this
  ...
  const paneId = process.env.FLIGHTDECK_PANE_ID;
  const socketPath = process.env.FLIGHTDECK_STATUS_SOCK;
  const port = Number.parseInt(process.env.FLIGHTDECK_STATUS_PORT ?? "", 10);
  const hasPort = Number.isInteger(port) && port > 0 && port <= 65_535;
  if (!paneId || (!socketPath && !hasPort)) return;

  let lastSentToFlightDeck: string | null = null;
  ...
  function getWorkspaceName(): string {
    return (
      process.env.FLIGHTDECK_WORKSPACE_NAME ||
  ...
  function sendOneShot(payloadObj: Record<string, unknown>) {
    try {
      if (process.env.FLIGHTDECK_STATUS_TOKEN && !payloadObj.tok) {
        payloadObj.tok = process.env.FLIGHTDECK_STATUS_TOKEN;
      }
  ...
  function syncName(rawName: string | undefined | null, forceSend = false) {
    ...
    // 2. If not yet sent to FlightDeck, or if forced (FlightDeck sent non-canonical name), notify FlightDeck over IPC
    if (forceSend || canonical !== lastSentToFlightDeck) {
      lastSentToFlightDeck = canonical;
      sendOneShot({
        type: "cmd",
        cmd: "rename-tab",
        tabId: paneId,
    ...
    try {
      ...
      client.once("connect", () => {
        const req = {
          ...
          cmd: "subscribe",
          tabId: paneId,
          ...(process.env.FLIGHTDECK_STATUS_TOKEN
            ? { tok: process.env.FLIGHTDECK_STATUS_TOKEN }
            : {}),
        };
      ...
      client.on("data", (chunk) => {
        ...
        while ((newlineIdx = buffer.indexOf("\n")) >= 0) {
          ...
          try {
            ...
            // FlightDeck tab renamed -> route through syncName to ensure canonical convergence.
            // If FlightDeck passed an un-prefixed name, force-send the canonical correction back!
            if (
              parsed &&
