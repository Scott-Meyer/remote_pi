...
///    (FlightDeck) morreu e as PIDs não são mais filhos dele.
///
/// 2. **PPID scan** — antes de qualquer spawn, busca todos os processos `pi`
///    cujo PPID == PID do FlightDeck. Esses são exatamente os órfãos do hot
///    restart: o processo macOS do FlightDeck continua vivo mas a isolate anterior
...
class PiProcessRegistry {
  ...
  static String get _legacyPath {
    final home = userHome() ?? '';
    return '$home/.pi/flightdeck/agent-pids';
  }
  ...
  /// processo FlightDeck. Esses são os órfãos deixados pelo hot restart: a isolate
  ...
  static Future<List<int>> _orphanedPiChildren() async {
    ...
    try {
      ...
      final myFlightDeckPid = pid; // dart:io — PID do processo macOS deste app
      final piPids = stdout
          .split('\n')
      ...
      for (final piPid in piPids) {
        ...
        if (ppid == myFlightDeckPid) orphans.add(piPid);
      }
      return orphans;
    } catch (_) {
      return const <int>[];
