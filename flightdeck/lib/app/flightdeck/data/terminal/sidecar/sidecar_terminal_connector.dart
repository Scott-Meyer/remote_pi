...
import 'package:flightdeck/app/core/utils/child_process_lifetime.dart';
import 'package:flightdeck/app/core/utils/user_home.dart';
import 'package:flightdeck_remote/flightdeck_remote.dart';
import 'package:flutter/foundation.dart';

/// Garante um `flightdeck-server` sidecar de pé e devolve o serviço de terminais
...
class SidecarTerminalConnector implements TurnStatusSource {
  ...
  Future<RemoteTerminalService?> _connect() async {
    try {
      ...
      // 2. Spawn do sidecar.
      if (binary == null) {
        return _giveUp('flightdeck-server binary not found');
      }
      ...
      _child = await Process.start(
        ...
        environment: {
          ...
          if (ptyDylib.existsSync()) 'FLIGHTDECK_PTY_DYLIB': ptyDylib.path,
        },
      );
  ...
  /// `flightdeck_server`, que raramente muda entre releases do app. O caminho do
  ...
  Future<RemoteConnection?> _tryConnect(String socketPath) async {
    ...
    try {
      return await RemoteConnection.connect(
        socketPath,
        clientName: 'flightdeck-gui',
        // Mesma máquina: as PTYs mantêm o FLIGHTDECK_STATUS_SOCK do app, que é
        // por onde a CLI interna (`flightdeck send`, `list-tabs`, `db`) fala. Um
  ...
  String? _socketPath() {
    ...
    final dir = Directory('$home/.flightdeck');
    if (!dir.existsSync()) dir.createSync(recursive: true);
    // Namespaceado por debug/release, pela mesma razão do `status.sock` e do
    // `bin/` da CLI: `~/.flightdeck` é o HOME real (não é isolado por bundle id),
    ...
    return '${dir.path}/flightdeck-server$suffix.sock';
  }
  ...
  static String serverExeNameFor(String os) =>
      os == 'windows' ? 'flightdeck-server.exe' : 'flightdeck-server';

  /// Mesmo executável, com sufixo de arquitetura (`flightdeck-server-arm64`).
  ///
  ...
  static String serverExeFor(String arch, {String? os}) =>
      (os ?? hostOs) == 'windows'
      ? 'flightdeck-server-$arch.exe'
      : 'flightdeck-server-$arch';
  ...
  /// app-managed (`~/.flightdeck/server`) → build de dev.
  static String? resolveServerBundleBinary({String? os, String? arch}) =>
      resolveServerBundleBinaryFrom(
        ...
        environmentBinary: Platform.environment['FLIGHTDECK_SERVER_BIN'],
      );
  ...
  static List<String> serverBundleBinDirs() {
    ...
    return <String>[
      if (Platform.isMacOS)
        // .app/Contents/MacOS/<exe> → ../Resources/flightdeck-server-bundle/bin
        '${File(Platform.resolvedExecutable).parent.parent.path}'
            '/Resources/flightdeck-server-bundle/bin'
      else
        // Windows/Linux: o bundle é instalado ao lado do executável, mesmo
        // lugar do flightdeck-hook e da flightdeck-cli (ver os CMakeLists).
        '${File(Platform.resolvedExecutable).parent.path}'
            '/flightdeck-server-bundle/bin',
      if (home != null) '$home/.flightdeck/server/bin',
