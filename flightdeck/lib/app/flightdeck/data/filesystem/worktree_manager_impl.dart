...
class WorktreeManagerImpl implements WorktreeManager {
  ...
  /// Onde as worktrees criadas pelo FlightDeck moram, relativo à raiz do repo
  /// (decisão 2). Migrado de `.pi/remote/worktrees` → `.flightdeck/worktrees`;
  /// worktrees antigas seguem funcionando (a descoberta é via
  /// `git worktree list`, não por caminho).
  static const String worktreesSubdir = '.flightdeck/worktrees';
  ...
  WorktreeAddRun<Worktree> add(
    ...
    () async {
      try {
        ...
        // Guard rail: garante `.flightdeck/worktrees/` no `.gitignore` do repo ANTES
        // de materializar a worktree. Sob `.pi/` a pasta era ignorada de graça
        // (repos pi já ignoram `.pi`); sob `.flightdeck/` isso não vale, e sem o
  ...
  /// Guard rail: garante que `.flightdeck/worktrees/` está no `.gitignore` da
  ...
  /// ignorado (não `.flightdeck/` inteiro: o `tasks.json` de lá é comitável).
