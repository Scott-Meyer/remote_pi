...
/// Installs FlightDeck's Pi turn-status bridge as a global Pi extension.
///
...
/// agents inherit the tab's `FLIGHTDECK_*` variables, but must not report status
/// for the parent tab.
class PiHookInstallerImpl implements HookInstaller {
  const PiHookInstallerImpl();

  static const assetPath = 'assets/hooks/pi_flightdeck.ts';
  ...
  Future<bool> writeExtension({
    ...
    final target = File('$home/.pi/agent/extensions/flightdeck.ts');
