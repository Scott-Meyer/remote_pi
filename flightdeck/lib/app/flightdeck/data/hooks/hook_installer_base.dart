...
/// Parte comum dos instaladores de hook: materializar a CLI `flightdeck` num
...
abstract class HookInstallerBase implements HookInstaller {
  ...
  /// mesmo** `~/.flightdeck/bin-<flavor>/flightdeck.exe`. Sem isto, ambos veem o
  ...
  /// binário `flightdeck-hook` legado, senão o status de turno morreria em silêncio
  ...
  Future<String?> resolveHookCommand(String? cliPath) async {
    ...
    final name = Platform.isWindows ? 'flightdeck-hook.exe' : 'flightdeck-hook';
    final dir = flightdeckHookDir();
  ...
  ///
  /// O sinal é o sufixo `r` da versão (`flightdeck 0.6.0r`), que só a CLI em Rust
  ...
  /// Materializa a CLI interna `flightdeck` em `~/.flightdeck/bin[-debug]/flightdeck[.exe]`
  /// (o fonte é empacotado como `flightdeck-cli` pra não colidir com `flightdeck.app`) e,
  /// se materializou, roda `flightdeck install-skill` (idempotente) pra a skill
  ...
  Future<String?> ensureCli() async {
    final bundledName = Platform.isWindows ? 'flightdeck-cli.exe' : 'flightdeck-cli';
    final destName = Platform.isWindows ? 'flightdeck.exe' : 'flightdeck';
    final dir = flightdeckCliDir();
    if (dir == null) return null;
  ...
  /// Nome curto da CLI — `deck` digita melhor que `flightdeck` num terminal onde o
  /// agente (ou o humano) chama o comando o tempo todo.
  ///
  /// É o **mesmo binário**, não um segundo programa: nada muda de acordo com o
  /// nome pelo qual foi invocado, e a ajuda segue dizendo `flightdeck`. Se o
  /// usuário já tem um `deck` próprio, o alias de shell dele vence o PATH e nada
  /// se quebra.
  static const String shortAliasName = 'deck';
  ...
  /// `C:\Users\x\.flightdeck\bin\flightdeck-hook.exe`) vira `C:Usersx.flightdeck...` e dá
  /// `command not found`. Como `$home` (`USERPROFILE`) vem com `\` no Windows, o
  /// caminho montado ficava misto/quebrado. Convertendo tudo para `/`
  /// (`C:/Users/x/.flightdeck/bin/flightdeck-hook.exe`) o bash executa normalmente. Em
  /// POSIX é no-op.
  String hookPath(String path) =>
      Platform.isWindows ? path.replaceAll(r'\', '/') : path;

  /// Caminho do helper empacotado no app, por plataforma:
  /// - macOS: `…/Contents/MacOS/<app>` → `…/Contents/Resources/flightdeck-hook`
  /// - Windows/Linux: ao lado do executável (`<dir>/flightdeck-hook[.exe]`)
