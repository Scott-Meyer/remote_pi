...
import 'package:flightdeck/app/flightdeck/data/hooks/hook_installer_base.dart';
...
/// nomes de evento, mesmo envelope JSON pelo stdin —, então o helper (`flightdeck
...
class CodexHookInstallerImpl extends HookInstallerBase {
  ...
  static const String _tomlBegin = '# >>> flightdeck hooks (auto-gerado)';
  static const String _tomlEnd = '# <<< flightdeck hooks';
  ...
  Future<void> _writeTrust({
    ...
    final buffer = StringBuffer()
      ..writeln(_tomlBegin)
      ..writeln('# Confia no helper de status de turno do FlightDeck.')
  ...
  bool _isOurs(dynamic group) {
    ...
    return inner.any((h) {
      ...
      return cmd.contains('flightdeck-hook') ||
          cmd.contains('flightdeck" hook') ||
          cmd.endsWith('flightdeck hook');
    });
  }
}
