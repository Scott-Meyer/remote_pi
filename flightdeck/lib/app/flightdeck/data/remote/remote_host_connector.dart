...
import 'package:flightdeck/app/flightdeck/data/remote/dartssh_host_connection.dart';
import 'package:flightdeck/app/flightdeck/data/remote/mobile_ssh_key_store.dart';
import 'package:flightdeck/app/flightdeck/data/remote/ssh_worker_connection.dart';
...
import 'package:flightdeck/app/flightdeck/data/remote/ssh_known_hosts.dart';
...
enum RemoteHostErrorKind {
  ...
  /// Host Windows sem FlightDeck instalado. Lá a instalação do servidor é cópia
  /// LOCAL do `flightdeck-server-bundle` que o app deixa (decisão D2 do plano
...
/// 1. túnel SSH pro socket do flightdeck-server remoto;
...
class RemoteHostConnector {
  ...
  /// Resolve o flightdeck-server embarcado usado como fonte do bootstrap, para o
  ...
  /// Comandos da CLI `flightdeck` rodando no host, encaminhados pelo servidor.
  ...
  Future<RemoteConnection?> _tryProtocol(
    ...
    for (var i = 0; i < attempts; i++) {
      ...
      try {
        ...
        // protegido pelas permissões do `~/.flightdeck`); host Windows tem, e o
  ...
  Future<void> _installAndStartServer(HostShell shell) async {
    ...
    if (!installed) {
      if (shell.installsFromHostBundle) {
        // Windows (D2): instalação é cópia LOCAL do bundle do FlightDeck
        ...
        if (binary == null) {
          ...
          throw RemoteHostException(
            RemoteHostErrorKind.serverInstallFailed,
            'local flightdeck-server binary not found for '
