...
class WindowsHostShell extends HostShell {
  ...
  static const _root = r'$env:USERPROFILE\.flightdeck';

  @override
  String get serverBinaryPath =>
      r'$env:USERPROFILE\.flightdeck\server\bin\flightdeck-server.exe';

  @override
  String get bootLogPath => r'$env:USERPROFILE\.flightdeck\server-boot.log';
  ...
  /// servidor já de pé.
  static const _endpointFile = r'$env:USERPROFILE\.flightdeck\flightdeck-server.sock';

  /// Bundle do FlightDeck instalado no host — a fonte da instalação (D2).
  static const _hostBundle =
      r'$env:LOCALAPPDATA\Programs\Remote Pi FlightDeck\flightdeck-server-bundle';
...
''');
  ...
  /// próprio sidecar `flightdeck-server.exe`, e derrubar o processo errado
  /// mataria os terminais locais de quem está sentado na máquina.
  @override
  Future<void> killServer() async {
    await _ps('''
Get-CimInstance Win32_Process -Filter "Name='flightdeck-server.exe'" |
...
''');
  ...
  /// método é o contrato falando — se um dia o upload voltar (host sem FlightDeck
  ...
  /// Copia o `flightdeck-server-bundle` do FlightDeck instalado no host para
  /// `~/.flightdeck/server` — cópia LOCAL, nenhum byte de binário pelo SSH (D2).
  @override
  Future<bool> installFromHost() async {
    final (code, out, err) = await _ps('''
\$bundle = "$_hostBundle"
if (-not (Test-Path "\$bundle\\bin\\flightdeck-server.exe")) { 'missing'; exit 0 }
\$root = "$_root\\server"
...
''');
  ...
  /// - `FLIGHTDECK_PTY_DYLIB` não é herdado por esse caminho; não faz falta,
  ///   porque o `flightdeck_pty.dll` é copiado AO LADO do exe e esse é o segundo
