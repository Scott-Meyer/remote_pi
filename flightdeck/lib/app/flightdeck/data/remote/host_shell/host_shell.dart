...
///
/// O bootstrap remoto (instalar/subir/derrubar o `flightdeck-server`, descobrir
...
/// Como alcançar o `flightdeck-server` de um host.
///
/// O servidor já nasce diferente nas duas famílias (ver `LocalEndpoint`, no
/// `flightdeck_protocol`): no POSIX é um socket UNIX num caminho conhecido; no
...
sealed class RemoteEndpoint {
  ...
  /// UNIX já nasce protegido pelas permissões do `~/.flightdeck`.
  String? get token;
}
...
class ClientBundle {
  ...
  /// Raiz do bundle: `<root>/bin/flightdeck-server` + `<root>/lib/*`.
  final String root;
  final String serverBinary;

  /// Arquivos que realmente chegam ao host, com seus nomes canônicos lá.
  ///
  /// O bundle macOS universal pode selecionar `flightdeck-server-arm64`, mas o
  /// host sempre o recebe como `bin/flightdeck-server`; o manifesto descreve o
  /// destino, não o nome incidental da fatia local.
  List<ClientBundleFile> deployedFiles() {
    final files = <ClientBundleFile>[
      ClientBundleFile(serverBinary, 'bin/flightdeck-server'),
    ];
    final cli = File('$root/bin/flightdeck');
    if (cli.existsSync()) files.add(ClientBundleFile(cli.path, 'bin/flightdeck'));
...
abstract class HostShell {
  ...
  /// copia o do FlightDeck instalado no próprio host (D2), então um cliente macOS
  ...
  /// Instala a partir do FlightDeck já instalado no HOST (Windows, D2).
