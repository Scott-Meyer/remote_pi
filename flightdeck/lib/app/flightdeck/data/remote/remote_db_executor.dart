...
/// Lê as conexões de DB de um workspace REMOTO: o `.flightdeck/databases.json`
...
Future<List<DbConnection>> loadRemoteConnections(
  ...
  Future<List<DbConnection>> read(
    ...
    try {
      final bytes = await fs.read('$workspaceRoot/.flightdeck/$name');
...
/// `flightdeck-server` (plano 58, Wave 4). [dbServiceProvider] resolve o serviço
...
/// [NoSqlRunner] que roda Redis/Mongo no host via `flightdeck-server` (plano 58,
