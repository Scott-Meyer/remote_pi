...
/// Túnel SSH para o socket do `flightdeck-server` de um host remoto (plano 58,
...
class SshTunnel {
  ...
  /// Socket local que espelha o socket remoto do flightdeck-server. Vazio quando
  ...
  static Future<SshTunnel> open({
    ...
    final localPath = useTcp
        ? ''
        : '${Directory.systemTemp.path}/flightdeck-ssh-'
...
class _Askpass {
  ...
  static Future<_Askpass?> create(String? password) async {
    ...
    final pwFile = File('$base/flightdeck-ssh-pw-$stamp');
    await pwFile.writeAsString(password, flush: true);
    final helper = File('$base/flightdeck-ssh-askpass-$stamp.sh');
