...
/// [WorktreeManager] local, mas roda tudo via `git.run` no `flightdeck-server` do
/// host — não há RPC novo, só sequências de git. O `.gitignore` é lido/escrito
/// pelo serviço de arquivos remoto (o worktree mora em `.flightdeck/worktrees/`,
...
class RemoteWorktreeGateway {
  ...
  /// local (`.flightdeck/worktrees/<name>`).
  static const worktreesDir = '.flightdeck/worktrees';
  ...
  /// Cria uma worktree em `<repo>/.flightdeck/worktrees/<name>` numa branch nova
  /// [name] (de [baseRef] no fork-of-fork). Garante `.flightdeck/worktrees/` no
  /// `.gitignore` antes. Devolve o resultado do `git worktree add`; o caminho
  /// criado é `<repo>/.flightdeck/worktrees/<name>`.
  ...
  /// Garante `.flightdeck/worktrees/` no `.gitignore` do repo (best-effort): sem
