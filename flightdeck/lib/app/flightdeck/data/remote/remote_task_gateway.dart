...
import 'package:flightdeck/app/flightdeck/data/tasks/tasks_json_loader.dart';
...
/// `<root>/.flightdeck/tasks.json` do host via `fs.read` e reusa o parser do
...
class RemoteTaskDiscovery implements TaskDiscovery {
  ...
  Future<List<TaskDefinition>> discover(String cwd) async {
    ...
    try {
      ...
      final bytes = await files.read('$cwd/.flightdeck/tasks.json');
...
/// `flightdeck-server` (via [TerminalService]) e mapeia os [PtyEvent] pro modelo
