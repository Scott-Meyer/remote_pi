...
import 'package:flightdeck/app/flightdeck/ui/widgets/git_process_dialog.dart';
...
///
/// Moram fora da `FlightDeckPage` porque não são apresentação: são orquestração de
...
Future<bool> addProject(BuildContext context) async {
  ...
  final path = await NativeFolderPicker.pick(
    dialogTitle: context.t.flightdeck.flightdeckPage.chooseProjectFolderDialogTitle,
...
Future<bool> createWorkspace(BuildContext context) async {
  final vm = _vm(context);
  final path = await NativeFolderPicker.pick(
    dialogTitle: context.t.flightdeck.flightdeckPage.chooseWorkspaceFolderDialogTitle,
    initialDirectory: vm.selectedProject?.path,
  );
...
Future<void> configureProject(BuildContext context, Project project) async {
  ...
  await showInfoDialog(
    context,
    title: context.t.flightdeck.flightdeckPage.workspaceRenamedTitle,
    message: context.t.flightdeck.flightdeckPage.workspaceRenamedMessage(
      name: result.name,
    ),
  );
}
...
/// **Não deleta** a pasta no disco — só sai do flightdeck.
Future<void> deleteProject(BuildContext context, Project project) async {
  final vm = _vm(context);
  final ok = await showConfirmDialog(
    context,
    title: context.t.flightdeck.flightdeckPage.closeWorkspaceTitle,
    message: context.t.flightdeck.flightdeckPage.closeWorkspaceMessage(
      name: project.name,
    ),
    confirmLabel: context.t.flightdeck.flightdeckPage.closeAction,
    danger: true,
  );
  if (!ok) return;
...
Future<void> syncProject(
  ...
  title: (label) => context.t.flightdeck.flightdeckPage.syncTitle(label: label),
);
...
Future<void> pullProject(
  ...
  title: (label) => context.t.flightdeck.flightdeckPage.pullTitle(label: label),
);
...
Future<void> pushProject(
  ...
  title: (label) => context.t.flightdeck.flightdeckPage.pushTitle(label: label),
);
...
Future<void> _gitOpDialog(
  ...
  Project project,
  String rootPath, {
  required GitRun Function(FlightDeckViewModel vm) run,
...
Future<void> createRealm(BuildContext context) async {
  ...
  final name = await showRealmNameDialog(
    context,
    title: context.t.flightdeck.flightdeckPage.newRealmTitle,
...
List<RealmTarget> moveTargets(FlightDeckViewModel vm, String projectId) {
