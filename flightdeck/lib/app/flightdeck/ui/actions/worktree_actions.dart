...
Future<void> updateWorktree(BuildContext context, Project fork) async {
  ...
  await showGitProcessDialog(
    context,
    title: context.t.flightdeck.flightdeckPage.updateFromParentTitle(name: fork.name),
    output: run.output,
...
Future<void> mergeWorktree(BuildContext context, Project fork) async {
  ...
  await showGitProcessDialog(
    context,
    title: context.t.flightdeck.flightdeckPage.mergeToParentTitle(name: fork.name),
    output: outcome.output,
    success: outcome.status.then((s) => s == GitMergeStatus.merged),
    finalMessage: (ok) => ok
        ? context.t.flightdeck.flightdeckPage.worktreeMergedAndRemoved
        : context.t.flightdeck.flightdeckPage.nothingWasChanged,
  );
}
...
Future<void> removeWorktree(BuildContext context, Project fork) async {
  ...
  final warn = merged
      ? ''
      : context.t.flightdeck.flightdeckPage.removeWorktreeWarning(name: fork.name);
  final ok = await showConfirmDialog(
    context,
    title: context.t.flightdeck.flightdeckPage.removeWorktreeTitle,
    message: context.t.flightdeck.flightdeckPage.removeWorktreeMessage(
      name: fork.name,
      warn: warn,
    ),
  ...
  await showInfoDialog(
    context,
    title: context.t.flightdeck.flightdeckPage.failedToRemoveWorktreeTitle,
    message: err,
  );
}
