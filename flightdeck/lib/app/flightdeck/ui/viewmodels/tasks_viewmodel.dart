...
class TasksViewModel extends ChangeNotifier {
  ...
  /// Contexto de tasks REMOTO por cwd (plano 58): setado pela `FlightDeckPage`,
  ...
  /// `true` se já existe um `.flightdeck/tasks.json` no projeto (esconde o botão
  /// de criar exemplo).
  ...
  String _configPath(String cwd) {
    ...
    return '$cwd$sep.flightdeck${sep}tasks.json';
  }
  ...
  /// Cria um `.flightdeck/tasks.json` de exemplo (Flutter + Node + C#) no projeto
  ...
  Future<void> createExampleConfig() async {
    ...
    final sep = Platform.pathSeparator;
    final dir = Directory('$_cwd$sep.flightdeck');
    await dir.create(recursive: true);
    final file = File(_configPath(_cwd));
    if (!await file.exists()) {
      await file.writeAsString(GalleryTemplate.tasks.content);
    }
    _watchConfig(_cwd); // `.flightdeck` agora existe → arma o watcher
    await reload();
  }

  /// Observa o `.flightdeck/tasks.json` do projeto e redescobre (debounced) quando
  ...
  void _watchConfig(String cwd) {
    ...
    final dir = Directory('$cwd${Platform.pathSeparator}.flightdeck');
    try {
