...
import 'package:flightdeck/app/flightdeck/domain/utils/coalescing_single_flight.dart';
...
/// Estado git do shell, extraído do `FlightDeckViewModel` (refactor 2026-07-19):
...
class GitController extends ChangeNotifier {
  ...
  /// `true` se o projeto é o workspace de sistema "FlightDeck" (sem pasta).
  ...
  Future<void> _refreshOnce(String projectId) async {
    ...
    // Sink único de git — barra o FlightDeck (sem pasta) de uma vez: cobre o
  ...
  void watchProject(String? projectId) {
    ...
    // FlightDeck não tem pasta → nunca observa (evita `Directory('').watch`).
