...
import 'package:flightdeck/app/flightdeck/ui/widgets/http_request_view.dart';
import 'package:flightdeck/app/flightdeck/ui/widgets/kanban_board_view.dart';
import 'package:flightdeck/app/flightdeck/ui/widgets/notebook_view.dart';
import 'package:flightdeck/app/flightdeck/ui/widgets/agent_composer.dart';
import 'package:flightdeck/app/flightdeck/ui/widgets/agent_setup_checklist.dart';
import 'package:flightdeck/app/flightdeck/ui/widgets/agent_transcript.dart';
import 'package:flightdeck/app/flightdeck/ui/widgets/browser_pane.dart';
import 'package:flightdeck/app/flightdeck/ui/widgets/active_listenable_builder.dart';
...
import 'package:flightdeck/app/flightdeck/ui/widgets/empty_pane.dart';
import 'package:flightdeck/app/flightdeck/ui/widgets/diff_viewer.dart';
import 'package:flightdeck/app/flightdeck/ui/widgets/db_query_view.dart';
import 'package:flightdeck/app/flightdeck/domain/entities/db_connection.dart';
import 'package:flightdeck/app/flightdeck/ui/widgets/db_engine_icon.dart';
import 'package:flightdeck/app/flightdeck/ui/widgets/db_mongo_view.dart';
import 'package:flightdeck/app/flightdeck/ui/widgets/db_redis_table.dart';
import 'package:flightdeck/app/flightdeck/ui/widgets/file_viewer.dart';
import 'package:flightdeck/app/flightdeck/ui/widgets/adaptive_terminal_pane.dart';
import 'package:flightdeck/app/flightdeck/ui/widgets/pane_tab_leading.dart';
...
class _TabStripState extends State<_TabStrip> {
  ...
  Future<void> _confirmClosePane(BuildContext context) async {
    ...
    final tr = context.t.flightdeck.paneView;
            ...
            if (_overflowing)
              Builder(
                builder: (ctx) => _StripButton(
                  ...
                  tooltip: context.t.flightdeck.paneView.allTabs,
...
class _TabAdd extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final colors = context.colors;
    return AppTooltip(
      message: context.t.flightdeck.paneView.newTab,
...
///
/// **Só no Windows** (ver `FlightDeckViewModel.showTerminalProfilePicker`): no
...
class _TabProfilePicker extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final colors = context.colors;
    return AppTooltip(
      message: context.t.flightdeck.paneView.newTerminal,
      child: GestureDetector(
        behavior: HitTestBehavior.opaque,
        onTap: () => _open(context),
        child: MouseRegion(
...
class _PaneTools extends StatelessWidget {
  ...
  Future<void> _openMenu(BuildContext ctx) async {
    final tr = ctx.t.flightdeck.paneView;
...
class _PaneBody extends StatefulWidget {
  ...
  /// Geração de "reassumir o teclado" da VM ([FlightDeckViewModel.tabFocusGen]).
  ...
  Widget build(BuildContext context) {
    ...
    if (item is FileViewerSession) {
      return FileViewer(
        ...
        focused: widget.focused,
        onSave: (content) =>
            context.read<FlightDeckViewModel>().saveFile(item.id, content),
      );
    }
          ...
          child: Padding(
            ...
            child: AdaptiveTerminalPane(
              ...
              onOpenFile: (path, {line}) =>
                  context.read<FlightDeckViewModel>().openTerminalPath(
                    path,
          ...
          child: ColoredBox(
            ...
            child: Padding(
              ...
              child: AdaptiveTerminalPane(
                ...
                // `Apple_Terminal`, então numa aba do FlightDeck ele não vem, e
    ...
    return ActiveListenableBuilder(
      ...
      builder: (context, _) {
        if (agent.status == AgentStatus.empty) {
          // No workspace de sistema "FlightDeck" agentes são desligados **sempre**
          // (terminal-only por construção), independente da flag global.
          final enableAgent =
              context.watch<SettingsController>().settings.enableAgent &&
              !context.read<FlightDeckViewModel>().isSystemTerminal(
                agent.projectId,
              );
        ...
        return _OpenTabDropTarget(
          vm: context.read<FlightDeckViewModel>(),
