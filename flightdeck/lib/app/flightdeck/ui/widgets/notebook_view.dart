...
import 'package:flightdeck/app/core/ui/widgets/markdown_editing_controller.dart';
...
class _NotebookViewState extends State<NotebookView> {
  ...
  Future<void> _pickNoteLink(Offset at) async {
    ...
    final choice = await showAppMenu<String>(
      context,
      globalPosition: at,
      searchHint: context.t.flightdeck.notebook.format.noteLinkSearch,
      searchThreshold: 6,
      items: [
  ...
  Future<void> _setTags(List<String> tags) async {
    ...
    if (!ok) {
      await showConfirmDialog(
        context,
        title: context.t.flightdeck.notebook.saveFailed,
  ...
  Future<void> _tagMenu(String tag, Offset position) async {
    ...
    final tr = context.t.flightdeck.notebook;
    final choice = await showAppMenu<String>(
      context,
  ...
  Future<void> _newNote() async {
    final title = context.t.flightdeck.notebook.untitled;
    final now = DateTime.now();
    var path = joinPath(
      widget.session.path,
    ...
    if (!ok) {
      await showConfirmDialog(
        context,
        title: context.t.flightdeck.notebook.createFailed,
        message: path,
        confirmLabel: context.t.common.ok,
      );
      return;
  ...
  Future<void> _addImageBytes(Uint8List bytes, String preferredName) async {
    ...
    if (!ok) {
      await showConfirmDialog(
        context,
        title: context.t.flightdeck.notebook.imageFailed,
        message: path,
        confirmLabel: context.t.common.ok,
      );
      return;
...
class _NotesColumn extends StatelessWidget {
  ...
  Widget _body(
    ...
    AppColors colors,
    Translations$flightdeck$notebook$en tr,
  ) {
...
class _FormatBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final colors = context.colors;
    final tr = context.t.flightdeck.notebook.format;
