...
import 'package:flightdeck/app/flightdeck/ui/widgets/db_connection_dialog.dart';
...
import 'package:flightdeck/app/flightdeck/ui/db_error_message.dart';
...
/// do workspace — registradas (`.flightdeck/databases.json`), locais e sqlites
...
class _DbPanelState extends State<DbPanel> {
  ...
  Future<void> _contextMenu(BuildContext anchor, DbConnection conn) async {
    final tr = context.t.flightdeck.dbPanel;
    ...
    switch (action) {
      ...
      case 'dbq':
        _newDbq(conn);
      case 'redis':
        context.read<FlightDeckViewModel>().openRedisBrowser(conn.name);
      case 'delete':
        await _delete(conn);
    }
  }
  ...
  void _newDbq(DbConnection conn, {SchemaTable? table}) {
    ...
    context.read<FlightDeckViewModel>().openScratchDbq(
      connName: conn.name,
      sql: table == null
          ? null
          : 'SELECT * FROM '
  ...
  Future<void> _delete(DbConnection conn) async {
    ...
    final ok = await showConfirmDialog(
      context,
      title: context.t.flightdeck.dbPanel.deleteConnectionTitle,
      message: context.t.flightdeck.dbPanel.deleteConnectionMessage(
        name: conn.name,
      ),
      ...
      children: [
        Padding(
          ...
          child: Row(
            children: [
              Text(
                context.t.flightdeck.dbPanel.sectionDatabase,
        ...
        Expanded(
          child: conns.isEmpty
              ? Padding(
                  ...
                  child: Text(
                    context.t.flightdeck.dbPanel.noConnections,
                    style: typo.label.copyWith(
                      fontSize: 11.5,
                      color: colors.text3,
                      height: 1.5,
        ...
        Container(
          ...
          child: Text(
            ...
            // não só o sufixo.
            conns.length == 1
                ? context.t.flightdeck.dbPanel.footerOne
                : context.t.flightdeck.dbPanel.footer(n: conns.length),
        ...
        Padding(
          ...
          child: HoverTap(
            onTap: expandable
                ? onToggle
                : isRedis
                ? () => context.read<FlightDeckViewModel>().openRedisBrowser(
                    conn.name,
                  )
                : null,
        ...
        child: HoverTap(
          ...
          child: Row(
            children: [
              ...
              AppTooltip(
                message: context.t.flightdeck.dbPanel.newQuery,
                child: HoverTap(
...
/// conexão — é o que a tab do browser e o `flightdeck mongo` do agente usam.
