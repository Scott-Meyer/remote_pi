...
import 'package:flightdeck/app/flightdeck/domain/entities/redis_key.dart';
...
class _RedisTableViewState extends State<RedisTableView> {
  ...
  void initState() {
    ...
    // Pattern semeado pelo CLI (`flightdeck redis browse`) vence o estado salvo.
  ...
  Future<void> _commitTtlEdit() async {
    ...
    if (raw.isNotEmpty && raw != '-1' && seconds == null) {
      setState(
        () => _view.error = context.t.flightdeck.dbRedisTable.ttlMustBeNumber,
      );
      return;
    }
  ...
  Future<void> _delete(RedisKeyEntry entry) async {
    final tr = context.t.flightdeck.dbRedisTable;
        ...
        children: [
          ...
          SizedBox(
            ...
            child: TextField(
              ...
              placeholder: Text(
                context.t.flightdeck.dbRedisTable.searchHint,
          ...
          const Spacer(),
          Text(
            '${context.t.flightdeck.dbRedisTable.keyCount(n: _view.entries.length)}'
            '${_view.cursor == '0' ? '' : '+'}',
            style: typo.label.copyWith(fontSize: 11, color: colors.text3),
          ),
          const SizedBox(width: 8),
          AppTooltip(
            message: context.t.flightdeck.dbRedisTable.refresh,
            child: HoverTap(
          ...
          AppTooltip(
            message: context.t.flightdeck.dbRedisTable.newKey,
            child: HoverTap(
              onTap: _openDraft,
              padding: const EdgeInsets.all(4),
        ...
        return Padding(
          ...
          child: Center(
            child: OutlineButton(
              onPressed: () => _loadMore(),
              child: Text(
                context.t.flightdeck.dbRedisTable.loadMore,
    ...
    return Container(
      ...
      child: _expandedLoading
          ? Padding(
              padding: const EdgeInsets.all(8),
              child: Text(
                context.t.flightdeck.dbRedisTable.loadingFullValue,
                style: typo.label.copyWith(fontSize: 11, color: colors.text3),
              ),
            )
          : Column(
