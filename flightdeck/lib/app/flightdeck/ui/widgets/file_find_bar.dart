...
class FileFindBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (invalidRegex) {
      counter = context.t.flightdeck.findBar.badPattern;
    } else if (!hasQuery) {
      counter = '';
    } else if (matchCount == 0) {
      counter = context.t.flightdeck.findBar.noResults;
    } else {
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          SizedBox(
            ...
            child: CallbackShortcuts(
              ...
              child: TextField(
                ...
                focusNode: focusNode,
                placeholder: Text(
                  context.t.flightdeck.findBar.find,
          ...
          _FindToggle(
            label: 'Aa',
            tooltip: context.t.flightdeck.findBar.matchCase,
            active: caseSensitive,
            onTap: onToggleCase,
          ),
          _FindToggle(
            label: 'ab',
            tooltip: context.t.flightdeck.findBar.wholeWord,
            active: wholeWord,
            underline: true,
            onTap: onToggleWord,
          ),
          _FindToggle(
            label: '.*',
            tooltip: context.t.flightdeck.findBar.useRegex,
            active: regex,
            onTap: onToggleRegex,
          ),
          const SizedBox(width: 6),
          ...
          _NavButton(
            ...
            tooltip: context.t.flightdeck.findBar.previous,
            enabled: matchCount > 0,
            onTap: onPrev,
          ),
          _NavButton(
            icon: Icons.keyboard_arrow_down,
            tooltip: context.t.flightdeck.findBar.next,
            enabled: matchCount > 0,
            onTap: onNext,
          ),
          _NavButton(
            icon: Icons.close,
            tooltip: context.t.flightdeck.findBar.close,
            enabled: true,
            onTap: onClose,
          ),
        ],
