...
import 'package:flightdeck/app/core/terminal/flightdeck_terminal.dart';
import 'package:flightdeck/app/core/terminal/flightdeck_terminal_render.dart';
import 'terminal_link.dart';

/// Envólucro do [FlightDeckTerminal] que adiciona **auto-scroll durante a
...
class _TerminalPaneState extends State<TerminalPane>
  ...
  final _viewKey = GlobalKey<FlightDeckTerminalState>();
  ...
  FlightDeckTerminalRender? get _render => _viewKey.currentState?.renderTerminal;
  ...
  void _onPointerSignal(PointerSignalEvent e) {
    ...
    // alt-buffer) é desligado nesse modo (ver flightdeck_terminal.dart) — então não
  ...
  void _beginSelecting(FlightDeckTerminalRender r, Offset down) {
    _anchor?.dispose();
  ...
  void _extendSelection(FlightDeckTerminalRender r) {
  ...
  void _syncAutoScroll(FlightDeckTerminalRender r) {
    if (_overshoot(r) == 0) {
  ...
  double _overshoot(FlightDeckTerminalRender r) {
    final p = _pointer;
    if (p == null) return 0;
    final h = r.size.height;
  ...
  Widget build(BuildContext context) {
    ...
    final terminal = MouseRegion(
      ...
      child: Listener(
        ...
        // Cmd, senão I-beam); o FlightDeckTerminal defere o dele pra cá.
        child: FlightDeckTerminal(
          widget.terminal,
          key: _viewKey,
    ...
    // do teclado) — ver FlightDeckTopbar. Aqui a pane é só o terminal.
    return terminal;
  }
}
