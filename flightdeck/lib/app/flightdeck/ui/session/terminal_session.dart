...
import 'package:flightdeck/app/core/domain/entities/harness.dart';
...
import 'package:flightdeck/app/core/terminal/pty_output_scheduler.dart';
import 'package:flightdeck/app/core/utils/quiet_period_debouncer.dart';
import 'package:flightdeck/app/flightdeck/ui/session/pane_item.dart';
import 'package:flightdeck/app/flightdeck/ui/session/terminal_input.dart';
...
/// terminal, reportado pelo `flightdeck-hook` via socket (ver
...
class TerminalSession extends PaneItem {
  TerminalSession({
    ...
    if (spawned != null && spawned.fellBack) {
      ...
      terminal.write(
        '\x1b[33m'
        '${t.flightdeck.terminal.cwdFallbackWarning(requested: spawned.requested, path: spawned.path)}'
        '\x1b[0m\r\n',
      );
    }
  ...
  /// Aplica um status reportado pelo `flightdeck hook` (via [TerminalStatusServer]).
  ...
  String _title;
  late final FlightDeckTerminalController terminal;
