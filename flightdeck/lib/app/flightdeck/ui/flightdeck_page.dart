...
import 'package:flightdeck/app/flightdeck/ui/actions/agent_actions.dart';
import 'package:flightdeck/app/flightdeck/ui/actions/remote_workspace_actions.dart';
import 'package:flightdeck/app/flightdeck/ui/actions/tab_actions.dart';
import 'package:flightdeck/app/flightdeck/ui/actions/workspace_actions.dart';
import 'package:flightdeck/app/flightdeck/ui/actions/worktree_actions.dart';
import 'package:flightdeck/app/core/app_intents.dart';
...
import 'package:flightdeck/app/flightdeck/data/remote/remote_db_executor.dart';
import 'package:flightdeck/app/flightdeck/data/remote/remote_task_gateway.dart';
...
import 'package:flightdeck/app/flightdeck/ui/widgets/remote_disconnected_banner.dart';
import 'package:flightdeck/app/flightdeck/ui/widgets/terminal_key_bar.dart';
...
import 'package:flightdeck/app/flightdeck/data/remote/remote_db_writer_impl.dart';
import 'package:flightdeck/app/flightdeck/domain/contracts/remote_db_writer.dart';
...
import 'package:flightdeck/app/flightdeck/ui/widgets/ssh_prompts.dart';

/// Os únicos valores do `FlightDeckViewModel` de que o shell (top bar + arranjo
...
/// Shell do FlightDeck: top bar + rail de projetos + multiplexador (árvore de
...
class _FlightDeckPageState extends State<FlightDeckPage> {
  FlightDeckViewModel get _vm => context.read<FlightDeckViewModel>();
  ...
  void initState() {
    ...
    context.read<FlightDeckViewModel>()
    ...
    context.read<FlightDeckViewModel>().init();
    ...
    // build da própria FlightDeckPage → "setState() or markNeedsBuild() called
    ...
    _menuVm = context.read<FlightDeckViewModel>()..addListener(_syncWorkspaceMenu);
    _syncWorkspaceMenu();
    ...
    final vm = context.read<FlightDeckViewModel>();
  ...
  void _wireSshPrompts() {
    ...
    service
      ...
      // a query roda no flightdeck-server do host. Resolvido por workspace pra
      ...
      // (.flightdeck/databases.json) — resolução da query E leitura do painel.
  ...
  void _syncWorkspaceMenu() {
    ...
    _workspaceMenu?.setWorkspace(
      ...
      // FlightDeck é terminal-only → sem "New Agent" no menu File.
  ...
  /// Espelha o toggle "Show FlightDeck terminal" (Configurações › General) para a
  ...
  void _syncFlightDeck() {
    _vm.setFlightDeckEnabled(_settings!.settings.showFlightDeck);
  }
  ...
  void dispose() {
    ...
    _settings?.removeListener(_syncFlightDeck);
  ...
  Widget build(BuildContext context) {
    ...
    final shell = context.select<FlightDeckViewModel, _ShellState>(
      (vm) => (
        ready: vm.ready,
        ...
        child: Focus(
          ...
          child: Scaffold(
            ...
            child: Column(
              children: [
                FlightDeckTopbar(
                  projectName: shell.title ?? 'FlightDeck',
                  ...
                  onToggleTree: narrow
                      ...
                      : _vm.toggleTree,
                  // Remoto TEM árvore (a pasta do host); só o FlightDeck
...
class _RailPanel extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final vm = context.watch<FlightDeckViewModel>();
    return Stack(
      children: [
        ProjectsRail(
          width: width,
          ...
          flightdeck: vm.flightdeckWorkspace,
          onSelectFlightDeck: () {
            vm.selectProject(Project.flightdeckId);
            onDismiss();
          },
...
class _CenterPanel extends StatelessWidget {
  ...
  int _activeIndex(FlightDeckViewModel vm) {
...
Future<void> _createFromGallery(
  ...
  if (result case Failure(:final error)) {
    await showConfirmDialog(
      context,
      title: context.t.flightdeck.gallery.createErrorTitle,
          ...
          onOpenLayout: (path) async {
            ...
            if (res case Failure(:final error)) {
              await showInfoDialog(
                context,
                title: context.t.flightdeck.flightdeckPage.openLayoutTitle,
                message: error,
              );
            }
          },
...
class _LspStatusBarState extends State<_LspStatusBar> {
  ...
  void initState() {
    super.initState();
    _sub = context.read<FlightDeckViewModel>().lspStatusChanges.listen((_) {
      if (mounted) setState(() {});
    });
  }
  ...
  Future<void> _restart(FlightDeckViewModel vm) async {
    ...
    return Container(
      ...
      child: status == null
          ? Align(
              ...
              child: Text(
                context.t.flightdeck.flightdeckPage.noLspAvailable,
          ...
          : Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    '${status.label} LSP · ${status.running ? context.t.flightdeck.flightdeckPage.lspRunning : context.t.flightdeck.flightdeckPage.lspStopped}',
                    overflow: TextOverflow.ellipsis,
                    style: context.typo.label.copyWith(color: colors.text2),
                  ),
                ),
                AppTooltip(
                  message: context.t.flightdeck.flightdeckPage.restartServerTooltip,
                  child: HoverTap(
