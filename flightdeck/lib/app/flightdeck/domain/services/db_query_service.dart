...
import 'package:flightdeck/app/flightdeck/domain/db_schema_sql.dart';
...
/// pelo app (fala com o `flightdeck-server` via `RemoteDbService`); o
...
/// CLI `flightdeck db` (decisão J do plano 51): resolve a conexão por nome no
...
class DbQueryService {
  ...
  /// devolve não-nulo, a query SQL vai pro `flightdeck-server` do host (que
  ...
  /// Conexões de um workspace remoto (o `.flightdeck/databases.json` vive no
  ...
  /// `flightdeck-server` do host. Setado pelo app junto do [remoteExecutorFor];
  ...
  /// e aí as conexões vêm do `.flightdeck/databases.json` do HOST, não do disco
  /// daqui. Sem esse parâmetro a função lia sempre o store local: num workspace
  /// remoto o caminho não existe no cliente, e `flightdeck db list` respondia uma
  ...
  /// entra na chave do cofre (`flightdeck.db.<workspaceId>.<nome>`).
  Future<DbResult> query({
    ...
    // Workspace remoto (plano 58): a query roda no flightdeck-server do host.
  ...
  /// (plano 53) e o `flightdeck mongo` do agente — mesmo caminho, mesmo database.
  ///
  ...
  static String legacySecretKey(String workspaceId, String connName) =>
      'flightdeck.db.$workspaceId.$connName';

  static String legacySshSecretKey(String workspaceId, String connName) =>
      'flightdeck.ssh.$workspaceId.$connName';
  ...
  /// (`.flightdeck/databases.json` lá) — sem túnel local, sem store local.
  ///
    ...
    if (prompt == null) {
      ...
      throw DbQueryException(
        ...
        'Connection "${conn.name}" uses a passphrase-protected SSH key and no '
            'passphrase is saved. Open it in the FlightDeck UI and enable '
  ...
  /// `flightdeck.db.<workspaceId>.<conn>`, e `workspaceId` é local de cada
