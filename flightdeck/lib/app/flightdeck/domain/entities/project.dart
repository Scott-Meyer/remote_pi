...
/// - [systemTerminal]: o workspace sintético "FlightDeck" — sem pasta, terminal-only,
...
///   roda no `flightdeck-server` do host via SSH). Serviços de path LOCAIS não sobem
...
/// Agentes do FlightDeck atuam em subpastas de [path].
class Project {
  const Project({
    required this.id,
    required this.name,
  ...
  /// Id sentinela do workspace de sistema "FlightDeck". Não é um UUID, então nunca
  /// colide com o id de um projeto real, e o repositório Hive nunca o retorna
  /// (só é injetado em runtime).
  static const String flightdeckId = '__flightdeck__';

  /// Constrói o workspace sintético "FlightDeck" (terminal-only, sem pasta).
  ...
  /// `true` quando este é o workspace sintético "FlightDeck" (terminal-only).
  ...
  /// `true` para qualquer workspace sem pasta local (FlightDeck ou remoto): os
  ...
  /// e, num workspace remoto, recebia vazio — o que fazia `flightdeck db`,
