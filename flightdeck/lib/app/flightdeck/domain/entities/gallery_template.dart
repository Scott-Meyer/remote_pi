/// Um "documento especial" do FlightDeck que a aba Gallery sabe criar na raiz do
/// workspace: extensão própria + conteúdo inicial que já abre na tab certa
/// (`.dbq` → editor SQL, `.kanban` → quadro, `.ckp` → layout, `.http` →
/// cliente HTTP, `.html` → preview renderizado, `.flightdeck/tasks.json` → aba
...
enum GalleryTemplate {
  ...
  notebook(
    ...
    fixedName: true,
    opensParent: true,
    iconAsset: 'assets/file_icons/flightdeck-notebook.svg',
    content:
        '---\n'
        'title: Welcome\n'
        'tags: [agent]\n'
  ...
  layout(
    baseName: 'dev',
    extension: 'ckp',
    iconAsset: 'assets/branding/flightdeck_logo.png',
    content:
        '# Pane layout — apply with right-click → Open layout,\n'
        '# or `flightdeck orchestrate dev.ckp` from a tab.\n'
  ...
  /// `true` = o nome é contrato do app (ex.: `.flightdeck/tasks.json` é o único
  ...
  /// Asset colorido do card (SVG do tema de ícones ou o logo do FlightDeck).
...
/// Modelo de `.flightdeck/tasks.json` gerado pelo botão "Create tasks.json":
...
{
  // .flightdeck/tasks.json — FlightDeck Task Run config (JSONC: // , /* */ and
  // trailing commas are allowed; they're stripped before parsing).
  // Lives at the workspace root you open in FlightDeck. Detected tasks (npm
  // scripts, pubspec) appear automatically; this file adds/overrides them.
  // Full reference: flightdeck/docs/tasks-json.md
  "tasks": [
    {
      ...
      // an IDE plugin) — FlightDeck watches the files and fires `onChange`.
      "watch": {
