...
/// Um tema completo do FlightDeck: identidade + um variant por brilho.
///
...
@immutable
class FlightDeckThemeSpec {
  const FlightDeckThemeSpec({
    required this.id,
    required this.name,
    this.author,
    this.version,
  ...
  /// Id estável, namespaced (`flightdeck.dark`, `acme.midnight`). É o que fica
  ...
  factory FlightDeckThemeSpec.fromJson(
    Map<String, Object?> json, {
    required FlightDeckThemeSpec? Function(String id)? resolveBase,
    required FlightDeckThemeSpec fallbackBase,
  }) {
    final id = json['id'];
