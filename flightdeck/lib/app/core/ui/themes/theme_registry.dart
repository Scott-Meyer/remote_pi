...
import 'package:flightdeck/app/core/ui/themes/app_colors.dart';
import 'package:flightdeck/app/core/ui/themes/syntax_colors.dart';
import 'package:flightdeck/app/core/ui/themes/terminal_theme.dart';
import 'package:flightdeck/app/core/ui/themes/theme_codec.dart';
import 'package:flightdeck/app/core/ui/themes/theme_spec.dart';
...
const String kDefaultThemeId = 'flightdeck';

/// O tema oficial do FlightDeck, e a raiz de todos os outros built-in.
///
...
/// superfícies neutras da UI do FlightDeck, e a que a maioria já reconhece de
/// olhar código no navegador.
final FlightDeckThemeSpec flightdeckDefaultTheme = FlightDeckThemeSpec(
  id: kDefaultThemeId,
  name: 'FlightDeck',
  author: 'Remote Pi',
  dark: _variant(
    ...
    terminal: flightdeckTerminalThemeDark,
  ),
  light: _variant(
    ui: AppColors.light,
    syntax: SyntaxColors.githubLight,
    terminal: flightdeckTerminalThemeLight,
  ),
);

/// **FlightDeck 2** — a hierarquia de superfícies invertida.
///
...
/// não de sintaxe.
final FlightDeckThemeSpec flightdeck2Theme = FlightDeckThemeSpec(
  id: 'flightdeck.2',
  name: 'FlightDeck 2',
  author: 'Remote Pi',
  dark: _variant(
  ...
  light: _variant(
    ...
    // FlightDeck 1 — tão legível quanto o arranjo clássico, só invertida.
...
final FlightDeckThemeSpec panteraTheme = FlightDeckThemeSpec(
  id: 'pantera',
  name: 'Pantera',
  author: 'Remote Pi',
  dark: _variant(
...
///
/// Não é um FlightDeck tingido: a paleta inteira (UI, syntax e ANSI) vem do
/// Flexoki. O arranjo é o clássico do FlightDeck 1 — chrome mais fundo, conteúdo
...
final FlightDeckThemeSpec flexokiTheme = FlightDeckThemeSpec(
  id: 'flexoki',
  name: 'Flexoki',
  author: 'Steph Ango',
  dark: _variant(
    ui: AppColors.dark.copyWith(
      ...
      // elevado — sobe para base-500, o mesmo piso que o FlightDeck 2 precisou.
...
/// herde de graça toda a calibragem do FlightDeck correspondente — a inversão do
/// FlightDeck 2, os degraus de elevação, o contraste dos textos fracos —, e que um
/// ajuste no FlightDeck se propague em vez de criar divergência silenciosa.
///
...
/// hexes. O arranjo de superfícies vem sempre de um FlightDeck; isto aqui é só a
...
///
/// A marca é o **próprio azul do FlightDeck**, e não um azul mais claro puxado
...
/// o azul da marca o botão do Midnight fica idêntico ao do FlightDeck (branco a
...
/// Constrói um tema a partir de um FlightDeck, herdando dele o arranjo de
/// superfícies e trocando só o tom.
/// [darkFrom]/[lightFrom] são independentes de propósito: um arranjo que
/// funciona no escuro pode não funcionar no claro. A inversão do FlightDeck 2 é
...
FlightDeckThemeSpec _tintedThemeOf(
  _Hue tone, {
  required String id,
...
/// **Violet 1** — o arranjo do FlightDeck oficial em roxo.
final FlightDeckThemeSpec violet1Theme = _tintedThemeOf(
  _violet,
  id: 'violet',
  name: 'Violet 1',
  darkFrom: flightdeckDefaultTheme.dark!,
  lightFrom: flightdeckDefaultTheme.light!,
  lightContent: const Color(0xFFFCFAFF),
);

/// **Violet 2** — o arranjo invertido do FlightDeck 2 em roxo: chrome acima,
/// corpo da aba afundado.
final FlightDeckThemeSpec violet2Theme = _tintedThemeOf(
  _violet,
  id: 'violet.2',
  name: 'Violet 2',
  darkFrom: flightdeck2Theme.dark!,
  lightFrom: flightdeck2Theme.light!,
);

/// Base do **claro** dos temas de cor: o arranjo do FlightDeck 1 (chrome fundo,
/// conteúdo no topo) com o chrome puxado mais um passo para baixo.
///
/// A inversão do FlightDeck 2 não sobrevive ao modo claro. No escuro ela funciona
...
/// além do FlightDeck 1. Isso devolve a separação painel/conteúdo que o modo claro
...
final ThemeVariant _deepChromeLightBase = ThemeVariant(
  ui: AppColors.light.copyWith(
    bg: const Color(0xFFE8E8EF), // painéis mais fundos que no FlightDeck 1
    ...
    // FlightDeck 2 sofreu no escuro, aqui espelhado. Com os valores do FlightDeck 1,
...
///
/// Escuro: a inversão do FlightDeck 2 (conteúdo afundado), que é o que faz o
/// marinho ler como "noite" em vez de "azulzinho no fundo".
/// Claro: o arranjo clássico do FlightDeck 1 com o chrome reforçado — ver
/// [_deepChromeLightBase] para o porquê.
final FlightDeckThemeSpec midnightTheme = _tintedThemeOf(
  _midnight,
  id: 'midnight',
  name: 'Midnight',
  darkFrom: flightdeck2Theme.dark!,
...
const _Hue _rose = _Hue(
  ...
  // luminância do FlightDeck 1 (0,914) — o rosa fica no tom, não no peso. A
  ...
  // conteúdo e a moldura some — que é justamente o defeito do FlightDeck 2 claro.
...
final FlightDeckThemeSpec sunTheme = _tintedThemeOf(
  _sun,
  id: 'sun',
  name: 'Sun',
  darkFrom: flightdeck2Theme.dark!,
...
final FlightDeckThemeSpec roseTheme = _tintedThemeOf(
  _rose,
  id: 'rose',
  name: 'Rosê',
  darkFrom: flightdeck2Theme.dark!,
...
final List<FlightDeckThemeSpec> builtInThemes = [
  flightdeckDefaultTheme,
  flightdeck2Theme,
  violet1Theme,
  violet2Theme,
  midnightTheme,
...
FlightDeckThemeSpec? builtInThemeById(String id) {
