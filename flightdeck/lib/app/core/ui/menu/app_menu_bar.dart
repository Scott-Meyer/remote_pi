...
import 'package:flightdeck/app/core/ui/menu/menu_model.dart';
...
/// configurações/projeto, checar updates) — resolvidas pelo `FlightDeckPage`, então
...
List<MenuBarMenu> buildAppMenus(
  ...
  return <MenuBarMenu>[
    ...
    MenuBarMenu('FlightDeck', <MenuNode>[
    ...
    MenuBarMenu(tr.file, <MenuNode>[
      // New Agent/Terminal abrem uma aba no workspace ativo (via FlightDeckPage →
      ...
      // workspace ativo permite agentes (o FlightDeck terminal-only não permite).
    ...
    MenuBarMenu(tr.view, <MenuNode>[
      ...
      // HardwareKeyboard no `FlightDeckPage`. Aqui ficam só os itens clicáveis (com
...
/// **submenus** de um único botão: clique → popup com `FlightDeck`/`File`/`View`/
...
class _WindowMenuBarState extends State<WindowMenuBar> {
  ...
  Future<void> _toggle(BuildContext anchor) async {
    ...
    // shadcn (a ação não dispararia). Os grupos (FlightDeck/File/View/Window) viram
