...
/// Extensões do FlightDeck que o material-icon-theme não conhece (o mapa `.g` é
...
class FileTypeIcon extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // `.ckp` (layout) e a pasta `.flightdeck/` (config) usam o logo do próprio
    // FlightDeck — não são tipos que o material-icon-theme conheça. O `.notebook`
    // tem SVG próprio via `_extensionOverrides` (a árvore o renderiza como
    // arquivo, então cai em `fileIconName`).
    final lower = name.toLowerCase();
    if (lower.endsWith('.ckp') || (_isFolder && lower == '.flightdeck')) {
      return Image.asset(
        'assets/branding/flightdeck_logo.png',
        width: size,
        height: size,
      );
    }
