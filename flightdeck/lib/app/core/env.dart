...
/// Configuração de como o FlightDeck spawna o `pi --mode rpc`.
///
/// **Revisita a decisão B (plano 37)** — o plano precisa registrar as duas
/// mudanças abaixo (tarefa do orquestrador; o agente flightdeck só edita `flightdeck/`):
/// - `noSession = false`: o pi **persiste** a sessão em
///   `~/.pi/agent/sessions/<cwd>/`, e o FlightDeck reanexa via `switch_session`.
...
/// ```bash
/// flutter run -d macos \
///   --dart-define=FLIGHTDECK_PI_PROVIDER=deepseek \
///   --dart-define=FLIGHTDECK_PI_MODEL=deepseek-chat
/// ```
///
...
class PiSpawnConfig {
  ...
  static Future<PiSpawnConfig> resolve() async {
    const provider = String.fromEnvironment('FLIGHTDECK_PI_PROVIDER');
    const model = String.fromEnvironment('FLIGHTDECK_PI_MODEL');
    const pathOverride = String.fromEnvironment('FLIGHTDECK_PI_PATH');
