...
class JsonSettingsStore implements SettingsStore {
  ...
  Future<AppSettings> load() async {
    ...
    if (raw is Map) {
      ...
      // Migração: registro sem `showFlightDeck` (versão anterior à flag) → liga o
      // workspace de sistema e persiste (default já é true; grava pra não
      // re-migrar e alinhar com o padrão de `enableAgent`).
      if (!raw.containsKey('showFlightDeck')) {
        final migrated = settings.copyWith(showFlightDeck: true);
        await save(migrated);
        return migrated;
      }
      return settings;
