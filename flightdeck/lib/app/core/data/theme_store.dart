...
class ThemeStore {
  ...
  Future<List<FlightDeckThemeSpec>> loadAll() async {
    final Directory dir;
    try {
      dir = await themesDir();
    ...
    final themes = <FlightDeckThemeSpec>[];
  ...
  /// gravamos.
  Future<Result<FlightDeckThemeSpec, ThemeStoreError>> import(File source) async {
    final parsed = _parseFile(source);
    if (parsed case Failure(:final error)) return Failure(error);
    final spec = (parsed as Success<FlightDeckThemeSpec, ThemeStoreError>).value;
  ...
  Result<FlightDeckThemeSpec, ThemeStoreError> _parseFile(File file) {
    final String raw;
    try {
  ...
  String _encode(FlightDeckThemeSpec spec) =>
