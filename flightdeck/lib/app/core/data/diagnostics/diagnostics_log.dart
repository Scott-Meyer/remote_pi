...
/// isso, "o FlightDeck fechou sozinho" chega como relato sem nenhuma pista.
///
/// **Por que fora da pasta relocável**: mora em `~/.flightdeck/logs/`, o mesmo lugar
...
class DiagnosticsLog {
  ...
  File? get currentFile {
    ...
    return File('${dir.path}/flightdeck-${_stamp(DateTime.now())}.log');
  }
  ...
  /// `~/.flightdeck/logs`, que não pode depender de configuração (é justamente
  ...
  Future<void> init({
    ...
    try {
      ...
      if (home == null) return;
      final dir = Directory(baseDir != null ? home : '$home/.flightdeck/logs');
      ...
      log('boot', 'FlightDeck $appVersion — ${_platformLine()}');
    } on Object catch (e) {
  ...
  Future<void> _rotate(Directory dir) async {
    ...
    await for (final entity in dir.list(followLinks: false)) {
      ...
      if (!name.startsWith('flightdeck-') || !name.endsWith('.log')) continue;
      try {
