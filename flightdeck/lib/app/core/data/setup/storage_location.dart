...
/// Subdiretório raiz do estado do FlightDeck (JSONs de estado, .hive legados de
/// backup e o cache de scrollback). Em debug usa `flightdeck-debug` para não
/// colidir com a build de produção (que costuma ficar aberta em paralelo
/// durante o desenvolvimento).
const String storageSubdir = kDebugMode ? 'flightdeck-debug' : 'flightdeck';

/// Resolve — e deixa o usuário escolher — a pasta onde o FlightDeck guarda seu
...
/// **não-relocável** (`~/.flightdeck/storage_root`) — um único caminho absoluto (a
...
class StorageLocation {
  ...
  static String? get _pointerPath {
    ...
    return p.join(home, '.flightdeck', 'storage_root');
  }
  ...
  /// "vê" como local do FlightDeck. É sob ela que fica o subdiretório
  /// [storageSubdir].
  ///
  ...
  /// FlightDeck existente naquela pasta). Retorna `true` se copiou.
  ...
  /// Apaga TODO o estado do FlightDeck (JSONs + .hive legados + scrollback) e
