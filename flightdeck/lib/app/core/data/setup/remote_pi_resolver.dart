...
/// Onde a **CLI interna** (`flightdeck`) é materializada, e o único diretório que o
/// app prefixa no PATH das suas abas.
///
/// Namespaceado por debug/release pela mesma razão do `status.sock`: `~/.flightdeck`
...
/// pro seu diretório e o comando continua se chamando `flightdeck` nos dois.
///
/// O `flightdeck-hook` **não** segue este namespace: o caminho dele vai gravado no
...
String? flightdeckCliDir() {
  final home = remotePiHome();
  if (home == null) return null;
  return '$home/.flightdeck/bin${kDebugMode ? '-debug' : ''}';
}

/// Diretório estável do helper de hooks, compartilhado entre builds.
String? flightdeckHookDir() {
  final home = remotePiHome();
  if (home == null) return null;
  return '$home/.flightdeck/bin';
}
