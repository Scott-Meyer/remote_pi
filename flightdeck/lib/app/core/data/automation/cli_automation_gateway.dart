...
import 'package:flightdeck/app/core/domain/contracts/automation_gateway.dart';
...
class CliAutomationGateway implements AutomationGateway {
  ...
  Future<List<AutomationModel>> _discoverCodexModels(String executable) async {
    ...
    process.stdin.writeln(
      jsonEncode({
        ...
        'id': 1,
        'params': {
          'clientInfo': {'name': 'FlightDeck', 'version': '1'},
        },
      }),
    );
    try {
  ...
  Future<GeneratedCommitMessage> generate({
    ...
    if (harnessId == HarnessKind.codex ||
      ...
      scratch = await Directory.systemTemp.createTemp('flightdeck_commit_');
      if (harnessId == HarnessKind.codex) {
