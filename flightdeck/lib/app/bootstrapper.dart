...
import 'package:flightdeck/app/app_module.dart';
import 'package:flightdeck/app/app_widget.dart';
import 'package:flightdeck/app/flightdeck/data/hooks/claude_hook_installer_impl.dart';
import 'package:flightdeck/app/flightdeck/data/hooks/codex_hook_installer_impl.dart';
import 'package:flightdeck/app/flightdeck/data/hooks/pi_hook_installer_impl.dart';
import 'package:flightdeck/app/flightdeck/data/rpc/pi_process_registry.dart';
import 'package:flightdeck/app/flightdeck/data/terminal/sidecar/sidecar_terminal_connector.dart';
import 'package:flightdeck/app/flightdeck/data/tasks/task_process_registry.dart';
import 'package:flightdeck/app/flightdeck/domain/contracts/hook_installer.dart';
import 'package:flightdeck/app/core/data/diagnostics/diagnostics_log.dart';
import 'package:flightdeck/app/core/data/lsp/lsp_process_registry.dart';
...
import 'package:flightdeck/app/core/data/setup/hive_migration.dart';
import 'package:flightdeck/app/core/data/setup/local_network_permission.dart';
...
import 'package:flightdeck/app/core/data/theme_store.dart';
import 'package:flightdeck/app/core/domain/entities/app_settings.dart';
import 'package:flightdeck/app/core/domain/services/window_placement.dart';
import 'package:flightdeck/app/core/env.dart';
import 'package:flightdeck/app/core/ui/automation_controller.dart';
import 'package:flightdeck/app/core/ui/menu/editor_menu_bridge.dart';
...
import 'package:flightdeck/app/core/ui/widgets/bootstrap_error_view.dart';
import 'package:flightdeck/app/core/ui/widgets/devtools_inspector.dart';
import 'package:flightdeck/app/core/ui/widgets/error_report_dialog.dart';
import 'package:flightdeck/app/core/ui/widgets/loading_screen.dart';
...
/// (hooks/registries do flightdeck) e o `core/` não pode importar de feature.
class FlightDeckBootstrapper extends StatefulWidget {
  const FlightDeckBootstrapper({super.key});

  @override
  State<FlightDeckBootstrapper> createState() => _FlightDeckBootstrapperState();
}

class _FlightDeckBootstrapperState extends State<FlightDeckBootstrapper> {
  ...
  void initState() {
    ...
    _lifecycle = AppLifecycleListener(
      onExitRequested: () async {
        ...
        // Encerra o sidecar JUNTO com o app. Sem isto, o `flightdeck-server`
  ...
  Future<void> _initApp() async {
    try {
      ...
      // Configurações (ponteiro fixo em `~/.flightdeck/storage_root`).
      // Subdiretório próprio (`flightdeck`/`flightdeck-debug`) separa debug de
      // produção. Os stores das features são abertos pelos próprios builders
      // async (ver buildFlightDeckModule); aqui só settings + window_state.
      //
      ...
      final Future<void> initTask = (() async {
        ...
        // preservando agents/LSP/tasks de outros FlightDecks ainda vivos.
        await Future.wait([
        ...
        // Hooks do FlightDeck nos harnesses suportados (idempotente) pra sessões
        // de agente nas abas reportarem status de turno: Claude Code em
        // ~/.claude/settings.json, Codex CLI em ~/.codex/hooks.json (+ trust no
        // config.toml), e Pi em ~/.pi/agent/extensions/flightdeck.ts. Não-fatal e
    ...
    return WindowStateKeeper(
      ...
      child: ModularApp(
        ...
        provide: (s) => s
          ...
          // Control e injetada nos controllers da feature FlightDeck.
...
class WindowStateKeeperState extends State<WindowStateKeeper>
  ...
  /// Fechamento da janela: **este** é o encerramento limpo do FlightDeck.
  ///
  ...
  /// marcador de sessão viva e acusa crash. Era o motivo de o aviso "o FlightDeck
