# Changelog — Remote Pi FlightDeck
...
<!--
  ...
  - A seção da versão que está saindo fica no TOPO. O job `meta` do
    .github/workflows/flightdeck-release.yml **falha a release** se a versão do
  ...
  - Markdown normal (parágrafo, `### Fixed`, lista, `**negrito**`, `código`) —
    o CI converte pra HTML (flightdeck/packaging/release_notes_html.py) antes de
...
- **Postgres and SQL Server queries with accents no longer fail.** Any SQL
  ...
  `flightdeck db` CLI and `.dbq` files all benefit.
...
doing: a **Gallery** tab that lists FlightDeck's special documents, and a
...
- **Gallery tab** in the right panel, next to Database. One card per special
  ...
  (`.ckp`), HTTP requests (`.http`), HTML view, Tasks (`.flightdeck/tasks.json`)
...
- **`flightdeck note` in the internal CLI.** `flightdeck note add <dir.notebook>
  --title … [--tag …] [--body … | --body -]` writes a note with the right
  frontmatter (the `agent` tag is always added) and refreshes the open tab;
  `flightdeck note list` lists titles and tags; `flightdeck open x.notebook` opens
  the notebook. The Claude Code skill and `docs/notebook.md` explain the
  format; run `flightdeck install-skill --force` to refresh the local skill.
...
- `.flightdeck/` folder and `.ckp` files use the FlightDeck logo in the file tree;
...
- **`flightdeck db` now finds the connections of a remote workspace.** The
...
- **`flightdeck db` now works from a terminal on a remote host.** It answered
...
- **The background server no longer outlives the app.** Closing FlightDeck left
  `flightdeck-server` running. Because an update replaces its file on disk while
...
- **`flightdeck close-tab`** in the internal CLI - the counterpart of `new-tab`,
...
- **FlightDeck no longer opens to a blank screen on Windows.** A workspace with no
...
- **The internal CLI now installs reliably.** Every boot logged a failure while
  copying `flightdeck.exe` into place, because the Claude and Codex hook installers
...
- **Remote sessions no longer freeze on iPad and Android.** Connecting to a
  ...
  altogether — file tree, terminal, tasks, everything. FlightDeck was issuing far
...
- Remote terminals on a Windows host opened forever empty: the host tried to
  ...
  PowerShell (or `cmd` on Windows ARM). Requires updating FlightDeck **on the
  host** as well.
...
**Still a beta for the upcoming 2.0.0.** The `flightdeck` CLI now has a short
name, connecting to a Windows machine says what is actually wrong, and closing
the window no longer hangs.

### Added

- **`deck` is the same command, shorter.** `deck list-tabs` == `flightdeck list-tabs`,
  in local and remote terminals. If you already have your own `deck` alias, yours
  still wins.

### Fixed

- **The remote CLI answered from the wrong device.** With two FlightDecks attached
  ...
  to the FlightDeck that owns the tab.
- **Pointing at a Windows machine failed with `FormatException: Missing
  ...
  character, hiding the real error. FlightDeck now says plainly that it connects
...
- **The server on your host never updated.** Once installed, it stayed on that
  ...
  remote `flightdeck` CLI answered nowhere and last release's Source Control fixes
...
ones: multi-repo folders work over SSH, and the `flightdeck` CLI now answers in
...
- **`flightdeck` works in a remote terminal.** An agent over SSH can query the
  workspace's databases, read another tab, send text to it, open a file or a new
  tab — all handled by the FlightDeck on your machine. Commands whose argument
...
- **A hidden folder with changes showed up as a file in Source Control.** A new
  `.flightdeck/` appeared as a single entry named `.flightdeck` on remote workspaces;
...
tab: write a request, hit it, read the response, without leaving FlightDeck.
...
- **`flightdeck http list` and `flightdeck http run` in the CLI.** Agents can fire a
...
- **The model list only offers what your account can actually use.** Each
  ...
  (Copilot, Claude Code) FlightDeck stops pretending you can choose.

### Added

- **`previewOpen` in `.flightdeck/tasks.json`** (`always` / `start` / `never`):
...
- After an update, FlightDeck no longer keeps talking to the background server
...
- **Windows: local terminals stopped opening in 1.28.5.** A tab would open and
  ...
  Windows — it died on startup, and FlightDeck quietly used its built-in terminal
...
- **Windows: remote terminals opened empty.** Picking the folder worked and the
  workspace appeared, but the tab never showed anything, because FlightDeck asked
  the remote machine to start *its own* shell — PowerShell on a Mac. The host
  now chooses its shell, and FlightDeck no longer sends its local `PATH` along,
...
- **Connecting to a remote host from Windows always failed** with `Bad local
  forwarding specification`. FlightDeck was asking SSH to open the local end of
...
- **A host that already had the server installed could refuse to start it**
  when the two machines ran different operating systems. FlightDeck now starts the
...
- **A remote host you had never connected to could not be added at all.** SSH
  ...
  open a terminal and connect by hand once. FlightDeck now shows you the host's
...
- **Automatic reconnect**: when a host drops, FlightDeck keeps retrying and shows a
...
- **Built-in browser pane**: open a web page right inside FlightDeck, with a
...
- **`flightdeck browse <url>`**: open (or reuse) a browser tab from the CLI.
...
- **Turn status for the Codex CLI.** A tab running `codex` now shows the
  ...
  too.
- FlightDeck sets this up on its own at startup, including Codex's hook trust, so
...
- **Restoring a tab reattaches the right session.** FlightDeck now remembers which
...
- **Themes.** One choice now paints the whole app: interface, code highlighting
  ...
  **FlightDeck** to **Pantera** (pure black in dark, pure white in light), each
...
- **"FlightDeck closed unexpectedly" on every launch (Windows).** Closing through
  the title bar X destroyed the window without FlightDeck noticing, so the next
  launch always assumed a crash, and dismissing the notice did not help because
  the notice is not what clears it. FlightDeck now handles the closing itself.
...
- **Deleting a workspace folder no longer bricks the app.** FlightDeck used to
...
FlightDeck now speaks your language, and can write your commit messages for you.

### Added

- **Interface in English, Português (BR) and Español.** FlightDeck follows your
...
### Added
- **`flightdeck send --enter`**: types the text and presses Enter in one call,
  instead of always pairing `send` with `send-key Enter`.
- **`flightdeck send --focused`**: targets the tab you are looking at, so external
  tools (a dictation app, a script) no longer need a tab id to type into.
- **The CLI works from outside a FlightDeck terminal.** With no environment
...
- **Intel Macs had no internal CLI and no Claude turn status.** The app itself
  ...
  `flightdeck` command and the spinner/chime silently did nothing there.

### Changed
- The `flightdeck-hook` helper is now `flightdeck hook`, a subcommand of the CLI. One
...
- **`flightdeck mongo --database <nome>`:** o agente escolhe a base sem mexer no
...
### Added
- **CLI `flightdeck new-tab`:** agentes abrem abas de terminal (cwd, título,
...
- **Motores absorvidos pro repo:** emulador de terminal (xterm) virou módulo
  interno; PTY nativo virou o plugin `flightdeck_pty`; frontmatter YAML do
...
- **CLI `flightdeck redis|mongo browse`:** o agente abre a view já filtrada pro
  humano (`--pattern` / `--filter`), sem expor credenciais.
- Logos de marca (Redis/MongoDB) nas abas dos browsers.

### Fixed
- `flightdeck mongo` agora devolve ObjectId/Date como extended JSON canônico
  (antes saía hex/string ambíguos).

## [1.12.0] — 2026-07-19

Novo: acesso a bancos de dados direto no FlightDeck — painel de conexões, tab de
query `.dbq` e a CLI `flightdeck db` para os agentes.

### Added
- **Painel Database:** conexões por workspace (`.flightdeck/databases.json`),
...
- **CLI `flightdeck db list|schema|query|execute|run`** — JSON de uma linha para
  os agentes; execução no app, credenciais nunca passam pela CLI.
- **Redis e MongoDB via CLI** (`flightdeck redis` / `flightdeck mongo`) — acesso do
...
### Changed
- **CLI interna (`flightdeck`):** nomenclatura alinhada pra *tab* — `list-tabs`,
  `read-tab`, `$FLIGHTDECK_TAB_ID`. Os antigos `list-panes`/`read-pane`/
  `$FLIGHTDECK_PANE_ID` seguem como aliases de compatibilidade.
...
- **Worktrees** passam a morar em `.flightdeck/worktrees` (antes `.pi/`), com
  `.flightdeck/worktrees/` garantido no `.gitignore` do repo.
...
### Added
- **Self-update (plano 47):** FlightDeck agora se atualiza sozinho no macOS e no
...
Primeira release distribuível do FlightDeck (cliente desktop do Remote Pi).

### Adicionado
- Identidade de release: app ID `dev.flightdeck.desktop`, nome de exibição
  **Remote Pi FlightDeck** nas três plataformas.
