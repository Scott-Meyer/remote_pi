# Empacotamento & Release — FlightDeck

Runbook de build/empacotamento das 3 plataformas. Base pro job de CI
(`.github/workflows/flightdeck-release.yml`, plano 43 passo 3). Plano de
referência: [`../../plan/43-flightdeck-packaging.md`](../../plan/43-flightdeck-packaging.md).

## Identidade (passo 1 — feito)

| Item | Valor |
|---|---|
| App ID (macOS bundle id / Linux app id) | `dev.flightdeck.desktop` |
| Nome de exibição | **Remote Pi FlightDeck** |
| Binário | `flightdeck` (Linux/Windows) / `FlightDeck` (macOS) — **não** renomeado |
...
- Linux: `.desktop` + ícones hicolor + `dev.flightdeck.desktop.metainfo.xml`
...
Config: `distribute_options.yaml` (raiz do flightdeck) + um `make_config.yaml` por
...
APP="build/macos/Build/Products/Release/FlightDeck.app"
...
DMG="dist/RemotePiFlightDeck-1.0.0-macos-universal.dmg"
hdiutil create -volname "Remote Pi FlightDeck" -srcfolder "$STAGE" -ov -format UDZO "$DMG"
rm -rf "$STAGE"
...
xcrun notarytool submit "$DMG" \
  --key "/Users/jacob/Library/Mobile Documents/com~apple~CloudDocs/Flutterando/RemotePi/FlightDeckApp/AuthKey_3Y2J8MA3M4.p8" \
  --key-id 3Y2J8MA3M4 \
...
`RemotePiFlightDeck-Setup-<v>-windows-x64.exe`.
...
falha em `file INSTALL cannot copy ... to /usr/local/flightdeck: Permission
...
`dist/<versão>/flightdeck-<versão>-linux.deb`; instalar com `sudo dpkg -i`, **com o
...
|---|---|---|
| macOS | `.dmg` notarizado | **`FlightDeck-<v>-macos.zip`** = `ditto` do `.app` notarizado+**stapled** |
...
- **Privada** (NUNCA commitar): backup em
  `…/CloudDocs/Flutterando/RemotePi/FlightDeckApp/sparkle_ed25519_private_key.txt`
  (iCloud, junto dos certs Apple) **e** no secret do GitHub `SPARKLE_PRIVATE_KEY`.
  Gerada via Sparkle na conta de keychain `remote-pi-flightdeck`.
...
./macos/Pods/Sparkle/bin/generate_keys --account remote-pi-flightdeck
# exporta a privada pro iCloud (pede "Allow" no Keychain)
./macos/Pods/Sparkle/bin/generate_keys --account remote-pi-flightdeck -x \
  "/Users/jacob/Library/Mobile Documents/com~apple~CloudDocs/Flutterando/RemotePi/FlightDeckApp/sparkle_ed25519_private_key.txt"
...
job assina `FlightDeck-<v>-macos.zip` e o `.exe` com PyNaCl (a `SPARKLE_PRIVATE_KEY`)
...
(`/Users/flutterando/flightdeck/data/`). Até subir, ninguém se auto-atualiza. As feed
URLs são `https://rp-s3.jacobmoura.work/downloads/flightdeck/appcast-{macos,windows}.xml`.
...
- E2E real: subir um appcast apontando pra v(N+1) e ver um flightdeck v(N) se atualizar.

## Próximos passos (plano 43)

- Passo 3: `.github/workflows/flightdeck-release.yml` (trigger `flightdeck-v*`). **Feito**
