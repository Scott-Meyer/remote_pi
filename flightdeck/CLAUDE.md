# Remote Pi — FlightDeck (Flutter Desktop)
...
remoto). Plano de referência: [`../plan/37-desktop-flightdeck.md`](../plan/37-desktop-flightdeck.md).
...
| # | Decisão |
|---|---|
| **A** | Código mora aqui em `flightdeck/` (não dentro de `app/`). Reuso futuro com `app/` via `packages/pi_core` — **ainda não extraído** |
...
> **Diverge do `app/` de propósito**: o flightdeck é organizado em **fatias verticais
...
Mapa dos motores que o FlightDeck usa — quem é **nosso** (no repo, manutenção
...
| **Render do terminal** | `flterm` + `lib/app/core/terminal/flightdeck_terminal*.dart` | `flterm` renderiza Ghostty; a view/painter interna permanece integral para xterm. Ver `core/terminal/CLAUDE.md` |
| **PTY** (spawn nativo de shell — forkpty/ConPTY) | `plugins/flightdeck_pty/` | **Nosso** — plugin C/FFI absorvido do `kyroon_pty` v1.0.6, renomeado; não publicado |
...
| **Agente** (`pi --mode rpc`) | `lib/app/flightdeck/data/rpc/` (spawn/stream/kill) | Motor é o binário `pi`; nosso é o harness RPC. Protocolo em `docs/rpc-protocol.md` |
| **DB drivers** (SQLite/Postgres/MySQL/MSSQL/Mongo/Redis) | pacotes `anaki_*` (Rust/FFI, do Jacob) + `lib/app/flightdeck/data/db/` (Isolate workers + serviços) | Externo-mas-nosso (mantido pelo Jacob fora do repo) |
| **Git** | `lib/app/flightdeck/data/filesystem/git_*` (roda o binário `git`) | Motor é o git do sistema; nosso é o parser/orquestração |
| **Mídia** (áudio/vídeo no viewer) | pacote `media_kit` (libmpv) | Externo |
| **Self-update** | pacote `auto_updater` (Sparkle/WinSparkle) + `lib/app/flightdeck/data/update/` | Externo + integração nossa (plano 47) |
...
|---|---|---|
| **Rust** (`cargo`, via [rustup](https://rustup.rs)) | a CLI interna (`cli/`) é um crate Rust, compilado por macOS/Linux/Windows e empacotado ao lado do binário | build para no alvo `flightdeck-cli` |
...
- `rustup target add aarch64-unknown-linux-gnu` — uma vez; o build macOS usa
  esse target + Zig para embarcar o flightdeck-server/CLI dos hosts Linux arm64
...
└── app/
    ...
    ├── flightdeck/              # FEATURE: o shell (projetos | panes/agentes/terminal | arquivos)
    │   ├── flightdeck_module.dart   # path '/', binds + route('/', provide: FlightDeck/Setup/Update VMs)
    │   └── domain/  data/  ui/   # ui/ = flightdeck_page + viewmodels/ session/ states/ widgets/
...
- **Imports**: relativos dentro do mesmo feature; absolutos via `package:flightdeck/...`
...
- **Injeção via `.new`** (regra): registre binds e ViewModels com o **tear-off do
  ...
  - **vários primitivos ambíguos** (vários `String`): troque por um **value object
    injetável** (ex.: `UpdateTarget` no `flightdeck_module`).
...
O flightdeck é internacionalizado com [`slang`](https://pub.dev/packages/slang)
...
- **Saída da CLI interna** (`flightdeck send`, `orchestrate`, `db`, e o parser
  ...
  `flightdeck_cli_handler` seguem `Result<_, String>` **por decisão**, não por
  esquecimento.
...
- Editar arquivos fora de `flightdeck/`
- Adicionar **relay, mesh, crypto ou pareamento** nesta fase — FlightDeck é
