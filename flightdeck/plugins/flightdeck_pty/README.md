...
```yaml
dependencies:
  flightdeck_pty: ^0.4.2
```

```sh
flutter pub add flightdeck_pty
```
...
```dart
import 'dart:convert';
import 'package:flightdeck_pty/flightdeck_pty.dart';
...
`flightdeck_pty` always sets `TERM=xterm-256color` and `LANG=en_US.UTF-8` (so tools
...
* **Local** — the shell runs on *this* machine; bytes come from `flightdeck_pty`.
...
```
   ...
   │  real PTY  (flightdeck_pty / node-pty / …)     │        │  pub/sub + buffer     │        │  Flutter app   │
   │                                             │        │  (e.g. Redis)         │        │  (xterm)       │
...
1. **Own the real PTY on the host.** Spawn the shell/process in a pseudo-terminal
   *on the target machine* (this is where `flightdeck_pty` itself can run, or
...
* `example/lib/pty_websocket_server.dart` — `PtyWebSocketServer`: runs on the
  **host** (a desktop app using flightdeck_pty), spawns a real `Pty`, and serves it
...
```dart
import 'package:flightdeck_pty_example/pty_websocket_server.dart';
...
```dart
import 'package:flightdeck_pty_example/pty_session.dart';
import 'package:flightdeck_pty_example/remote_pty_backend.dart';
import 'package:flightdeck_pty_example/websocket_pty_transport.dart';
...
* `lib/` — the Dart API in `flightdeck_pty.dart`, calling the native library via
  `dart:ffi`. Bindings in `lib/src/flightdeck_pty_bindings_generated.dart` are
  generated from `src/flightdeck_pty.h` by [`package:ffigen`](https://pub.dev/packages/ffigen)
...
After editing the native header `src/flightdeck_pty.h`, regenerate the FFI
bindings:
