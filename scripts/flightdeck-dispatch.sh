...
# Despacha uma tarefa orquestrada pra um pane do FlightDeck (via a CLI interna
# `flightdeck`). Espelha scripts/cmux-dispatch.sh, mas usa o transporte do
# FlightDeck em vez do cmux. Sempre prefixa o prompt com [ORCH:<task-id>] —
...
#
# Uso:
#   scripts/flightdeck-dispatch.sh [--wait [--timeout <s>]] <Pane|tab-id> <task-id> <prompt>
#
# Exemplos:
#   scripts/flightdeck-dispatch.sh Extension 03-ts-codec "Implemente passo 3 do plan/03-protocol.md"
#   scripts/flightdeck-dispatch.sh --wait Extension 03-ts-codec "Implemente..."
#   scripts/flightdeck-dispatch.sh t319 quick-check "roda os testes"   # tab-id direto
#
# Argumentos:
...
#                       encerra (sinal nativo do FlightDeck, cobre agente que
...
#                       um tab-id literal do FlightDeck (ex: t319). O label é o
...
# `flightdeck send` direto. Esse script existe pra não esquecer o marker.
...
command -v flightdeck >/dev/null || {
  echo "erro: 'flightdeck' não encontrado no PATH — você está dentro de um pane do FlightDeck?" >&2
  exit 1
}

panes_json=$(flightdeck list-panes --json 2>/dev/null) || {
  echo "erro: falha ao listar panes (FLIGHTDECK_STATUS_SOCK unset? fora de pane FlightDeck?)" >&2
  exit 1
}
...
' "$target")
  case "$tab_id" in
    NONE)
      ...
      echo "      obs: se o campo 'label' não aparece, o FlightDeck em execução ainda não tem a feature — rebuild/relançar." >&2
      exit 3 ;;
    MULTI:*)
...
# FlightDeck, FLIGHTDECK_PANE_ID é o nosso próprio tab-id — embuta-o num segundo
# marker. O INSTRUCTIONS.md instrui o worker a mandar `flightdeck send --tab-id
...
reply_marker=""
if [ -n "${FLIGHTDECK_PANE_ID:-}" ]; then
  reply_marker="[ORCH-REPLY:${FLIGHTDECK_PANE_ID}] "
fi
...
flightdeck send --tab-id "$tab_id" "$full_prompt" >/dev/null
...
flightdeck send-key --tab-id "$tab_id" Enter >/dev/null
...
if [ "$wait_flag" -eq 1 ]; then
  ...
  while :; do
    ...
    # 2) reforço nativo FlightDeck: working true -> false. Só conta como fim
    # depois de ter visto working=true ao menos uma vez (evita disparar no
    # arranque, antes do claude começar a processar).
    working=$(flightdeck list-panes --json 2>/dev/null | python3 -c '
import json,sys
tid=sys.argv[1]
for p in json.load(sys.stdin):
    if p.get("id")==tid:
