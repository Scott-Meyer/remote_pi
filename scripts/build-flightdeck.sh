...
#
# Options:
#   --install       Installs FlightDeck.app to ~/Applications/FlightDeck.app (with backup)
...
echo "==> Installing CLI to ~/.flightdeck/bin..."
mkdir -p "$HOME/.flightdeck/bin"
cp "$CLI_DIR/target/release/flightdeck" "$HOME/.flightdeck/bin/flightdeck"
chmod +x "$HOME/.flightdeck/bin/flightdeck"
ln -sf flightdeck "$HOME/.flightdeck/bin/deck"

if [ -f "$FLIGHTDECK_DIR/assets/hooks/pi_flightdeck.ts" ]; then
  echo "==> Installing Pi extension to ~/.pi/agent/extensions..."
  mkdir -p "$HOME/.pi/agent/extensions"
  cp "$FLIGHTDECK_DIR/assets/hooks/pi_flightdeck.ts" "$HOME/.pi/agent/extensions/flightdeck.ts"
fi
...
if [ "$DO_INSTALL" = true ]; then
  ...
  BACKUP_APP="$DEST_DIR/FlightDeck.app.bak"
  mkdir -p "$DEST_DIR"
  ...
  TEMP_APP="$DEST_DIR/FlightDeck.app.tmp.$$"
  rm -rf "$TEMP_APP"
