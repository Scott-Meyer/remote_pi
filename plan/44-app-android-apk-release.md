...
canal extra no Android: **APK direto**, no mesmo modelo do plano 43 (flightdeck) —
...
| Tag | **`app-v<versão>`** (separada da `flightdeck-v*`; versão bate com `app/pubspec.yaml`) |
...
flightdeck (passo 4 do plano 43), com 1 artefato:
...
Gate manual idêntico ao do flightdeck: colocar o `latest.json` em
...
configurável, mock + fallback gracioso, mesmo pattern do flightdeck). Instruções:
