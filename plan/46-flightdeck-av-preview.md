# 46 — FlightDeck: preview de áudio e vídeo no visualizador de arquivos

## Contexto

O visualizador de arquivos do FlightDeck hoje renderiza markdown, código, imagem e
...
- **Sealed class `FileView`** (`domain/entities/file_view.dart`) com switch
  exaustivo no widget `FileViewer` (`ui/flightdeck/widgets/file_viewer.dart:11-36`)
...
.github/workflows/flightdeck-release.yml     # linux: apt install libmpv-dev no build
```

## Passos

### 1. Dependências + init (flightdeck/)
...
### 4. Pausa fora de foco (flightdeck/)
...
### 5. Empacotamento Linux (flightdeck/ + raiz)
...
- `flightdeck-release.yml` (job linux x64 e arm64): `apt-get install -y libmpv-dev`
...
- [x] Primeira release com A/V: `flightdeck-v1.2.0` (2026-06-16) — 6 jobs verdes; DMG +21MB / EXE +14MB (libmpv embarcado), deb/rpm +0.3MB (libmpv do sistema, linkou no CI)
