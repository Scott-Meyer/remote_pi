# Plano 48 — FlightDeck Task Run (executor de tarefas com watch/hot-reload)

> Subprojeto alvo: **`flightdeck/`** (Flutter Desktop). Local-only, sem relay.
> Referência de arquitetura: [`37-desktop-flightdeck.md`](./37-desktop-flightdeck.md) e
> as convenções de feature vertical em `flightdeck/lib/app/CLAUDE.md`.

## Contexto

O flightdeck já roda processos (`pi --mode rpc`), tem PTY embutido com cores
corretas e terminal próprio (`FlightDeckTerminal`), detecta raiz de projeto por
...
- **Streaming de output** num terminal (reusa `FlightDeckTerminal`).
...
- **Watch opcional** ("reload ao salvar") implementado pelo flightdeck — porque o
...
Nova feature vertical `flightdeck/lib/app/tasks/` com módulo próprio
...
class TaskRun {
  ...
  final int? pid;
  // output vai pro FlightDeckTerminal; o TaskRun só guarda status/metadata.
}
...
`args`/`env` na mão (campo cru) ou via `.flightdeck/tasks.json`.
...
- **`.flightdeck/tasks.json`** (versionável) só pra customização: profiles, env,
...
redimensionável e colapsável** abaixo da árvore (ver `flightdeck_page.dart:479-539`).
...
- **Subpane = controle; terminal central = janela.** Clicar numa task rodando
  abre/foca o output dela num `FlightDeckTerminal` (aba). Não enche a coluna de logs.
...
observando o save e mandando `r`. O flightdeck reimplementa como watcher genérico
...
|---|---|---|
| Spawn/stream/kill gracioso + anti-órfão | `flightdeck/data/rpc/pi_rpc_process.dart` | molde do runner oneShot/captured |
| PTY real (stdin/stdout/resize, cores) | `flightdeck/data/terminal/pty_terminal_gateway.dart` | runner de watch (TUI/spinners/teclas) |
| Terminal próprio | `flightdeck/ui/widgets/` (`FlightDeckTerminal`) | janela de output das tasks |
...
| Slots da coluna direita | `flightdeck_page.dart:479-539` (`FileTreePanel` `searchPanel`/`footer`) | encaixe do subpane de Tasks |
| Diagnósticos | LSP em curso (memória `project_flightdeck_lsp`) | substitui `problemMatcher` regex |
...
`.flightdeck/tasks.json` (com `cwd` opcional relativo ao arquivo → resolve absoluto
...
**Aceite**: salvar um `.dart` com toggle on → flightdeck manda `r` sozinho e o
...
- [x] Passo 5 — `.flightdeck/tasks.json` loader + `cwd` per-task (mesclado no
...
- [x] Doc do `.flightdeck/tasks.json`: `docs/tasks-json.md` (todos os campos) +
...
2. **Onde persistir**: só Hive, ou também `.flightdeck/tasks.json` versionado desde
   o MVP? (recomendação: JSON desde já, dá portabilidade entre máquinas.)
3. **Diagnósticos**: acoplar ao LSP (`project_flightdeck_lsp`) e pular regex de
