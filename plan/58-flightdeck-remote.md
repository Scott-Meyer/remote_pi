# 58 — FlightDeck Remote: cliente/servidor, hosts acima de realms

> **Status**: RASCUNHO em discussão (2026-08-10). Nada aprovado para implementação.
> Reabre formalmente a decisão B do plano 37 ("FlightDeck é local-only"). Registrar
> em `plan/00-decisions.md` quando este plano for aprovado.

## Contexto

O FlightDeck hoje é 100% local: spawna PTY, roda git, LSP e drivers de DB na
máquina onde a GUI está. Queremos três cenários novos, em ordem de prioridade:

1. **MacBook → iMac**: o iMac roda o FlightDeck (ou só o servidor) e o MacBook
   conecta remotamente, vendo e operando tudo que está lá (estilo tmux attach).
2. **VPS headless**: uma máquina SEM FlightDeck instalado, só com o **servidor**
   standalone (binário leve), à qual qualquer cliente FlightDeck conecta.
3. **iPad**: build Flutter do cliente FlightDeck para iPadOS, obrigatoriamente
...
| # | Decisão |
|---|---|
| **A** | **Sem agente RPC.** O harness `pi --mode rpc` fica FORA do escopo remoto e entra em rota de deprecação. Agentes (pi, claude, codex) rodam como processos dentro de terminais, alinhado ao flag `enableAgent` terminal-first. A fatia `flightdeck/data/rpc/` vira legado. |
...
│ UI (rail, tabs, ghostty,    │  ws/   │ flightdeck-server (Dart AOT)     │
│ viewer, db grid)            │◄──────►│ ├ PTY (flightdeck_pty extraído)  │
...
- No desktop local, o app garante o `flightdeck-server` de pé (serviço de
...
| SSH | as chaves SSH do usuário; servidor escuta só localhost, canal tunelado | a de dar `ssh` na máquina (sem pareamento FlightDeck por cima — seria autenticar 2x) |
...
`flightdeck/lib/app/**/data/` para pacotes Dart puros. **Os pacotes são
...
```
packages/
├── flightdeck_core/      contratos domain, Result, erros tipados (zero deps pesadas)
├── flightdeck_protocol/  mensagens do fio (depende só de core)
├── flightdeck_engine/    impls nativas: pty/dylib, fs, git, anaki, catálogo
│                      (dart:io + dart:ffi — SÓ o servidor importa)
└── flightdeck_remote/    proxies dos contratos falando protocol (cliente importa)

flightdeck-server  → core + protocol + engine  (compõe com auto_injector puro,
...
- Enforcement por pubspec, não disciplina: o target tablet simplesmente NÃO
  importa `flightdeck_engine` — o guardrail do iPad vira erro de compilação
...
`native()` só é composto pelo `flightdeck-server`. O que muda entre os modos é
...
`~/.flightdeck/bin`), guards conhecidos de spawn no macOS (SIG_IGN de SIGPIPE;
...
O socket que atende o `flightdeck` (CLI) e o `flightdeck-hook` (status de turno)
passa da GUI para o `flightdeck-server` — mesmos binários, mesmo protocolo de
...
(`FLIGHTDECK_PANE_ID`, PATH escopado, endereço do socket).
...
app quanto pelo binário `flightdeck-server` (`dart compile exe`). O risco técnico
concentrado, e gate do spike da Wave 0, é o **empacotamento das dylibs FFI
fora do build do Flutter** (flightdeck_pty, anaki_*): carregamento manual de
...
- Nenhum `Process.start`, `dart:ffi` de engine ou path de filesystem local no
  código de cliente dos 4 domínios; tudo atrás dos contratos de `flightdeck_core`.
- Enforcement por pubspec: código nativo vive em `flightdeck_engine`, que o
...
- Transporte no iPad: **revisado em 2026-08-13** — o mobile usa `dartssh2`
  ...
  [`59-flightdeck-ipad.md`](./59-flightdeck-ipad.md), decisão B.
- Teclado virtual + terminal é problema de UX próprio; detalhado no
  [`59-flightdeck-ipad.md`](./59-flightdeck-ipad.md), não neste.

## Passos (waves)

### Wave 0 — Spike: pacotes + servidor Dart mínimo (terminais) ✅ (2026-08-11)
- [x] Pacotes `flightdeck_core` / `flightdeck_protocol` / `flightdeck_engine` /
      `flightdeck_remote` / `flightdeck_server` em `flightdeck/packages/`; binário
      `flightdeck-server` (dart compile exe OK, sem Flutter) composto com
      auto_injector puro; UDS listener + handshake com versão.
- [x] **Gate FFI: PASSOU.** flightdeck_pty compilado standalone com `cc` puro
      ...
      `flightdeck_engine` (a API são 6 funções). Anaki: mesmo caminho, provar
      na Wave 4.
- [x] `docs/remote-protocol.md` (handshake + Terminais; JSONL, envelope,
      offsets, erros); mensagens em `flightdeck_protocol`, mesma classe dos
      dois lados.
...
- [x] Ciclo de vida do sidecar: descoberta pelo socket
      (`~/.flightdeck/flightdeck-server.sock`) ANTES de spawnar; spawn com
      ...
      env `FLIGHTDECK_SERVER_BIN` → `~/.flightdeck/bin` → `build/wave0` (dev,
...
- [x] E2E manual no app real (2026-08-11, validado pelo Jacob): terminais
      ...
      `/bin/zsh -l` filho do `flightdeck-server` (~17MB RSS), GUI conectada no
...
- [x] Empacotamento no bundle `.app` (2026-08-11): `macos/build_server.sh` +
      ...
      `flightdeck-server` (AOT, FATIA ÚNICA do host — AOT Dart não sobrevive ao
      ...
      `libflightdeck_pty.dylib` universal; connector resolve Resources antes de
      `~/.flightdeck/bin`. Verificação do binário por probe socket (NUNCA rodar
      sem --socket: vira servidor órfão — bug real corrigido).
- [ ] **Pendente da wave**: benchmark revalidado dentro do app real.
- **Aceite**: FlightDeck local funciona igual a hoje com terminais servidos pelo
...
- [x] Bridge UI↔backend (2026-08-11): `RemoteHostTerminalGateway`
      ...
      i18n `flightdeck.remoteHost.*` nas 3 línguas (slang gerado) + tradutores
...
- [ ] **Último passo da wave (integração no rail, pra fechar)**: widget do pin
      no `projects_rail` + workspace kind remoto no `FlightDeckViewModel`
...
- [ ] **Pendente da wave**: discard/amend remotos (bloqueados com msg);
      ...
      CLI/`flightdeck-hook` no servidor + rebroadcast.
- **Aceite**: do MacBook, navegar árvore, abrir arquivo, stage/commit no
  iMac; GUI do iMac aberta ao mesmo tempo espelha (decisão D); `flightdeck send`
...
- [x] Anaki + túneis de DB executando no servidor; grid/.dbq no cliente
      ...
      `flightdeck-server` do host via `dart build cli` (native assets do anaki);
      grid, `.dbq` e painéis Redis/Mongo consomem os resultados do host.
      Conexões lidas do `.flightdeck/databases.json` do host; senha cacheada por
...
> **local-only por construção**: a descoberta lê `.flightdeck/tasks.json` do disco
...
> (gate por `isRemoteTerminal` em `flightdeck_page`, 2026-08-12).
>
> **ENTREGUE (2026-08-13)**:
...
> `flightdeck-hook` reporta por um **socket Unix na máquina cliente**
> (`TerminalStatusServer`). Quando o claude roda num **terminal remoto**, o hook
> dispara **no host** — que não tem o `flightdeck-hook` instalado nem alcança o
...
> - [ ] **Bootstrap**: instalar o `flightdeck-hook` no host (empurrar o binário,
>       como o server + dylibs). **Pegadinha**: `flightdeck-hook` é exe Dart AOT →
>       host Linux (VPS) precisa da fatia Linux empacotada; macOS→macOS direto.
> - [ ] **Spawn remoto**: injetar `FLIGHTDECK_TAB_ID` + caminho do socket **do
...
>       remota certa (por `FLIGHTDECK_TAB_ID`) → reusa a lógica de som/notificação
...
> cenário fora-de-casa justificar; relay PRÓPRIO do FlightDeck, não o do
> Remote Pi.
...
- `59-flightdeck-ipad.md` (cliente iPadOS)
- Deprecação formal de `flightdeck/data/rpc/` (pi --mode rpc)
