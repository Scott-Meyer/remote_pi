# Plano 47 — Self-update do FlightDeck (Sparkle + WinSparkle)

> Referências: [`43-flightdeck-packaging.md`](./43-flightdeck-packaging.md) (notify + download),
...
> Sparkle na notarização; E2E real (subir appcast e ver um flightdeck antigo atualizar);
> subir os appcasts no rp-s3 (gate).

## Contexto

Hoje o flightdeck tem **"notify + download manual"** (plano 43): no boot, `UpdateChecker`
...
Resultado pretendido: em macOS/Windows o flightdeck **baixa o update em background**,
...
- **macOS**: chave pública EdDSA vai no `Info.plist` (`SUPublicEDKey`); flightdeck é
...
└── flightdeck_module.dart                        # EDITAR: bind SelfUpdater (switch plataforma) + injeta no VM
flightdeck/lib/app/flightdeck/ui/flightdeck_page.dart   # EDITAR: selfUpdater.initialize() no initState (junto do check())
flightdeck/macos/Runner/Info.plist                # EDITAR: SUPublicEDKey + chaves de auto-update
flightdeck/windows/runner/Runner.rc               # EDITAR: recurso EdDSAPub
flightdeck/windows/packaging/exe/make_config.yaml # EDITAR: Inno CloseApplications/RestartApplications
.github/workflows/flightdeck-release.yml          # EDITAR: zip+sign+appcast macOS; sign+appcast Windows; gate
```
...
2. **Chaves EdDSA.** Gerar par Sparkle (`generate_keys -x privfile`) e par WinSparkle
   ...
   **e** backup junto dos certs na pasta iCloud
   `/Users/jacob/Library/Mobile Documents/com~apple~CloudDocs/Flutterando/RemotePi/FlightDeckApp`
...
4. **ViewModel + card + DI.** `UpdateViewModel` decide modo por plataforma e injeta
   `SelfUpdater`; `UpdateCard` muda texto/ação por `isSelfUpdate`; `flightdeck_module.dart`
   registra `SelfUpdater` com switch (`Platform.isMacOS||isWindows ? AutoUpdaterSelfUpdater : NoopSelfUpdater`);
   `flightdeck_page.dart` chama `initialize()` no `initState` ao lado do `check()` (linha 51).
...
7. **CI macOS.** Notarizar+`staple` o **`.app`** (hoje só o `.dmg`); `ditto -c -k
   --keepParent FlightDeck.app FlightDeck-<v>-macos.zip`; `sign_update` com `SPARKLE_PRIVATE_KEY`;
...
9. **CI publish + versão.** Ajustar o **gate de 6 artefatos** (`flightdeck-release.yml:284`)
   ...
   (`/Users/flutterando/flightdeck/data/`); o job `meta` passa a validar que o **build
   number `+n` foi incrementado**. **Aceite:** release ponta a ponta gera os dois appcasts;
   subir os appcasts no rp-s3 faz um flightdeck antigo se auto-atualizar.

10. **Docs.** Atualizar `flightdeck/packaging/README.md` (runbook: zip/sign/appcast, rotação
...
- [ ] `UpdateViewModel`/`UpdateCard`/`flightdeck_module`/`flightdeck_page` em modo por plataforma; Linux sem regressão
...
- **Codesign do Sparkle**: o `codesign --force --deep` atual (`flightdeck-release.yml:93`)
...
- **Inno + Restart Manager**: confirmar que o flightdeck em execução (per-user) é fechado e
...
3. **macOS**: instalar v(N) do `.dmg`; subir appcast apontando p/ v(N+1) `.app.zip` num
   ...
   `~/.pi/flightdeck/agent-pids` sem órfãos e panes restaurados.
...
6. Release real via tag `flightdeck-v<x>` gera `.app.zip`+appcasts; gate manual publica os
