# 37 — Desktop FlightDeck: cliente visual do Pi via RPC

> **✅ ENCERRADO (2026-07-19).** O plano cumpriu seu papel — o FlightDeck existe,
> roda em produção (1.13.0) e as Waves 1–4 estão entregues. **Mas a identidade
> mudou no caminho**: o plano descrevia um flightdeck *agent-centric* (grid de N
...
- **Plano 26 (daemon mode)** já resolveu a parte difícil: spawn de `pi --mode
  ...
  no spike, ver `flightdeck/docs/rpc-protocol.md`), CWD
  lock (UDS em `~/.pi/remote/locks/<roomId>.sock`, 1 Pi por pasta), config local
  em `<cwd>/.pi/remote-pi/config.json`. O flightdeck é um **gerenciador de processos
  paralelo** ao supervisor: o supervisor serve daemons headless 24/7
  (fire-and-forget, sem streaming); o flightdeck serve agentes **interativos com
...
- **CWD lock (1 Pi por pasta)** é construto da extensão remote-pi, não do Pi puro.
  Importa só quando o flightdeck spawna **com** a extensão (ver decisão B).
...
|---|---|---|---|
| **A** | Onde mora o app | **Pasta própria `flightdeck/`** (não dentro de `app/`). `packages/pi_core` (protocolo + modelos + tema compartilhados com `app/`) é **extração futura — ainda não feita** | UI mobile ≠ UI desktop (não compartilhar `lib/ui`). No MVP, `flightdeck/` é projeto Flutter standalone; o reuso real com `app/` vem depois, quando doer duplicação |
| **B** | MVP carrega a extensão remote-pi? | **Não** — spawna `pi --mode rpc` puro (local, sem relay) | Desacopla o MVP de config de relay/mesh. Sem extensão, sem CWD lock cross-process; dedup é interno ao flightdeck. Quando carregar a extensão (futuro), agentes do flightdeck ficam alcançáveis do celular **de graça** |
| **C** | FlightDeck reusa o supervisor (plano 26) ou spawna próprio? | **Spawna próprio** | Supervisor é fire-and-forget sem streaming (plano 26:285); o flightdeck precisa de stdout streaming pra renderizar. São gerenciadores paralelos; convergência depois se valer |
| **D** | Como o FlightDeck injeta config no Pi | **Via env `REMOTE_PI_DIRECT_CONFIG`** (JSON do `LocalConfig` inteiro inline; precedência sobre o `config.json`; espelha `REMOTE_PI_RELAY`) — **committado+pushado** (bundled em `af66d04`, mensagem enganosa; ver "higiene" abaixo) | FlightDeck spawna `pi --mode rpc` e injeta config sem escrever arquivo na pasta do usuário. Carrega `agent_name`/`workspace`/`worktree`/`auto_start_relay` do plano 38 de graça |
| **E** | Update de config em runtime? | **NÃO** — sem `/remote-pi ctl` via stdin. Updates vão por **env no (re)spawn**: FlightDeck re-spawna com o env atualizado | Restart-only de fato (o único campo imutável é `cwd`); evita um canal de controle paralelo e mantém o env como fonte única no modo FlightDeck |
| **F** | Como o celular pareia/revoga com um agente do FlightDeck | **Caminho A — pelo agente RPC vivo** (`/remote-pi pair` e `/remote-pi revoke` no Pi que já roda; reusam a conexão de relay; sem 2º processo) — **pi-ext feito**: pair `054b16c`, revoke `fe79759` (locais, **não pushados**) | O relay rejeita 2ª conexão lado-Pi com mesmo `(pubkey, roomId)` (`RoomAlreadyOpenError`) e o token vive na memória do **ocupante da room** → quem emite token / recebe `pair_request` / derruba o canal no revoke tem que ser o agente vivo, não um 2º processo |

### Pareamento + revoke RPC-friendly — contrato pro FlightDeck (pi-ext feito; lado flightdeck = próximo)

O FlightDeck gerencia pareamento pela própria UI mandando comandos pelo stdin do
...
**Lado FlightDeck (renderizar o QR + fechar no `paired` + UI de revoke) é o próximo
...
`flightdeck/` segue a **mesma arquitetura em camadas do `app/`**, com `CLAUDE.md` por
...
```
remote_pi/
├── flightdeck/                            ← NOVO: app Flutter desktop (pkg `flightdeck`, macOS)
...
> O `PiRpcProcess` (spawn/stream/send/kill) mora em `flightdeck/lib/data/rpc/`
...
> `flightdeck/lib/data/rpc/` + schema em `flightdeck/docs/rpc-protocol.md`. Detalhes em
...
escrito em `plan/37-desktop-flightdeck.md` ou doc anexo. **Se isso não fechar, o
...
- `flightdeck/` Flutter roda no macOS com tema dark próprio (`ui/core/themes`).
- `PiRpcProcess` (em `flightdeck/lib/data/rpc/`): spawn/stream/send/kill com base no
  schema do spike.
- UI single-pane: escolher pasta → ver o agente bootar → mandar prompt → stream
  renderiza (texto + tool calls). Sem multiplexação ainda.
- Dedup interno: flightdeck não abre a **mesma pasta** em dois panes (decisão B —
...
- Painel direito: árvore **read-only** da pasta do pane ativo. Clique no arquivo →
  viewer read-only. (Editar é trabalho do agente, não do flightdeck — a árvore é pra
...
- [x] 0 — Spike: spawn/stream/kill de `pi --mode rpc` validado (headless `dart run
      ...
      `flightdeck/docs/rpc-protocol.md`
- [x] A/B/C — decisões confirmadas pelo usuário (2026-06-05; registradas acima)
- [x] Scaffold — `flightdeck/` criado (Flutter, pkg `flightdeck`) + camadas espelhadas
      do `app/` com `CLAUDE.md` por camada; `scout-flightdeck` + pane registrados
- [x] 1 — `flightdeck/` roda no macOS; `PiRpcProcess` em `data/rpc/`; single-pane com
...
- **Protocolo** (canônico em `flightdeck/docs/rpc-protocol.md`): framing JSONL
...
- **macOS — duas pegadinhas de ambiente**:
  1. App **não herda o PATH do shell** → `pi` resolvido por caminhos conhecidos ou
     `--dart-define=FLIGHTDECK_PI_PATH`.
...
- **Provider/model de demo**: default da máquina é `ollama` (estava fora no teste).
  Rodar a demo com `--dart-define=FLIGHTDECK_PI_PROVIDER=deepseek
  --dart-define=FLIGHTDECK_PI_MODEL=deepseek-chat` (chaves já em `~/.pi/agent/auth.json`).
...
- **FlightDeck e supervisor (plano 26) são gerenciadores paralelos.** Um agente está
  no flightdeck **ou** é daemon do supervisor, não os dois (CWD lock, 1 Pi por
  pasta). Convergir (flightdeck "adota" um daemon) é evolução, não MVP.
- **MVP é local-only (decisão B).** Agentes do flightdeck **não** são alcançáveis do
  celular até a wave futura que carrega a extensão remote-pi no spawn. Aceito pra
  desacoplar o MVP do relay/mesh.
- **`send` é streaming, mas a árvore é read-only.** O flightdeck não edita arquivos;
...
- Editor de código embutido (o agente edita; o flightdeck observa).
...
- **Reachability remota** — spawnar `pi --mode rpc -e <remote-pi dist>`: o agente
  do flightdeck também entra no relay e fica alcançável do celular **de graça**
  (stdout pro flightdeck, socket de relay pra mobile — canais distintos no mesmo
  ...
  (decisão F + contrato acima); falta o lado FlightDeck renderizar o QR.
- **Attach a Pi já rodando** — em vez de recusar pasta com Pi ativo (terminal ou
  daemon), o flightdeck *anexa* via malha e observa/dirige o agente existente,
...
- **Convergência flightdeck ↔ supervisor** — "promover" um pane do flightdeck a daemon
