# 42 — FlightDeck: Worktrees por workspace

## Contexto

O FlightDeck (plano 37) tem hoje uma lista **plana** de workspaces no rail esquerdo
— cada workspace é um `Project` (`flightdeck/lib/domain/entities/project.dart`:
`id`/`name`/`path`/`colorValue`), persistido em Hive
(`hive_project_repository.dart`), com sua própria árvore de panes
(`FlightDeckViewModel._trees: Map<projectId, PaneNode>`) e um `_GitBadge`
...
- **Reusa a máquina existente do FlightDeck**: um worktree é um `Project` de 1ª
...
Segue a regra de ouro do `flightdeck/` (ui → domain ← data; config injeta). O lado
...
└── ui/flightdeck/
    ├── viewmodels/flightdeck_viewmodel.dart ← reconciliação de worktrees por pai, kill-on-remove,
...
- `Project` ganha `parentId`. `FlightDeckViewModel` reconcilia os forks de cada
...
- [~] 2 — `Project.parentId`; reconciliação de forks no `FlightDeckViewModel`
