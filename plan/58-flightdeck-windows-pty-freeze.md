# Plano 58 — Fix: FlightDeck Windows congela ao escrever no PTY

**Objetivo**: eliminar o congelamento total da UI do FlightDeck no Windows
...
```
flightdeck.exe (message loop)
→ user32!DispatchMessageWorker
→ flutter_windows (platform message dispatch)
→ código Dart (AOT)
→ flightdeck_pty!pty_write            (FFI síncrono)
...
O código em `plugins/flightdeck_pty/src/flightdeck_pty_win.c:507-516`:
...
`flightdeck/CLAUDE.md`). Os backends POSIX (`forkpty`) não têm flush — só
Windows trava.

### Evidência

- Minidump com stacks: `C:\Users\fabio\.flightdeck\dumps\flightdeck-hang-149976.dmp`
  (19,7 MB)
...
Fix em duas camadas, no plugin `flightdeck_pty` (Windows):
...
```text
flightdeck/plugins/flightdeck_pty/
└── src/
    └── flightdeck_pty_win.c    # pty_write sem FlushFileBuffers (+ writer
...
- `flutter analyze` / testes existentes do flightdeck verdes.
- Bump de versão do flightdeck + entrada no CHANGELOG descrevendo o fix.

## Definition of Done

- [x] `FlushFileBuffers` removido do `pty_write` (flightdeck_pty_win.c).
...
- [x] Repro do congelamento validado como corrigido no Windows. Evidência
      (2026-08-13, harness em C incluindo o `flightdeck_pty_win.c` real, com
      ...
      `flightdeck/plugins/flightdeck_pty/test/windows/` (`build.ps1`).
...
- [x] CHANGELOG do flightdeck atualizado (1.26.1+61; plugin 1.0.7).
...
- CLI `flightdeck` sem descoberta de socket fora das tabs no Windows
  (`~/.flightdeck` sem socket) — impede orquestração externa nesta plataforma.
