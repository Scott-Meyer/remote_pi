# 60 — FlightDeck Mobile: polish do MVP (iOS/Android)

## Contexto

O cliente mobile (plano 59) está funcional em iPad real: conecta por SSH ao
`flightdeck-server` do host, abre terminal (login shell do host, oh-my-zsh), árvore
...
device), **sempre landscape**, superfície enxuta (ver `59-flightdeck-ipad.md`).
...
| **C** | Nome/ícone do app | "**FlightDeck Remote**" + ícone próprio **só no mobile** (iOS/Android). Desktop segue "FlightDeck". |
...
- **B2. Scroll vs seleção no terminal touch.** Na camada de gesto do terminal
  (`FlightDeckTerminalGesture`): 1 dedo arrastando = scroll do buffer; seleção de
...
- **C1. Seção de ajuda na aba Remote hosts.** Explicar que o host precisa do
  FlightDeck (desktop) ou do `flightdeck-server` instalado, e mostrar a chave pública
...
- **D1. Diagnóstico.** Rodar uma task remota com log instrumentado: confirmar se
  ...
  `FlightDeckTerminalController`. Hipótese primária: a aba (`TaskOutputSession`) não
...
- **E1. Bundle IDs com flavor `.debug`.** Espelhar o padrão desktop
  (`dev.flightdeck.desktop` / `.debug`) em iOS (schemes/xcconfig) e Android
...
- **E2. Nome + ícone "FlightDeck Remote" (decisão C), só mobile.** `CFBundleDisplayName`
  / label Android = "FlightDeck Remote"; ícone próprio (iOS asset catalog + Android
  mipmaps/adaptive). Desktop inalterado.
  - Aceite: home screen mostra "FlightDeck Remote" + ícone novo; desktop segue "FlightDeck".
...
- [ ] B2 — scroll de 1 dedo no terminal sem virar seleção. Investigado: as
      camadas do FlightDeck já ignoram touch pra seleção (`terminal_pane` retorna
      cedo em `PointerDeviceKind.touch`; `FlightDeckTerminal` só rola no touch). O
...
- [x] C1 — seção de ajuda (requisito flightdeck/server + chave) na aba Remote hosts
...
- [ ] E2 — nome/ícone "FlightDeck Remote" no mobile
...
- **Som/turn-status no terminal remoto**: confirmado que é gap de transporte
  ...
  (flightdeck-server detecta o hook e emite evento no protocolo RPC). É a Wave G.
...
Decisão (2026-08-16): **embarcar o `flightdeck-cli` no server**. Ele é necessário
no modo server de qualquer forma (`send`/`send-keys`/`list-panes` cross-pane),
e já traz o `flightdeck hook` — então instalar o hook no `~/.claude` do host e ter
...
- [x] G1 — server sobe `TurnStatusReceiver` (socket `<sock>.status` no host) +
      injeta `FLIGHTDECK_STATUS_SOCK` nas PTYs + instala o hook do Claude no
      `~/.claude` do host (`HostHookInstaller`, resolve a CLI ao lado do server).
      `build_server.sh` embarca a `flightdeck` CLI no bundle; o bootstrap desktop a
...
- Auto-update / versionamento de protocolo do `flightdeck-server` remoto (pendência
