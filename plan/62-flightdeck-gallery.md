# 62 — FlightDeck: aba Gallery (documentos especiais)
...
O painel direito do FlightDeck tinha quatro abas: Files, Search, Source Control e
...
> Documentos especiais do FlightDeck para que você tenha o visual do que o agente
...
├── ui/viewmodels/flightdeck_viewmodel.dart    # createFromTemplate(template) → Result<String, FileOperationError>
└── ui/flightdeck_page.dart                    # fiação + diálogo de erro
flightdeck/test/domain/gallery_template_test.dart
flightdeck/test/ui/gallery_panel_test.dart
```
...
`todo`, `http`) e o logo do FlightDeck para `.ckp`.
...
- **Tarefas** cria `.flightdeck/tasks.json` com o exemplo do painel de Tasks (o
...
| **N7** | Na árvore a pasta é **item único** com o logo do FlightDeck, sem expandir (como o `.app` do Finder). Duplo clique abre a tab |
...
- ~~**CLI**~~ FEITO: `flightdeck note add|list`, `flightdeck open x.notebook`,
  `docs/notebook.md`, seção Notebooks na skill (`flightdeck install-skill --force`
...
- [x] Cards Tarefas (`.flightdeck/tasks.json`, reutilizando o exemplo do painel) e Visual HTML (passo 2)
