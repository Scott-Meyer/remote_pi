# 59 — FlightDeck no iPad/Android: cliente remoto puro

> Continuação natural do [`58-flightdeck-remote.md`](./58-flightdeck-remote.md). O 58
> provou o cliente/servidor (terminais, arquivos, git, DB, tasks rodando no host
> via `flightdeck-server`, transporte SSH). Este plano leva o **cliente** para
> iPadOS/Android, sem motor local.

## Contexto

O FlightDeck já é cliente/servidor: todo o motor (PTY, git, DB, LSP, tasks) roda no
...
host (iMac/VPS) que já tem o `flightdeck-server` instalado.
...
| **B** | **Transporte único em `dartssh2`** (Dart puro), para desktop **e** mobile. Substitui o `SshTunnel` baseado no binário `ssh` do sistema (plano 58). Verificado: `dartssh2` tem `forwardLocalUnix` (`direct-streamlocal@openssh.com`), então encaminha direto pro socket UNIX remoto `~/.flightdeck/flightdeck-server.sock` — **zero mudança no `flightdeck-server`**. |
...
|---|---|---|
| General → toggle "FlightDeck terminal" | ❌ | sem shell local; pseudo-workspace nem existe |
...
Fora das configs:
- **Pseudo-workspace "FlightDeck" sempre desativado** (nem renderiza no rail).
...
Hoje o `RemoteConnection` (em `flightdeck_remote`) está preso ao `Socket` do
...
| 4 | cliente linka `anaki_*`/`flightdeck_pty` direto? (não deveria — é no host) | isolar atrás de fábrica com fallback |
...
- [x] **iOS**: compilou direto (pods via CocoaPods; `flightdeck_pty`/`media_kit`/
...
em landscape** — titlebar estilo Windows ("☰ FlightDeck"), aba Terminal, teclado
virtual, e o `flightdeck_pty` até **spawnou um shell** (`I have no name!$`, o sim é
...
- [ ] Isolar o **motor-local de DB** (anaki) do app (hoje só o build hook está
      ...
      pubspec do cliente). `flightdeck_pty`/`libghostty` compilam no mobile, então
...
- [x] Gating de Configurações (matriz): esconde Terminal/Languages/Automations/
      Connectivity/Daemons/Scheduling + seções Agent/FlightDeck/Updates do General.
- [x] Pseudo-workspace "FlightDeck" nunca injeta no mobile.
...
- Rodar/instalar `flightdeck-server` no próprio dispositivo mobile (decisão D).
