# Plano 50 — FlightDeck Terminal Profiles (padrão + WSL/PowerShell/cmd + login shell)

> Subprojeto alvo: **`flightdeck/`** (Flutter Desktop). Local-only, sem relay.
> Referência: PTY em `flightdeck/lib/app/flightdeck/data/terminal/pty_terminal_gateway.dart`,
> resolução de shell POSIX em `flightdeck/lib/app/core/utils/login_shell.dart`
> (feito na issue #42), convenções de feature vertical em `flightdeck/lib/app/CLAUDE.md`.
...
Nova feature (pode morar em `flightdeck/lib/app/flightdeck/` junto do terminal, ou
...
- Ícones/temas por perfil no `FlightDeckTerminal`.
