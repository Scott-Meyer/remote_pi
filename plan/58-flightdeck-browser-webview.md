# 58 — FlightDeck: Navegador embutido + preview de markdown/HTML via webview

## Contexto

Duas dores convergem na mesma solução:

1. **Não há navegador no FlightDeck.** Preview de localhost (dev server), docs e
...
### 4. CLI: `flightdeck browse`

- Novo comando na CLI interna: `flightdeck browse <url> [--pane-id <id>]`.
  - Capability `inline`: abre/reusa um `BrowserSession` (no pane do chamador
    por default, mesmo roteamento por `FLIGHTDECK_PANE_ID` do `flightdeck db`).
...
- Documentar na skill `flightdeck-cli` embutida (`install-skill`).

**Aceite**: de dentro de um pane, `flightdeck browse http://localhost:3000`
...
- Ao rodar uma task (plano 48) cujo output emite uma URL local (regex de
  `https?://(localhost|127\.0\.0\.1|0\.0\.0\.0):\d+`), o FlightDeck abre
...
| `flightdeck browse` | abre pane | abre browser do SO (JSON indica `"mode":"system"`) |
...
- [x] `flightdeck browse` na CLI + skill atualizada
...
> Implementado 2026-08-09 na worktree `navegador` (flightdeck/). Persistência da
