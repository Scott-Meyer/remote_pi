# Plano 59 — FlightDeck Windows: retomada sem backlog

**Objetivo**: eliminar a degradação percebida quando o FlightDeck volta ao foco
...
- Confirmar versão do FlightDeck `>= 1.26.1` e que o build Windows inclui a
  implementação de `flightdeck_pty` com writer thread e sem `FlushFileBuffers` no
  caminho de escrita.
...
Executar no subprojeto `flightdeck/`, registrando resultados antes e depois quando
...
- Linhagem: FlightDeck `1.26.1+61`, fix ConPTY `4fb9249`, writer thread ativa e
...
`flightdeck/tool/benchmarks/windows_resume_performance.dart`. Execução AOT, dois
