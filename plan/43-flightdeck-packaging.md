# 43 — FlightDeck: Empacotamento & Distribuição (macOS · Windows · Linux)

## Contexto

O FlightDeck (`flightdeck/`, Flutter desktop) está com MVP funcional: os três runners
existem (`macos/`, `windows/`, `linux/`), lint e testes verdes, e o Linux já tem
integração de desktop (app ID `dev.flightdeck.desktop`, `.desktop`, ícones).
Mas **nada está pronto pra distribuir**: assinatura macOS é ad-hoc, bundle ID é
placeholder `com.example.flightdeck`, `CompanyName` do Windows é `com.example`,
...
| Builds | **GitHub Actions** — matrix macOS/Windows/Linux, trigger por tag `flightdeck-v*` |
| Hospedagem dos artefatos | **GitHub Releases** (tags prefixadas `flightdeck-v*`; CDN/banda grátis). ⚠️ **Revisão 2026-06-12**: a decisão original era "artefatos direto pra VPS via rsync", mas **a VPS não tem acesso SSH** — Releases com tag prefixada resolve o monorepo como puro storage de assets. A VPS (rp-s3) hospeda **só o `latest.json`** (e opcionalmente `SHA256SUMS`), colocado **manualmente** pelo usuário a cada release — funciona como gate de publicação |
...
| Updates | **Sem auto-update in-app** nesta fase. Descoberta de versão nova acontece pelo **Site** (`site/`), via página de downloads alimentada por um `latest.json` na VPS. ⚠️ **Revisão 2026-06-12**: além do site, o FlightDeck ganha um **aviso leve in-app** (mini card no shell, dispensável por versão, consultando o `latest.json`) com download por clique — continua **sem** auto-replace (passo 7) |

### Matriz de artefatos por release

| Plataforma | Arquitetura | Artefato |
|---|---|---|
| macOS | universal (x64+arm64, binário único do Flutter) | `RemotePiFlightDeck-<v>-macos-universal.dmg` (assinado + notarizado + stapled) |
| Windows | x64 | `RemotePiFlightDeck-Setup-<v>-windows-x64.exe` (Inno Setup, sem assinatura) |
| Linux | x86_64 | `remote-pi-flightdeck_<v>_amd64.deb` · `remote-pi-flightdeck-<v>.x86_64.rpm` |
| Linux | arm64 | `remote-pi-flightdeck_<v>_arm64.deb` · `remote-pi-flightdeck-<v>.aarch64.rpm` |
...
- **Dependências runtime do FlightDeck** (`pi` CLI, supervisor, node): **não
...
flightdeck/
  ...
  CHANGELOG.md
.github/workflows/
  flightdeck-release.yml               # trigger: tag flightdeck-v* (+ workflow_dispatch)
site/
  src/app/download/...              # página de downloads (consome latest.json)
VPS:
  /downloads/flightdeck/<versão>/<artefatos>
  /downloads/flightdeck/latest.json    # {version, date, platforms: {url, sha256, size}}
  /downloads/flightdeck/SHA256SUMS
```

## Passos

### 1. Identidade e metadados (flightdeck/)

> **Status 2026-06-12**: concluído via `[ORCH:43-flightdeck-identity-packaging]`.
> Bundle ID já estava `dev.flightdeck.desktop`; display name, Hardened
...
- Bundle ID macOS: adotar **`dev.flightdeck.desktop`** (mesmo app ID que o
  Linux já usa), fixando `PRODUCT_BUNDLE_IDENTIFIER` no pbxproj/xcconfig.
  Nome de exibição: **"Remote Pi FlightDeck"**; binário continua `flightdeck`.
- Windows `Runner.rc`: `CompanyName` → `Jacob Moura`, `ProductName` →
  `Remote Pi FlightDeck`; confirmar que a versão vem dos defines
...
- macOS (manual primeiro, antes de automatizar): assinar com **Developer ID
  ...
  `flightdeck/packaging/README.md` (vira base do job de CI).
...
### 3. Workflow `flightdeck-release.yml` (raiz `.github/`)

> **Status 2026-06-12**: workflow escrito
> (`.github/workflows/flightdeck-release.yml`, YAML validado) no fluxo revisado
...
> `flightdeck-v1.0.0` criada com 6 artefatos + SHA256SUMS + latest.json.
...
- Trigger: push de tag `flightdeck-v*` + `workflow_dispatch`. Primeiro step
...
- Jobs:
  ...
  - `publish` (needs: todos) — **revisado 2026-06-12 (sem SSH na VPS)**:
    ...
    `https://github.com/jacobaraujo7/remote_pi/releases/download/flightdeck-v<versão>/<arquivo>`),
    e cria a **GitHub Release** `flightdeck-v<versão>` com os 6 artefatos +
...
**Aceite**: tag de teste (ex: `flightdeck-v1.0.0`) produz os 6 artefatos +
...
> `VPS_DEPLOY_PATH` (`/Users/flutterando/flightdeck/data`, host macOS) montado
> read-only em `/data/flightdeck`; serve `/downloads/...` com cache/CORS/
...
- A VPS guarda só `/downloads/flightdeck/latest.json` (URL estável, nunca muda)
  e opcionalmente `SHA256SUMS`. As URLs **dentro** do manifest apontam pros
  assets da GitHub Release (`releases/download/flightdeck-v<versão>/...`).
...
**Aceite**: `curl -fsSL https://<host>/downloads/flightdeck/latest.json` retorna
...
> `NEXT_PUBLIC_FLIGHTDECK_MANIFEST_URL` (ou usar o default
> `<host do site>/downloads/flightdeck/latest.json`). Aceite final depende do
> passo 4.
...
- Documentar (em `flightdeck/packaging/README.md`): bump de `version:` no
  pubspec → commit → tag `flightdeck-vX.Y.Z` → push → CI publica → smoke test
  de download nas 3 plataformas → anúncio.
- Iniciar `flightdeck/CHANGELOG.md` (entrada por release; o `notes` do
  `latest.json` pode derivar dele).

**Aceite**: uma release inteira executada só seguindo o runbook, sem passo
improvisado.

### 7. Aviso de atualização in-app (flightdeck/)

> **Status 2026-06-12**: implementado via `[ORCH:43-flightdeck-update-banner]`
...
- No startup, consultar
  `https://rp-s3.jacobmoura.work/downloads/flightdeck/latest.json` (GET com
...
- [x] Identidade aplicada: bundle ID `dev.flightdeck.desktop`, nome "Remote Pi FlightDeck", CompanyName corrigido, versão Windows dinâmica
...
- [x] `flightdeck-release.yml` gera os 6 artefatos + `SHA256SUMS` + `latest.json` a partir de tag `flightdeck-v*`
- [x] Release `flightdeck-v*` no GitHub com 6 artefatos + `SHA256SUMS` + `latest.json`; checksums conferindo após download; `latest.json` no rp-s3 servindo na URL estável (validado 2026-06-12: headers CORS/cache OK, 7 URLs de artefato respondendo, sha256 do DMG conferido)
...
- **Nome interno do pacote rpm** — hoje sai como `flightdeck` (vem do pubspec),
  que **colide com o pacote `flightdeck` do Fedora** (UI de admin de servidor).
  Dar nome próprio (`remote-pi-flightdeck`) no make_config do rpm.
