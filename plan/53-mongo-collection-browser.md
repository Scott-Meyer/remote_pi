...
| C | **Extended JSON lossless**: o runner Mongo passa a usar `extendedJsonCodec: false` do anaki — replies mantêm `{"$oid":…}`/`{"$date":…}` como JSON puro. Round-trip de edição sem achatar tipos ("nunca salvar sobre JSON com tipos achatados"). Também conserta o `flightdeck mongo` (hoje imprime ObjectId como hex ambíguo) |
| D | **CLI abre a view**: `flightdeck mongo browse --db <conn> <collection> [--filter '<json>']` e `flightdeck redis browse --db <conn> [--pattern '<glob>']` — o agente investiga via CLI e abre o browser já filtrado pro humano. Abrir view ≠ executar: não devolve documentos (pra isso já existem `flightdeck mongo`/`flightdeck redis`) |
...
- Skill `flightdeck-cli`: comandos novos + noção de como criar `.flightdeck/databases.json`

## Passos

1. **Runner lossless**: `extendedJsonCodec: false` no `NoSqlRunnerImpl.mongo`.
   Aceite: `flightdeck mongo --command '{"find":…}'` devolve `_id` como `{"$oid":…}`.
...
6. **Skill**: documentar `browse` + snippet de `.flightdeck/databases.json`
...
- [x] `flightdeck mongo browse` / `flightdeck redis browse` abrem a view filtrada
- [x] Skill atualizada (browse + databases.json) — `~/.claude/skills/flightdeck-cli`
      e a cópia embutida do `install-skill` no `flightdeck_cli.dart`
- [x] `flutter analyze` zero issues; testes do serviço passando
- [ ] E2E manual contra Mongo local

Implementado 2026-07-19 (flightdeck/). Notas: `find` usa `singleBatch` (nunca
